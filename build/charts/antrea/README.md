...
| hostNetworkAcceleration.enable | bool | `true` | Enable to accelerate Pod-to-Pod traffic on Node host networking when traffic mode is noEncap or hybrid. Currently only Pod-to-Pod traffic is supported. Pod-to-Pod traffic is supported. External-to-Service and Pod-to-external traffic will be supported in the future. |
