...
| antreaProxy.ProxyHealthServerBindAddress | string | `""` | ProxyHealthServerBindAddress is the IP address that the AntreaProxy health server binds to. If not specified, it will be automatically set to "0\.0\.0\.0" for IPv4-only clusters, or "::" for IPv6-only and dual-stack clusters. Note that "::" may act as either IPv6-only or dual-stack depending on the system setting of IPV6_V6ONLY (sysctl: net.ipv6.bindv6only). |
| antreaProxy.ProxyHealthServerPort | int | `10352` | ProxyHealthServerPort is the port for AntreaProxy health server to serve on. Defaults to 10352. |
