...
| hostNetworkAcceleration.enable | bool | `true` | Enable to accelerate traffic in the Node's host network using nftables flowtable. The supported traffic types are: - Pod-to-Pod traffic (when traffic mode is noEncap or hybrid) - External-to-Service traffic (NodePort, LoadBalancer ingress IPs, and externalIPs, when proxyAll is enabled). Note: When proxyAll is enabled and kube-proxy is running on the Node, External-to-Service traffic to LoadBalancer ingress IPs and externalIPs is not accelerated; it can be accelerated only after kube-proxy is removed. |
