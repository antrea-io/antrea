...
| egress.ignoreUserInterfaces | bool | `false` | Ignore VLAN sub-interfaces of the transport interface not created by antrea-agent during Egress IP reconciliation. When enabled, only sub-interfaces named with the "antrea-ext." prefix are managed, preventing accidental deletion of user-managed VLAN interfaces and their IPs. |
