...
| agent.antreaSysctlInit.mountSysctlDir | string | `"/host/etc/sysctl.d"` | Mount path inside the init container for the host sysctl configuration directory. The init container writes sysctl files to this directory. |
| agent.antreaSysctlInit.overwriteFile | string | `"99-zzzz-antrea.conf"` | Name of the sysctl configuration file created or overwritten by the init container. The filename is chosen to sort last in lexicographic order within the sysctl configuration directory, so that the settings in this file are applied after other distribution- or administrator- provided sysctl configuration files. |
| agent.antreaSysctlInit.resources | object | `{"requests":{"cpu":"100m"}}` | Resource requests and limits for the antrea-sysctl-init init container. |
