...
| hostNetworkAcceleration.enable | bool | `true` | Enable to accelerate traffic in the Node's host network using nftables flowtable. The supported traffic types are: - Pod-to-Pod traffic (when traffic mode is noEncap or hybrid) - External-to-Service traffic (NodePort, LoadBalancer ingress IPs, and externalIPs, when proxyAll is enabled) |
| hostNetworkAcceleration.serviceAccelerationPacketThreshold | int | `20` | Minimum number of packets an external-to-Service connection must have before it is accelerated using nftables flowtable. Applies to external clients accessing NodePort, LoadBalancer ingress IPs, and externalIPs (requires proxyAll to be enabled). The Valid range is 0-1000. Setting to 0 accelerates all external-to-Service connections; higher values (default 20) only accelerate connections after the packet count exceeds the threshold, reducing overhead for short-lived connections. |
