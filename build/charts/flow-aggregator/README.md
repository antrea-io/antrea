...
![Version: 2.8.0-dev](https://img.shields.io/badge/Version-2.8.0--dev-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: latest](https://img.shields.io/badge/AppVersion-latest-informational?style=flat-square)
...
| clusterScopedNameSuffix | string | `""` | Suffix appended to the base name of the chart, used when naming objects whose name must be unique across the whole cluster: currently the RBAC ClusterRole and ClusterRoleBindings, and the RoleBinding created in kube-system. A distinct suffix must be provided for each instance when installing the chart several times (in different Namespaces) in the same cluster. Otherwise, installing the second instance will rebind the shared ClusterRoleBindings to the second ServiceAccount, revoking the permissions of the first instance. |
...
| flowStreamService.enable | bool | `true` | Enable is the switch to enable the FlowStreamService gRPC server (port 14740). The server uses server-side TLS (same self-signed certificate as the gRPC collector). Clients must authenticate with either a Kubernetes bearer token in gRPC metadata or an X.509 client certificate signed by the cluster's client CA, presented during the TLS handshake. Note that the Flow Aggregator's own client certificate is signed by its self-signed CA, not the cluster's, so it cannot be used here. Enabled by default. |
| flowStreamService.installClusterRoles | bool | `true` | InstallClusterRoles installs the antrea-flow-viewer and antrea-flow-identity-viewer ClusterRoles, which administrators bind to grant flow visibility. Unlike every other cluster-scoped object this chart ships, these two are cluster singletons with fixed names: they name no ServiceAccount and no release, so one copy serves every Flow Aggregator in the cluster. A second release of this chart in the same cluster must therefore set this to false and bind against the roles the first release already installed, since Helm refuses to adopt objects another release owns. Ownership follows whichever release installed them, so uninstalling that release deletes both roles: to hand them to a surviving release, set this to true there and helm upgrade it once the previous owner is gone. |
| flowStreamService.maxStreamsPerClientIP | int | `64` | MaxStreamsPerClientIP is the maximum number of concurrent flow streams the server accepts from one client IP address. A stream refused by this limit, or by maxTotalStreams, is rejected with the retryable gRPC ResourceExhausted code. Note that this limit keys on the source IP of the connection and not on the authenticated user, which is why the default is loose: a client which authenticates its own users and opens one stream per user is a single Pod and so a single key, so for it this is in effect a ceiling on concurrent viewers rather than an abuse bound (antrea-ui works this way); and a hostNetwork client, or an external client reaching the server through a NodePort Service with externalTrafficPolicy=Cluster, is seen as the ingress node's antrea-gw0 address, so every client behind that node shares one key. This chart exposes port 14740 only on the flow-aggregator Service, which is a ClusterIP Service that preserves the client Pod IP. |
| flowStreamService.maxTotalStreams | int | `256` | MaxTotalStreams is the maximum number of concurrent flow streams the server accepts across all clients, and the number of concurrent streams it advertises as the limit for a single connection. It must be greater than maxStreamsPerClientIP, so that a client reaching its own limit is answered with ResourceExhausted rather than having its call parked by its own gRPC transport; the Flow Aggregator refuses to start otherwise. Note that this value and maxStreamsPerClientIP are read once at startup, so changing either requires the Flow Aggregator to be restarted. |
...
| recordBufferSize | int | `8192` | Number of entries in the ring buffer used to distribute flow records to exporters. Each exporter independently consumes from the buffer. This defines the maximum number of flow records the buffer can store before new records overwrite the oldest ones. If the value is too small, slower consumers are more likely to lose records, there is less tolerance to temporary consumer unavailability, and fewer historical snapshots are available. If the value is too large, it will result in higher memory usage. Defaults to 8192. |
