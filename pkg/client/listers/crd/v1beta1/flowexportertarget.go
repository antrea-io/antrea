...
type FlowExporterTargetLister interface {
	...
	List(selector labels.Selector) (ret []*crdv1beta1.FlowExporterTarget, err error)
	// Get retrieves the FlowExporterTarget from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*crdv1beta1.FlowExporterTarget, error)
...
type flowExporterTargetLister struct {
	listers.ResourceIndexer[*crdv1beta1.FlowExporterTarget]
}
...
func NewFlowExporterTargetLister(indexer cache.Indexer) FlowExporterTargetLister {
	return &flowExporterTargetLister{listers.New[*crdv1beta1.FlowExporterTarget](indexer, crdv1beta1.Resource("flowexportertarget"))}
}
