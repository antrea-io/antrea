...
type ExternalIPPoolLister interface {
	...
	List(selector labels.Selector) (ret []*crdv1beta2.ExternalIPPool, err error)
	// Get retrieves the ExternalIPPool from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*crdv1beta2.ExternalIPPool, error)
...
type externalIPPoolLister struct {
	listers.ResourceIndexer[*crdv1beta2.ExternalIPPool]
}
...
func NewExternalIPPoolLister(indexer cache.Indexer) ExternalIPPoolLister {
	return &externalIPPoolLister{listers.New[*crdv1beta2.ExternalIPPool](indexer, crdv1beta2.Resource("externalippool"))}
}
