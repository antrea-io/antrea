...
type ClusterNetworkPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.ClusterNetworkPolicy, err error)
	// Get retrieves the ClusterNetworkPolicy from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.ClusterNetworkPolicy, error)
...
// List lists all ClusterNetworkPolicies in the indexer.
func (s *clusterNetworkPolicyLister) List(selector labels.Selector) (ret []*v1alpha2.ClusterNetworkPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.ClusterNetworkPolicy))
	})
	return ret, err
}

// Get retrieves the ClusterNetworkPolicy from the index for a given name.
func (s *clusterNetworkPolicyLister) Get(name string) (*v1alpha2.ClusterNetworkPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("clusternetworkpolicy"), name)
	}
	return obj.(*v1alpha2.ClusterNetworkPolicy), nil
}
