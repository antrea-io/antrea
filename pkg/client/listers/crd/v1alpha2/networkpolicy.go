...
type NetworkPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha2.NetworkPolicy, err error)
...
// List lists all NetworkPolicies in the indexer.
func (s *networkPolicyLister) List(selector labels.Selector) (ret []*v1alpha2.NetworkPolicy, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha2.NetworkPolicy))
	})
	return ret, err
}
...
type NetworkPolicyNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha2.NetworkPolicy, error)
...
// List lists all NetworkPolicies in the indexer for a given namespace.
func (s networkPolicyNamespaceLister) List(selector labels.Selector) (ret []*v1alpha2.NetworkPolicy, err error) {
...
// Get retrieves the NetworkPolicy from the indexer for a given namespace and name.
func (s networkPolicyNamespaceLister) Get(name string) (*v1alpha2.NetworkPolicy, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha2.Resource("networkpolicy"), name)
	}
	return obj.(*v1alpha2.NetworkPolicy), nil
}
