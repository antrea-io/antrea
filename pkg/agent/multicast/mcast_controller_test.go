...
func TestNodeUpdate(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name:          "stale member with no matching Node is removed",
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Ensure the channel is empty before the test
			...
			// The event sends to groupEventCh are synchronous and occur before wg.Wait() returns,
			// so we can just drain the channel completely with a non-blocking loop.
			for len(testController.groupEventCh) > 0 {
				e := <-testController.groupEventCh
