...
func (h *ipFamilyHandler) calculateSkipOffset(chunkIndex int, skipFalse, skipToEnd uint8, needsCheck bool) uint8 {
	if needsCheck {
		if h.etherType == etherTypeIPv4 {
			return skipFalse
		}
...
func (h *ipFamilyHandler) countAddrInstForSkipFalse(srcIP, dstIP net.IP) uint8 {
	var count uint8
	if h.etherType == etherTypeIPv6 {
		...
		if srcIP != nil {
			count += uint8(h.addressChunks * 2) // 8
		}
		if dstIP != nil {
			count += uint8(h.addressChunks * 2) // 8
		}
		...
		if srcIP != nil && dstIP != nil {
			count += uint8(h.addressChunks * 2) // 2
		}
	}
	return count
}
...
func compileIPFilters(handler *ipFamilyHandler, srcIP, dstIP net.IP, size, curLen, skipFalse uint8, needsOtherTrafficDirectionCheck bool) []bpf.Instruction {
	...
	if dstIP != nil {
		for i := range handler.addressChunks {
			...
			// (guaranteed no match).
			needsCheck := srcIP == nil && needsOtherTrafficDirectionCheck
			currentSkipFalse := handler.calculateSkipOffset(i, skipFalse, skipToEnd(), needsCheck)
			inst = append(inst, bpf.JumpIf{Cond: bpf.JumpEqual, Val: addrVal, SkipTrue: 0, SkipFalse: currentSkipFalse})
		}
	}
	return inst
