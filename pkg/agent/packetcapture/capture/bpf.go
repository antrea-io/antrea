...
func calculateSkipFalse(transport *transportFilters, dstIP net.IP) uint8 {
	var count uint8
	if dstIP != nil {
...
func compileIPAndTransportFilters(size, curLen uint8, transport *transportFilters, needsOtherTrafficDirectionCheck bool, srcIP, dstIP net.IP) []bpf.Instruction {
	...
	if srcIP != nil {
		...
		if needsOtherTrafficDirectionCheck {
			inst = append(inst, bpf.JumpIf{Cond: bpf.JumpEqual, Val: srcAddrVal, SkipTrue: 0, SkipFalse: calculateSkipFalse(transport, dstIP)})
		} else {
...
func compilePacketFilter(packetSpec *crdv1alpha1.Packet, srcIP, dstIP net.IP, direction crdv1alpha1.CaptureDirection) []bpf.Instruction {
	size := uint8(calculateInstructionsSize(packetSpec, direction, srcIP, dstIP))
	...
	case crdv1alpha1.CaptureDirectionSourceToDestination:
		inst = append(inst, compileIPAndTransportFilters(size, uint8(len(inst)), &transport, false, srcIP, dstIP)...)
	case crdv1alpha1.CaptureDirectionDestinationToSource:
		...
		inst = append(inst, compileIPAndTransportFilters(size, uint8(len(inst)), &transport, false, dstIP, srcIP)...)
	default:
		inst = append(inst, compileIPAndTransportFilters(size, uint8(len(inst)), &transport, true, srcIP, dstIP)...)
		transport.srcPort, transport.dstPort = transport.dstPort, transport.srcPort
		inst = append(inst, compileIPAndTransportFilters(size, uint8(len(inst)), &transport, false, dstIP, srcIP)...)
	}
...
func calculateInstructionsSize(packet *crdv1alpha1.Packet, direction crdv1alpha1.CaptureDirection, srcIP, dstIP net.IP) int {
	count := 0
	// load ethertype
	count++
	// ip check
