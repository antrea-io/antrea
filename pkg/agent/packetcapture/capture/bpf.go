...
// Generates BPF instructions for matching source and destination IP addresses.
func compileIPFilters(srcIP, dstIP net.IP, size, curLen uint8, transport *transportFilters, needsOtherTrafficDirectionCheck, isRetTraffic bool) []bpf.Instruction {
	...
	if srcIP != nil {
		// To avoid loading the Source IP address when capturing return traffic (direction == Both)
		// and both srcIP & dstIP are specified,
		if !isRetTraffic || dstIP == nil {
...
func compilePacketFilter(packetSpec *crdv1alpha1.Packet, srcIP, dstIP net.IP, direction crdv1alpha1.CaptureDirection) []bpf.Instruction {
	...
	case crdv1alpha1.CaptureDirectionSourceToDestination:
		inst = append(inst, compileIPFilters(srcIP, dstIP, size, uint8(len(inst)), &transport, false, false)...)
	case crdv1alpha1.CaptureDirectionDestinationToSource:
		...
		inst = append(inst, compileIPFilters(dstIP, srcIP, size, uint8(len(inst)), &transport, false, false)...)
	default:
		inst = append(inst, compileIPFilters(srcIP, dstIP, size, uint8(len(inst)), &transport, true, false)...)
		inst = append(inst, compileTransportFilters(size, uint8(len(inst)), &transport)...)
		transport.srcPort, transport.dstPort = transport.dstPort, transport.srcPort
		inst = append(inst, compileIPFilters(dstIP, srcIP, size, uint8(len(inst)), &transport, false, true)...)
	}
	inst = append(inst, compileTransportFilters(size, uint8(len(inst)), &transport)...)
