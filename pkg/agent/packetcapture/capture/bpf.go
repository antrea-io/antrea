	...
	if transport.srcPort > 0 || transport.dstPort > 0 || len(transport.tcpFlags) > 0 || len(transport.icmp) > 0 {
		...
		if len(transport.icmp) > 0 {
			...
			for i, m := range transport.icmp {
				...
				inst = append(inst, bpf.JumpIf{Cond: bpf.JumpEqual, Val: m.icmpType, SkipTrue: skipTrue, SkipFalse: skipFalse})
				if m.icmpCode != nil {
					inst = append(inst, loadIPv4ICMPCode)
					inst = append(inst, bpf.JumpIf{Cond: bpf.JumpEqual, Val: *m.icmpCode, SkipTrue: skipToEnd() - 1, SkipFalse: skipToEnd()})
				}
			}
		}
	}
