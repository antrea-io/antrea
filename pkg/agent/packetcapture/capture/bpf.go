...
func compileIPFilters(srcIP, dstIP net.IP, size, curLen uint8, transport *transportFilters, needsOtherTrafficDirectionCheck bool) []bpf.Instruction {
	...
	if dstIP != nil {
		...
		// When capturing in 'Both' directions and only a dstIP is specified (srcIP == nil),
		// this block handles the check for the initial traffic flow. If the packet's destination IP
		// doesn't match, we must not jump to the final 'drop' instruction. Instead, we use
		// calculateSkipFalse to compute the exact offset required to skip the remaining forward-traffic
		// checks and proceed to check for the return traffic.
		if srcIP == nil {
...
func compilePacketFilter(packetSpec *crdv1alpha1.Packet, srcIP, dstIP net.IP, direction crdv1alpha1.CaptureDirection) []bpf.Instruction {
	...
	case crdv1alpha1.CaptureDirectionSourceToDestination:
		inst = append(inst, compileIPFilters(srcIP, dstIP, size, uint8(len(inst)), &transport, false)...)
	case crdv1alpha1.CaptureDirectionDestinationToSource:
		...
		inst = append(inst, compileIPFilters(dstIP, srcIP, size, uint8(len(inst)), &transport, false)...)
	default:
		inst = append(inst, compileIPFilters(srcIP, dstIP, size, uint8(len(inst)), &transport, true)...)
