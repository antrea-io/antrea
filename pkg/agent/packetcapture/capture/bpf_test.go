...
func TestCalculateIPv4InstructionsSize(t *testing.T) {
	...
	for _, item := range tt {
		t.Run(item.name, func(t *testing.T) {
			assert.Equal(t, item.count, calculateIPv4InstructionsSize(item.packet, item.srcIP, item.dstIP, item.direction))
		})
	}
}
...
func TestCalculateIPv6InstructionsSize(t *testing.T) {
	...
	for _, item := range tt {
		t.Run(item.name, func(t *testing.T) {
			assert.Equal(t, item.count, calculateIPv6InstructionsSize(item.packet, item.srcIP, item.dstIP, item.direction))
		})
	}
}
