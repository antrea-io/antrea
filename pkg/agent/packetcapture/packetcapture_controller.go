		...
		if protocol != nil {
			if protocol.Type == intstr.String {
				if _, ok := capture.ProtocolMap[protocol.StrVal]; !ok {
					return fmt.Errorf("invalid protocol string, supported values are: %v (case sensitive)", slices.Collect(maps.Keys(capture.ProtocolMap)))
				}
			}
		}
...
func (c *Controller) parseIPs(ctx context.Context, pc *crdv1alpha1.PacketCapture) (srcIP, dstIP net.IP, err error) {
	if pc.Spec.Source.Pod != nil {
		srcIP, err = c.getPodIP(ctx, pc.Spec.Source.Pod, pc.Spec.Packet.IPFamily)
		if err != nil {
			return
		}
	...
	if pc.Spec.Destination.Pod != nil {
		dstIP, err = c.getPodIP(ctx, pc.Spec.Destination.Pod, pc.Spec.Packet.IPFamily)
		if err != nil {
			return
		}
