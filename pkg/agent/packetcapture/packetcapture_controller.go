...
func (c *Controller) getTargetCaptureDevices(pc *crdv1alpha1.PacketCapture) map[crdv1alpha1.CaptureLocation]string {
	...
	// Handle the special case where both Pods are on the same Node.
	// If so, we perform a single capture.
	if pc.Spec.CaptureLocation == crdv1alpha1.CaptureLocationBoth && sourceDevice != "" && destinationDevice != "" {
		devices[crdv1alpha1.CaptureLocationSource] = sourceDevice
		return devices
	}

	// If not the same-node case, proceed with the original distributed logic.
...
func (c *Controller) updateStatus(ctx context.Context, pc *crdv1alpha1.PacketCapture, state packetCaptureState) error {
	if retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		case packetCapturePhaseStarted:
			...
			addLocalConditions := func(instance *captureInstance, location crdv1alpha1.CaptureLocation) {
				...
				if !instance.complete {
					...
					conditions = append(conditions, crdv1alpha1.PacketCaptureCondition{Type: compCondType, Status: metav1.ConditionTrue, Reason: compReason, Message: compMsg, LastTransitionTime: t})

					// Add upload completion status if applicable.
					if pc.Spec.FileServer != nil && state.capturedPacketsNum > 0 {
						...
						conditions = append(conditions, crdv1alpha1.PacketCaptureCondition{Type: upCondType, Status: upStatus, Reason: upReason, Message: upMsg, LastTransitionTime: t})
					}
				}
			}

			if instance, exists := state.instances[crdv1alpha1.CaptureLocationSource]; exists {
				addLocalConditions(instance, crdv1alpha1.CaptureLocationSource)
			}
			if instance, exists := state.instances[crdv1alpha1.CaptureLocationDestination]; exists {
				addLocalConditions(instance, crdv1alpha1.CaptureLocationDestination)
			}
			...
			case crdv1alpha1.CaptureLocationBoth:
				if isSrcFinished && isDstFinished {
					// 1. The normal, distributed case where both agents have reported in.
					overallComplete = true
				} else if isSrcFinished && !dstExists {
					// 2. The special same-node case, where only a source capture was started
					//    and therefore a destination condition will never exist.
					overallComplete = true
				}
			}

			if !overallComplete {
				// If not yet ready to finalize, ensure the overall Complete status is Progressing.
				...
				if pc.Spec.FileServer != nil {
					...
					case crdv1alpha1.CaptureLocationBoth:
						if srcUploadExists && dstUploadExists {
							// 1. The normal, distributed case where both agents have reported in.
							overallUpload = true
						} else if srcUploadExists && !dstUploadExists {
							// 2. The special same-node case, where only a source capture was started
							//    and therefore a destination condition will never exist.
							overallUpload = true
						}
