...
func (f *fqdnController) onDNSResponseMsg(dnsMsg *dns.Msg, lookupTime time.Time, waitCh chan error) {
	...
	klog.Infof("=========== parse dns:  fqdn: %s, msgType: %s, responseIPs: %s", fqdn, msgType, responseIPs)
	if err != nil {
...
func (f *fqdnController) lookupIP(ctx context.Context, fqdn string, scope dnsQueryScope) error {
	...
	if (scope == dnsScopeIPv4 || scope == dnsScopeDualStack) && f.ipv4Enabled {
		lookupTime := time.Now()
	...
	if (scope == dnsScopeIPv6 || scope == dnsScopeDualStack) && f.ipv6Enabled {
		lookupTime := time.Now()
...
func (f *fqdnController) handlePacketIn(pktIn *ofctrl.PacketIn) error {
	klog.InfoS("Received a packetIn for DNS response")
