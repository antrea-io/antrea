...
func TestSchedule(t *testing.T) {
	egresses := []runtime.Object{
		&crdv1b2.Egress{
			...
			Spec:       crdv1b2.EgressSpec{EgressIPs: []string{"1.1.1.1"}, ExternalIPPool: "pool1"},
		},
		&crdv1b2.Egress{
			ObjectMeta: metav1.ObjectMeta{Name: "egressB", UID: "uidB", CreationTimestamp: metav1.NewTime(time.Unix(2, 0))},
			Spec:       crdv1b2.EgressSpec{EgressIPs: []string{"1.1.1.11"}, ExternalIPPool: "pool1"},
		},
		&crdv1b2.Egress{
			ObjectMeta: metav1.ObjectMeta{Name: "egressC", UID: "uidC", CreationTimestamp: metav1.NewTime(time.Unix(3, 0))},
			Spec:       crdv1b2.EgressSpec{EgressIPs: []string{"1.1.1.21"}, ExternalIPPool: "pool1"},
		},
	}
	tests := []struct {
		name                string
...
func BenchmarkSchedule(b *testing.B) {
	...
	for i := 0; i < 1000; i++ {
		egresses = append(egresses, &crdv1b2.Egress{
			...
			Spec:       crdv1b2.EgressSpec{EgressIPs: []string{fmt.Sprintf("1.1.%d.%d", rand.Intn(256), rand.Intn(256))}, ExternalIPPool: "pool1"},
		})
	}
	var nodes []string
	for i := 0; i < 1000; i++ {
...
func TestRun(t *testing.T) {
	...
	patch := map[string]interface{}{
		"spec": map[string]interface{}{
			"egressIPs": []string{"1.1.1.5"},
		},
	}
	patchBytes, _ := json.Marshal(patch)
	crdClient.CrdV1beta2().Egresses().Patch(context.TODO(), "egressA", types.MergePatchType, patchBytes, metav1.PatchOptions{})
	...
	crdClient.CrdV1beta2().Egresses().Delete(ctx, "egressA", metav1.DeleteOptions{})
	...
	crdClient.CrdV1beta2().Egresses().Create(ctx, &crdv1b2.Egress{
