...
func (s *egressIPScheduler) scheduleDualStackEgress(egress *crdv1b1.Egress, nodeToIPs map[string]sets.Set[string]) *scheduleResult {
	...
	// Currently only the first IPv4/IPv6 pair is realized on the datapath.
	// The scheduler does not over-count Node usage when users configure more than one pair in spec.egressIPs.
	egressIPs := firstDualStackPair(egress.Spec.EgressIPs)
	maxEgressIPsFilter := func(node string) bool {
		ipsOnNode := nodeToIPs[node]
		numIPs := ipsOnNode.Len()
