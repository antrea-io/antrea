...
type egressState struct {
	...
	egressIP string
	// The actual egress dual-stack IPs for the Egress.
	egressIPs []string
	// The actual datapath mark of this Egress. Used to check if the mark changes since last process.
	// For dual-stack Egress, both address families share the same mark.
	mark uint32
...
func (e *egressState) getEffectiveEgressIPs() []string {
	if len(e.egressIPs) > 0 {
		return firstDualStackPair(e.egressIPs)
	}
	if e.egressIP != "" {
	...
	for _, egress := range egresses {
		...
		if isDualStackEgressSchedulable(egress) && egress.Status.EgressNode == c.nodeName && len(egress.Status.EgressIPs) >= effectiveDualStackCount {
			...
			for i, ip := range effectiveStatusIPs {
				desiredLocalEgressIPs[ip] = subnetInfos[i]
			}
...
//     different SubnetInfos, so they map to different egressRouteTables with independent mark sets.
//   - The shared mark is released exactly once when transitioning to non-local.
func (c *EgressController) realizeDualStackEgressIPs(egressName string, egressIPs []string, subnetInfos []*crdv1b1.SubnetInfo) (uint32, error) {
	...
	if isLocalIP {
		...
		if sharedMark != 0 {
			if err := c.markAllocator.release(sharedMark); err != nil {
				return 0, fmt.Errorf("error releasing shared mark %d for dual-stack IPs: %v", sharedMark, err)
			}
			for _, s := range ipStates {
				s.mark = 0
			}
...
func (c *EgressController) unrealizeEgressIP(egressName, egressIP string) error {
	...
	if ipState.mark != 0 {
		...
		if err := c.markAllocator.release(ipState.mark); err != nil {
			return fmt.Errorf("error releasing mark for IP %s: %v", egressIP, err)
		}
	}
...
// unrealizeDualStackEgressIPs unrealizes dual-stack Egress IPs, reverts what
// realizeDualStackEgressIPs does.
func (c *EgressController) unrealizeDualStackEgressIPs(egressName string, egressIPs []string) error {
	lenIPs := len(egressIPs)
	if lenIPs != effectiveDualStackCount {
		return fmt.Errorf("expected exactly %d dual-stack IPs for unrealize, got %d", effectiveDualStackCount, lenIPs)
	}
	c.egressIPStatesMutex.Lock()
	defer c.egressIPStatesMutex.Unlock()
	...
	if sharedMark != 0 {
		...
		if err := c.markAllocator.release(sharedMark); err != nil {
			return fmt.Errorf("error releasing shared mark %d for dual-stack IPs: %v", sharedMark, err)
		}
	}
...
func (c *EgressController) reconcileEgressMark(egressName string, eState *egressState, mark uint32) error {
	...
	// If the mark changes, uninstall all of the Egress's Pod flows first, then install them with the new mark.
...
func (c *EgressController) syncPodSNATFlows(egressName string, eState *egressState, egressIPs []string, mark uint32) error {
	...
	for pod := range pods {
		...
		for _, egressIP := range egressIPs {
			if err := c.ofClient.InstallPodSNATFlows(uint32(ofPort), net.ParseIP(egressIP), mark); err != nil {
				return err
			}
		}
	...
	return c.uninstallPodFlows(egressName, eState, staleOFPorts, stalePods)
}

func (c *EgressController) updateEgressStatusWithRetry(egress *crdv1b1.Egress, desiredStatus *crdv1b1.EgressStatus, compareStatus func(current, desired *crdv1b1.EgressStatus) bool, logUpdate func(status *crdv1b1.EgressStatus), logUpdatedMsg string) error {
	toUpdate := egress.DeepCopy()
	var updateErr, getErr error
	if err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		if compareStatus(&toUpdate.Status, desiredStatus) {
			return nil
		}
	...
	klog.V(2).InfoS(logUpdatedMsg, "Egress", egress.Name)
...
func (c *EgressController) updateEgressStatus(egress *crdv1b1.Egress, egressIP string, scheduleErr error) error {
	...
	if isLocal {
		...
		// Keep status.egressIPs in sync with status.egressIP for single-stack Egresses. This preserves the
		// legacy status.egressIP field while giving new consumers one list-shaped status field to read.
	...
	return c.updateEgressStatusWithRetry(
		egress,
		desiredStatus,
		compareEgressStatus,
		func(status *crdv1b1.EgressStatus) {
			klog.V(2).InfoS("Updating Egress status", "Egress", egress.Name, "oldNode", egress.Status.EgressNode, "newNode", status.EgressNode)
		},
		"Updated Egress status",
	)
}
...
func (c *EgressController) getEgressIPInfo(egressName string, egress *crdv1b1.Egress) egressIPInfo {
	...
	if ipInfo.dualStack {
		...
		if isDualStackEgressSchedulable(egress) {
			...
			if scheduled {
				...
				ipInfo.scheduleErr = err
			}
		} else {
			egressIPs = egress.Spec.EgressIPs
		}
		if len(egressIPs) >= effectiveDualStackCount {
			// Only the first IPv4/IPv6 pair takes effect in the current implementation.
			ipInfo.egressIPs = firstDualStackPair(egressIPs)
		}
		return ipInfo
	}
	...
	if isEgressSchedulable(egress) {
		...
		ipInfo.egressIPs = []string{egress.Spec.EgressIP}
	}
	return ipInfo
}

func egressStateMatchesIPInfo(eState *egressState, ipInfo egressIPInfo) bool {
	if ipInfo.dualStack {
		return len(ipInfo.egressIPs) > 0 && slices.Equal(eState.egressIPs, ipInfo.egressIPs)
	}
	if len(ipInfo.egressIPs) == 0 {
		return eState.egressIP == ""
	}
	return eState.egressIP == ipInfo.egressIPs[0]
}

func (c *EgressController) resolveEgressSubnetInfos(egress *crdv1b1.Egress, ipInfo egressIPInfo) ([]*crdv1b1.SubnetInfo, error) {
	subnetInfos := make([]*crdv1b1.SubnetInfo, len(ipInfo.egressIPs))
	if ipInfo.egressNode != c.nodeName || !c.supportSeparateSubnet {
	...
	if !ipInfo.dualStack {
		...
		subnetInfos[0] = pool.Spec.SubnetInfo
		return subnetInfos, nil
	}

	// Resolve the effective pair before assigning either IP, so dual-stack assignment stays atomic.
	for i := range ipInfo.egressIPs {
		if i >= len(egress.Spec.ExternalIPPools) {
			continue
		}
...
func (c *EgressController) realizeEgressIPs(egressName string, ipInfo egressIPInfo, subnetInfos []*crdv1b1.SubnetInfo) (uint32, error) {
	if ipInfo.dualStack {
		return c.realizeDualStackEgressIPs(egressName, ipInfo.egressIPs, subnetInfos)
	}
	return c.realizeEgressIP(egressName, ipInfo.egressIPs[0], subnetInfos[0])
}

func (c *EgressController) updateEgressIPsStatus(egress *crdv1b1.Egress, ipInfo egressIPInfo, scheduleErr error) error {
	if ipInfo.dualStack {
		return c.updateDualStackEgressStatus(egress, ipInfo.egressIPs, scheduleErr)
	}
	egressIP := ""
	if len(ipInfo.egressIPs) > 0 {
		egressIP = ipInfo.egressIPs[0]
	}
	return c.updateEgressStatus(egress, egressIP, scheduleErr)
}
...
func (c *EgressController) syncEgress(egressName string) error {
	...
	// Do not proceed if no effective Egress IP is available.
	if len(ipInfo.egressIPs) == 0 {
		if err := c.updateEgressIPsStatus(egress, ipInfo, ipInfo.scheduleErr); err != nil {
			return fmt.Errorf("update Egress %s status error: %v", egressName, err)
		}
		return nil
	}
	if !exist {
		eState = c.newEgressState(egressName, ipInfo.egressIPs...)
	}

	subnetInfos, err := c.resolveEgressSubnetInfos(egress, ipInfo)
	if err != nil {
		return err
	}
	if err := c.reconcileEgressIPAssignments(egress, ipInfo, subnetInfos); err != nil {
		return err
	}

	// Realize the latest effective Egress IPs and get the desired mark.
	mark, err := c.realizeEgressIPs(egressName, ipInfo, subnetInfos)
	if err != nil {
		return err
	}
	...
	if err := c.reconcileEgressMark(egressName, eState, mark); err != nil {
		return err
	}

	if err := c.updateEgressIPsStatus(egress, ipInfo, nil); err != nil {
		return fmt.Errorf("update Egress %s status error: %v", egressName, err)
	}

	return c.syncPodSNATFlows(egressName, eState, ipInfo.egressIPs, mark)
}
...
func (c *EgressController) updateDualStackEgressStatus(egress *crdv1b1.Egress, egressIPs []string, scheduleErr error) error {
	...
	if isLocal {
		desiredStatus.EgressNode = c.nodeName
		// status.egressIP remains the compatibility field and points to the first effective IP (IPv4 for valid
		// dual-stack Egresses). status.egressIPs is the canonical list-shaped field for dual-stack consumers and is
		// intended to let future code move away from status.egressIP without changing the single-stack API behavior.
			...
			desiredStatus.Conditions = []crdv1b1.EgressCondition{
				{
					...
					LastTransitionTime: metav1.Now(),
					Reason:             "Assigned",
					Message:            "Dual-stack EgressIPs are successfully assigned to EgressNode",
				},
			}
		}
	} else if lenIPs == 0 {
		// Select one Node to update false status among all Nodes.
		// We don't care about the value of the key, just use it to reach a consensus among all agents about which one
		// should do the update. Keep this aligned with single-stack Egresses: pool-based Egresses may not have
		// spec.egressIPs populated yet while waiting for IP allocation, and that pending state should not fail sync.
		key := ""
		if len(egress.Spec.EgressIPs) > 0 {
			key = egress.Spec.EgressIPs[0]
		}
		nodeToUpdateStatus, err := c.cluster.SelectNodeForIP(key, "")
		if err != nil {
			return err
		}
		if nodeToUpdateStatus != c.nodeName {
			...
			desiredStatus.Conditions = []crdv1b1.EgressCondition{
				{
					...
					LastTransitionTime: metav1.Now(),
					Reason:             "AssignmentError",
					Message:            fmt.Sprintf("Failed to assign dual-stack EgressIPs to a Node: %v", scheduleErr),
				},
			}
		}
	} else {
	...
	return c.updateEgressStatusWithRetry(
		egress,
		desiredStatus,
		compareDualStackEgressStatus,
		func(_ *crdv1b1.EgressStatus) {
			klog.V(2).InfoS("Updating dual-stack Egress status", "Egress", egress.Name, "IPs", egressIPs)
		},
		"Updated dual-stack Egress status",
	)
}

func compareDualStackEgressStatus(current, desired *crdv1b1.EgressStatus) bool {
	return compareEgressStatusWithIPs(current, desired, func(status *crdv1b1.EgressStatus) []string {
		return status.EgressIPs
	})
}

func compareEgressStatusWithIPs(current, desired *crdv1b1.EgressStatus, getIPs func(status *crdv1b1.EgressStatus) []string) bool {
	...
	if current.EgressNode != desired.EgressNode {
		return false
	}
	if current.EgressIP != desired.EgressIP {
		return false
	}
	if !slices.Equal(getIPs(current), getIPs(desired)) {
		return false
	}
	currentIPAssignedCondition := crdv1b1.GetEgressCondition(current.Conditions, crdv1b1.IPAssigned)
	desiredIPAssignedCondition := crdv1b1.GetEgressCondition(desired.Conditions, crdv1b1.IPAssigned)
...
func (c *EgressController) uninstallEgress(egressName string, eState *egressState, egress *crdv1b1.Egress) error {
	...
	// For dual-stack Egress, use the dual-stack unrealize path to release the shared mark only once.
	egressIPs := eState.getEffectiveEgressIPs()
	if len(eState.egressIPs) > 0 {
		if err := c.unrealizeDualStackEgressIPs(egressName, egressIPs); err != nil {
			return err
		}
	} else {
		for _, egressIP := range egressIPs {
			if err := c.unrealizeEgressIP(egressName, egressIP); err != nil {
				return err
			}
		}
	}
...
// GetEgressIPByMark returns the Egress IP of the requested IP family associated with the snatMark.
func (c *EgressController) GetEgressIPByMark(mark uint32, isIPv6 bool) (string, error) {
	c.egressIPStatesMutex.Lock()
	defer c.egressIPStatesMutex.Unlock()
	for _, e := range c.egressIPStates {
		if e.mark == mark && (e.egressIP.To4() == nil) == isIPv6 {
			return e.egressIP.String(), nil
		}
	}
	return "", fmt.Errorf("no EgressIP associated with mark %v and isIPv6 %v", mark, isIPv6)
}
...
func (c *EgressController) GetEgress(ns, podName string) (types.EgressConfig, error) {
	...
	return types.EgressConfig{
		...
		EgressIPs:  egress.Status.EgressIPs,
		EgressNode: egress.Status.EgressNode,
	}, nil
}

func isDualStackEgress(egress *crdv1b1.Egress) bool {
	return len(egress.Spec.EgressIPs) >= effectiveDualStackCount || len(egress.Spec.ExternalIPPools) >= effectiveDualStackCount
}

// firstDualStackPair extracts the first IPv4/IPv6 pair from the list.
// Egress validation guarantees that, for valid dual-stack Egresses, the first two
// entries are an IPv4/IPv6 pair. Only the first pair takes effect currently.
func firstDualStackPair(ips []string) []string {
...
// A dual-stack Egress is schedulable if its Dual-Stack Egress IPs are allocated from ExternalIPPools.
func isDualStackEgressSchedulable(egress *crdv1b1.Egress) bool {
	return len(egress.Spec.EgressIPs) >= effectiveDualStackCount && len(egress.Spec.ExternalIPPools) >= effectiveDualStackCount
}
...
func compareEgressStatus(currentStatus, desiredStatus *crdv1b1.EgressStatus) bool {
	return compareEgressStatusWithIPs(currentStatus, desiredStatus, func(status *crdv1b1.EgressStatus) []string {
		return status.EgressIPs
	})
}
