...
func (c *EgressController) updateExternalIPPool(old, cur interface{}) {
	...
	// We only care about subnets here.
	if crdv1b2.CompareExternalIPPoolSubnets(oldPool.Spec.Subnets, curPool.Spec.Subnets, false) {
		return
	}
...
func subnetInfoForIP(subnets []crdv1b2.SubnetInfo, ip string) (*crdv1b2.SubnetInfo, error) {
	if len(subnets) == 0 {
		return nil, nil
	}
	...
	for _, subnet := range subnets {
		parsedGateway := net.ParseIP(subnet.Gateway)
...
func (c *EgressController) replaceEgressIPs() error {
	...
	for _, egress := range egresses {
		...
		if isEgressSchedulable(egress) && egress.Status.EgressNode == c.nodeName && egressIP != "" {
			...
			subnetInfo, err := subnetInfoForIP(pool.Spec.Subnets, egressIP)
			if err != nil {
	...
	if desiredNode == c.nodeName {
		if c.supportSeparateSubnet && egress.Spec.ExternalIPPool != "" {
			if pool, err := c.externalIPPoolLister.Get(egress.Spec.ExternalIPPool); err != nil {
				return err
			} else if subnetInfo, err = subnetInfoForIP(pool.Spec.Subnets, desiredEgressIP); err != nil {
				return err
			}
		}
