...
type egressState struct {
	...
	mark uint32
	// The actual datapath marks for dual-stack Egress.
	marks []uint32
	...
	// Rate-limit meters for dual-stack Egress.
	rateLimitMeters []*rateLimitMeter
}
...
func (e *egressState) getEffectiveRateLimitMeters() []*rateLimitMeter {
	if len(e.rateLimitMeters) > 0 {
		return e.rateLimitMeters
	}
...
// realizeEgressQoSDualStack installs or uninstalls OVS meters for each
// address-family mark, keeping per-mark state in eState.rateLimitMeters.
func (c *EgressController) realizeEgressQoSDualStack(egressName string, eState *egressState, marks []uint32, bandwidth *crdv1b1.Bandwidth) error {
	...
	// Build the desired meter list (one per mark).
	desired := make([]*rateLimitMeter, expectedDualStackCount)
	for i, m := range marks {
		if m != 0 {
			desired[i] = bandwidthToRateLimitMeter(bandwidth, m)
		}
	}

	if len(eState.rateLimitMeters) == 0 {
		eState.rateLimitMeters = make([]*rateLimitMeter, expectedDualStackCount)
	}

	// Reconcile each meter.
	for i, d := range desired {
		prev := eState.rateLimitMeters[i]
		if prev.Equals(d) {
			continue
		}
		if d != nil {
			if err := c.ofClient.InstallEgressQoS(marks[i], d.Rate, d.Burst); err != nil {
				return err
			}
		} else if prev != nil {
			if err := c.ofClient.UninstallEgressQoS(prev.MeterID); err != nil {
				return err
			}
		}
		eState.rateLimitMeters[i] = d
	}
	return nil
}
...
// For dual-stack egress, we must ensure all marks are consistent. Since the IP family order of the
// egress IPs is fixed, we can perform a simple match based on the same index.
func marksChanged(current, desired []uint32) bool {
...
func (c *EgressController) syncDualStackEgress(egressName string, egress *crdv1b1.Egress) error {
	...
	// Realize the Egress IPs and get the desired marks.
	var marks []uint32
	for i, ipStr := range desiredEgressIPs {
		var subnetInfo *crdv1b1.SubnetInfo
		if c.supportSeparateSubnet && i < len(egress.Spec.ExternalIPPools) {
		...
		mark, err := c.realizeEgressIP(egressName, ipStr, subnetInfo)
		if err != nil {
			return err
		}
		marks = append(marks, mark)
	}
	if err := c.realizeEgressQoSDualStack(egressName, eState, marks, egress.Spec.Bandwidth); err != nil {
		return err
	}

	if marksChanged(eState.marks, marks) {
		if err := c.uninstallPodFlows(egressName, eState, eState.ofPorts, eState.pods); err != nil {
			return err
		}
	}
	...
	for pod := range pods {
		...
		// Install flows for both IPv4 and IPv6.
		for i, ipStr := range desiredEgressIPs {
			egressIP := net.ParseIP(ipStr)
			if err := c.ofClient.InstallPodSNATFlows(uint32(ofPort), egressIP, marks[i]); err != nil {
				return err
			}
		}
		eState.ofPorts.Insert(ofPort)
