...
func (c *EgressController) watchEgressGroup() {
	...
	if err != nil {
		klog.InfoS("Failed to get antrea client", "err", err)
		return
	}
	...
	if err != nil {
		klog.InfoS("Failed to start watch for EgressGroup", "err", err)
		return
	}
	...
	if reflect.TypeOf(watcher) == reflect.TypeOf(emptyWatch) {
		klog.Info("Failed to start watch for EgressGroup, please ensure antrea service is reachable for the agent")
		return
	}
...
loop:
	for {
		...
		if !ok {
			klog.Info("Result channel for EgressGroup was closed")
			return
		}
		switch event.Type {
		case watch.Added:
