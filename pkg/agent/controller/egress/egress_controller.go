...
func NewEgressController(
	...
	eventBroadcaster := events.NewBroadcaster(&events.EventSinkImpl{Interface: k8sClient.EventsV1()})
...
func (c *EgressController) syncEgress(egressName string) error {
	...
	if desiredNode == c.nodeName {
		...
		if assigned {
			c.record.Eventf(egress, nil, corev1.EventTypeNormal, "IPAssigned", "Assigned", "Assigned Egress %s with IP %s on Node %s", egress.Name, desiredEgressIP, desiredNode)
		}
	} else {
		...
		if unassigned {
			c.record.Eventf(egress, nil, corev1.EventTypeNormal, "IPUnassigned", "Unassigned", "Unassigned Egress %s with IP %s from Node %s", egress.Name, desiredEgressIP, c.nodeName)
		}
	}
...
func (c *EgressController) uninstallEgress(egressName string, eState *egressState, egress *crdv1b1.Egress) error {
	...
	if unassigned && egress != nil {
		c.record.Eventf(egress, nil, corev1.EventTypeNormal, "IPUnassigned", "Unassigned", "Unassigned Egress %s with IP %s from Node %s", egressName, eState.egressIP, c.nodeName)
	}
