...
// enqueueEgressesByExternalIPPool enqueues the Egress objects with specific ExternalIPPool,
...
func (c *EgressController) enqueueEgressesByExternalIPPool(eipName string) {
	...
	count := 0
	for _, egress := range egresses {
		if egress.Spec.ExternalIPPool == eipName {
			c.queue.Add(egress.Name)
			count++
		}
	}
	klog.InfoS("Detected ExternalIPPool event", "ExternalIPPool", eipName, "enqueueEgressNum", count)
}
...
func (c *EgressController) updateEgressStatus(egress *crdv1a2.Egress, nodeName string) error {
	...
	klog.V(2).InfoS("Updating Egress status", "Egress", egress.Name, "oldNode", egress.Status.NodeName, "newNode", nodeName)
