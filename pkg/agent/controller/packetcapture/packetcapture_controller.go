...
func (c *Controller) checkPacketCaptureTimeout() {
	result, err := c.packetCaptureLister.List(labels.Everything())
	if err != nil {
		klog.ErrorS(err, "Failed to list PacketCaptures")
	}
...
func (c *Controller) cleanupPacketCapture(pcName string) {
	if err := defaultFS.Remove(nameToPath(pcName)); err == nil {
		klog.InfoS("Deleted pcap file", "name", pcName, "path", nameToPath(pcName))
	}
}
...
func (c *Controller) startPacketCapture(pc *crdv1alpha1.PacketCapture) error {
	...
	klog.V(2).InfoS("PacketCapture match packet", "packet", *matchPacket)
	...
	klog.InfoS("Perform capture on device", "name", pc.Name, "device", device, "filter", exp)
...
func (c *Controller) getTargetPodIP(podRef *crdv1alpha1.PodReference, isIPv6 bool) (net.IP, error) {
	srcPodInterfaces := c.interfaceStore.GetContainerInterfacesByPod(podRef.Name, podRef.Namespace)
	var result net.IP
	if len(srcPodInterfaces) > 0 {
		if isIPv6 {
			result = srcPodInterfaces[0].GetIPv6Addr()
		} else {
			result = srcPodInterfaces[0].GetIPv4Addr()
		}
	} else {
	...
	if result == nil {
		...
		return nil, errors.New(fmt.Sprintf("cannot find IP with %s AddressFamily for pod %s/%s", family, podRef.Namespace, podRef.Name))
	}
	return result, nil
}
...
func (c *Controller) createMatchPacket(pc *crdv1alpha1.PacketCapture) (*binding.Packet, error) {
	...
	if pc.Spec.Source.Pod != nil {
		ip, err := c.getTargetPodIP(pc.Spec.Source.Pod, packet.IsIPv6)
		if err != nil {
			return nil, err
		} else {
			packet.SourceIP = ip
	...
	if pc.Spec.Destination.Pod != nil {
		ip, err := c.getTargetPodIP(pc.Spec.Destination.Pod, packet.IsIPv6)
		if err != nil {
			return nil, err
		} else {
