...
func (r BGPPolicyResponse) GetTableRow(_ int) []string {
	...
	if len(r.MemberASNs) != 0 {
		for _, memberASN := range r.MemberASNs {
			memberASNs = append(memberASNs, strconv.Itoa(int(memberASN)))
		}
	}
	return []string{r.BGPPolicyName, r.RouterID, strconv.Itoa(int(r.LocalASN)), strconv.Itoa(int(r.ListenPort)), confederationIdentifierStr, strings.Join(memberASNs, ",")}
}
