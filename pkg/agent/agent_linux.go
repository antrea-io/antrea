...
func (i *Initializer) prepareOVSBridge() error {
	...
	klog.Infof("Preparing OVS bridge for AntreaIPAM")
	...
	if _, err = i.ovsBridgeClient.GetOFPort(brName); err == nil {
		...
		if uplinkNetConfig.Name == brName {
			...
			if err != nil {
				klog.Errorf("Cannot list OVS ports.")
				return err
			}
			klog.Infof("Found ports from OVS bridge: %+v", ports)
			...
			if err2 != nil {
				klog.Errorf("Cannot find uplink port %s", uplinkPort.Name)
				return err2
			}
	...
	wait.PollImmediate(100*time.Millisecond, 10000*time.Millisecond, func() (bool, error) {
		...
		if err != nil {
			klog.Infof("OVS bridge local port %s is not ready, err: %+v", brName, err)
			return false, nil
		}
		klog.Infof("OVS bridge local port is ready, type: %s, attrs: %+v", link.Type(), link.Attrs())
		return true, nil
	})
...
func (i *Initializer) saveHostRoutes() error {
	...
	for _, route := range routes {
		if route.LinkIndex != i.nodeConfig.UplinkNetConfig.Index {
			klog.Infof("Skipped host route not on uplink: %+v", route)
			continue
		}
		// Skip IPv6 routes before we support IPv6 stack.
		// TODO(gran): support IPv6
		if route.Gw.To4() == nil {
			klog.Infof("Skipped IPv6 host route: %+v", route)
			continue
		}
...
func (i *Initializer) RestoreOVSBridge() {
	...
	if uplink != "" {
		if err := util.DeleteOVSPort(brName, uplink); err != nil {
			klog.Errorf("Remove uplink port %s from %s failed, err: %+v", uplink, brName, err)
		}
		if err := util.ConfigureLinkAddresses(uplinkNetConfig.Index, []*net.IPNet{uplinkNetConfig.IP}); err != nil {
			klog.Errorf("Configure IP to %s failed, err: %+v", uplink, err)
		}
	}
	if brLink != nil {
		if err := util.ConfigureLinkAddresses(brLink.Attrs().Index, nil); err != nil {
			klog.Errorf("Remove IP from %s failed, err: %+v", brName, err)
		}
		if err := netlink.LinkSetDown(brLink); err != nil {
			klog.Errorf("Disable interface %s failed, err: %+v", brName, err)
		}
	}
	if uplink != "" {
		if err := i.restoreHostRoutesToInterface(uplink); err != nil {
			klog.Errorf("Configure route to %s failed, err: %+v", uplink, err)
		}
	}
