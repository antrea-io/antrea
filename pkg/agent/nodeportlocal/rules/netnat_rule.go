...
func (nn *netnatRules) initRules() error {
	nn.DeleteAllRules()
	if err := nn.netUtil.NewNetNat(antreaNatNPL, route.PodCIDRIPv4); err != nil {
		return err
	}
...
func (nn *netnatRules) AddRule(nodePort int, podIP string, podPort int, protocol string) error {
	...
	if err := nn.netUtil.ReplaceNetNatStaticMapping(netNatStaticMapping); err != nil {
		return err
	}
...
func (nn *netnatRules) DeleteRule(nodePort int, podIP string, podPort int, protocol string) error {
	...
	if err := nn.netUtil.RemoveNetNatStaticMapping(netNatStaticMapping); err != nil {
		return err
	}
...
func (nn *netnatRules) DeleteAllRules() error {
	if err := nn.netUtil.RemoveNetNatStaticMappings(antreaNatNPL); err != nil {
		return err
	}
