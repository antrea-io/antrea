...
func TestNetworkPolicyMetrics(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			gomock.InOrder(
				mockOVSClient.EXPECT().DumpTableFlowsWithFilters(EgressMetricTable.ofTable.GetID(), flowFilter).Return(tt.egressFlows, nil),
				mockOVSClient.EXPECT().DumpTableFlowsWithFilters(IngressMetricTable.ofTable.GetID(), flowFilter).Return(tt.ingressFlows, nil),
			)
