...
func serviceInitFlows(proxyEnabled, isIPv4, proxyAllEnabled bool) []string {
	if !proxyEnabled {
		return []string{
	...
	if isIPv4 {
		flows = []string{
			...
			"cookie=0x1030000000000, table=EndpointDNAT, priority=200,reg0=0x4000/0x4000 actions=controller(id=32776,reason=no_match,userdata=04,max_len=128)",
...
func Test_featureService_initFlows(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectedFlows: serviceInitFlows(true, true, false),
		},
		{
			name:          "IPv6,Proxy",
			enableIPv6:    true,
			clientOptions: []clientOptionsFn{enableProxy},
			expectedFlows: serviceInitFlows(true, false, false),
		},
		{
			name:          "IPv4,ProxyAll",
			enableIPv4:    true,
			clientOptions: []clientOptionsFn{enableProxyAll},
			expectedFlows: serviceInitFlows(true, true, true),
		},
		{
			name:          "IPv6,ProxyAll",
			enableIPv6:    true,
			clientOptions: []clientOptionsFn{enableProxyAll},
			expectedFlows: serviceInitFlows(true, false, true),
		},
		{
			name:          "No Proxy",
			enableIPv4:    true,
			...
			expectedFlows: serviceInitFlows(false, true, false),
		},
	}
