...
const (
	...
	marksRegServiceNeedLB uint32 = 0xfff0
	// marksRegServiceHitted indicates a packet has done service selection.
	marksRegServiceHitted uint32 = 0xfff1
	// marksRegServiceHitted indicates a packet has done service selection and
	// the selection result needs to be cached.
	marksRegServiceNeedLearn uint32 = 0xfff2
...
var (
	...
	// serviceIPRegRange takes a 32-bit range of register serviceIPReg to store
	// the selected Service Endpoint IP.
	serviceIPRegRange = binding.Range{0, 31}
	// servicePortRegRange takes a 16-bit range of register servicePortReg to store
	// the selected Service Endpoint port.
	servicePortRegRange = binding.Range{0, 15}
	...
	// decision.
	serviceLearnRegRange = binding.Range{16, 31}
...
// 5) Let other traffic goes to the sessionAffinityTable first and then the serviceLBTable.
//    The sessionAffinityTable is a side-effect table which means traffic will
...
func (c *client) hairpinDNATFlow(category cookie.Category) binding.Flow {
	connectionTrackStateTable := c.pipeline[conntrackStateTable]
	return connectionTrackStateTable.BuildFlow(priorityNormal).MatchProtocol(binding.ProtocolIP).
...
func (c *client) l3FlowsToPod(localGatewayMAC net.HardwareAddr, podInterfaceIP net.IP, podInterfaceMAC net.HardwareAddr, category cookie.Category) binding.Flow {
	l3FwdTable := c.pipeline[l3ForwardingTable]
	var nextTable binding.TableIDType
	if c.enableProxy {
...
// serviceHairpinResponseDNATFlow generate the flow which transforms destination
// IP of the hairpin packet to the source IP.
func (c *client) serviceHairpinResponseDNATFlow(category cookie.Category) binding.Flow {
...
// sessionAffinityReselectFlow generates the flow resubmit the service accessing
// packet back to serviceLBTable if there is no endpointDNAT flow matched. This
// case will be occur if an Endpoint is removed and is the learned Endpoint
// selection of the Service.
func (c *client) sessionAffinityReselectFlow(category cookie.Category) binding.Flow {
...
func (c *client) serviceNeedLBFlow(category cookie.Category) binding.Flow {
...
func (c *client) l3ToExternalFlows(nodeIP net.IP, localSubnet net.IPNet, outputPort int, category cookie.Category) []binding.Flow {
	l3ForwardingNext := l2ForwardingCalcTable
	if c.enableProxy {
		l3ForwardingNext = c.pipeline[l3ForwardingTable].GetNext()
	}
	flows := []binding.Flow{
		...
		c.pipeline[l3ForwardingTable].BuildFlow(priorityLow).
			...
			MatchRegRange(int(marksReg), markTrafficFromLocal, binding.Range{0, 15}).
			MatchDstIPNet(localSubnet).
			Action().GotoTable(l3ForwardingNext).
			Cookie(c.cookieAllocator.Request(category).Raw()).
			Done(),
...
func (c *client) serviceLearnFlow(groupID binding.GroupIDType, svcIP net.IP, svcPort uint16, protocol binding.Protocol, affinityTimeout uint16) binding.Flow {
	...
	cookieID := c.cookieAllocator.Request(cookie.Service).Raw() | uint64(groupID)
	...
	return learnFlowBuilderLearnAction.
		MatchLearnedDstIP().
		MatchLearnedSrcIP().
		LoadRegToReg(int(serviceIPReg), int(serviceIPReg), serviceIPRegRange, serviceIPRegRange).
		LoadRegToReg(int(servicePortReg), int(servicePortReg), servicePortRegRange, servicePortRegRange).
...
func (c *client) endpointDNATFlow(endpointIP net.IP, endpointPort uint16, protocol binding.Protocol) binding.Flow {
	...
	unionVal := (marksRegServiceHitted << serviceLearnRegRange.Length()) + uint32(endpointPort)
	return c.pipeline[endpointDNATTable].BuildFlow(priorityNormal).
		Cookie(c.cookieAllocator.Request(cookie.Service).Raw()).
		MatchProtocol(protocol).
		MatchReg(int(serviceIPReg), ipVal).
		MatchRegRange(int(servicePortReg), unionVal, binding.Range{0, 31}).
...
// endpointHairpinFlow generates the flow which does DNAT for Service
// hairpin packets and loads the hairpin mark to markReg.
func (c *client) endpointHairpinFlow(endpointIP net.IP) binding.Flow {
	return c.pipeline[conntrackCommitTable].BuildFlow(priorityNormal).
		Cookie(c.cookieAllocator.Request(cookie.Service).Raw()).
		MatchProtocol(binding.ProtocolIP).
...
// serviceEndpointGroup creates/modify the group/buckets of Endpoints. If the
...
func (c *client) serviceEndpointGroup(groupID binding.GroupIDType, withSessionAffinity bool, endpoints ...upstream.Endpoint) binding.Group {
	...
	if withSessionAffinity {
		...
		lbResultMark = marksRegServiceHitted
	}
	...
	for _, endpoint := range endpoints {
		...
		group = group.Bucket().Weight(100).
			...
			LoadRegRange(int(servicePortReg), uint32(portVal), binding.Range{0, 15}).
			LoadRegRange(int(serviceLearnReg), lbResultMark, binding.Range{16, 31}).
