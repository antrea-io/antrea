...
func (f *featureService) serviceLearnFlow(groupID binding.GroupIDType,
	...
	// Loading the EpSelectedRegMark indicates that the Endpoint selection is complete. RewriteMACRegMark is required to
	// load for Service packets
	regMarksToLoad := []*binding.RegMark{EpSelectedRegMark, RewriteMACRegMark}
	// If the Service is accessible externally, the ExternalAccessibleRegMark should be loaded to determine whether SNAT
	// is required for the connection.
	if externallyAccessible {
		regMarksToLoad = append(regMarksToLoad, ExternalAccessibleRegMark)
	}
...
func (f *featureService) serviceLBFlow(groupID binding.GroupIDType,
	...
	// RewriteMACRegMark is required to load for Service packets.
...
func (f *featureService) gatewaySNATFlows() []binding.Flow {
	...
	for _, ipProtocol := range f.ipProtocols {
		...
		for _, pktDstRegMark := range pktDstRegMarks {
			// This generates the flow to match the first packet of Service which is accessible externally connection
			// initiated through the Antrea gateway, and the selected Endpoint is on a remote Node, then ConnSNATCTMark
			// will be loaded in DNAT CT zone, indicating that SNAT is required for the connection.
			flows = append(flows, SNATMarkTable.ofTable.BuildFlow(priorityNormal).
				Cookie(cookieID).
				MatchProtocol(ipProtocol).
				MatchCTStateNew(true).
				MatchCTStateTrk(true).
				MatchRegMark(FromGatewayRegMark, pktDstRegMark, ExternalAccessibleRegMark).
				Action().CT(true, SNATMarkTable.GetNext(), f.dnatCtZones[ipProtocol], f.ctZoneSrcField).
