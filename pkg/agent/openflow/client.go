...
func (c *client) InstallNodeFlows(hostname string, localGatewayMAC net.HardwareAddr, peerGatewayIP net.IP, peerPodCIDR net.IPNet, tunnelPeerAddr net.IP) error {
	id := c.cookieAllocator.Request(cookie.Node).Raw()
	flows := []binding.Flow{
		c.arpResponderFlow(peerGatewayIP).SetCookie(id),
		c.l3FwdFlowToRemote(localGatewayMAC, peerPodCIDR, tunnelPeerAddr).SetCookie(id),
	}
...
func (c *client) InstallPodFlows(containerID string, podInterfaceIP net.IP, podInterfaceMAC, gatewayMAC net.HardwareAddr, ofPort uint32) error {
	id := c.cookieAllocator.Request(cookie.Pod).Raw()
	flows := []binding.Flow{
		c.podClassifierFlow(ofPort).SetCookie(id),
		c.podIPSpoofGuardFlow(podInterfaceIP, podInterfaceMAC, ofPort).SetCookie(id),
		c.arpSpoofGuardFlow(podInterfaceIP, podInterfaceMAC, ofPort).SetCookie(id),
		c.l2ForwardCalcFlow(podInterfaceMAC, ofPort).SetCookie(id),
		c.l3FlowsToPod(gatewayMAC, podInterfaceIP, podInterfaceMAC).SetCookie(id),
	}
...
func (c *client) InstallClusterServiceCIDRFlows(serviceNet *net.IPNet, gatewayOFPort uint32) error {
	id := c.cookieAllocator.Request(cookie.Service).Raw()
	return c.flowOperations.Add(c.serviceCIDRDNATFlow(serviceNet, gatewayOFPort).SetCookie(id))
}

func (c *client) InstallGatewayFlows(gatewayAddr net.IP, gatewayMAC net.HardwareAddr, gatewayOFPort uint32) error {
	id := c.cookieAllocator.Request(cookie.Default).Raw()
	if err := c.flowOperations.Add(c.gatewayClassifierFlow(gatewayOFPort).SetCookie(id)); err != nil {
		return err
	} else if err := c.flowOperations.Add(c.gatewayIPSpoofGuardFlow(gatewayOFPort).SetCookie(id)); err != nil {
		return err
	} else if err := c.flowOperations.Add(c.gatewayARPSpoofGuardFlow(gatewayOFPort).SetCookie(id)); err != nil {
		return err
	} else if err := c.flowOperations.Add(c.l3ToGatewayFlow(gatewayAddr, gatewayMAC).SetCookie(id)); err != nil {
		return err
	} else if err := c.flowOperations.Add(c.l2ForwardCalcFlow(gatewayMAC, gatewayOFPort).SetCookie(id)); err != nil {
		return err
	}
	return nil
}
...
func (c *client) InstallTunnelFlows(tunnelOFPort uint32) error {
	id := c.cookieAllocator.Request(cookie.Default).Raw()
	if err := c.flowOperations.Add(c.tunnelClassifierFlow(tunnelOFPort).SetCookie(id)); err != nil {
		return err
	} else if err := c.flowOperations.Add(c.l2ForwardCalcFlow(globalVirtualMAC, tunnelOFPort).SetCookie(id)); err != nil {
		return err
	}
	return nil
}
...
func (c *client) Initialize() error {
	...
	id := c.cookieAllocator.Request(cookie.Default)
	for _, flow := range c.defaultFlows() {
		flow = flow.SetCookie(id.Raw())
		if err := c.flowOperations.Add(flow); err != nil {
			return fmt.Errorf("failed to install default flows: %v", err)
		}
	}
	if err := c.flowOperations.Add(c.arpNormalFlow().SetCookie(id.Raw())); err != nil {
		return fmt.Errorf("failed to install arp normal flow: %v", err)
	}
	if err := c.flowOperations.Add(c.l2ForwardOutputFlow().SetCookie(id.Raw())); err != nil {
