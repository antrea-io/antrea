...
type Client interface {
	...
	// otherwise ipsecTunOFPort must be 0. In dual-stack, IPv6 traffic must be encapsulated in the
	// primary (IPv4 when present) tunnel so it goes through the IPsec tunnel and gets encrypted.
...
func (c *client) InstallNodeFlows(hostname string,
	...
	// When IPsec is enabled, use primary (IPv4 when present) tunnel for all traffic so IPv6 is
	// encapsulated in IPv4 and goes through the IPsec tunnel.
	primaryTunnelPeer := tunnelPeerIPs.IPv4
	if primaryTunnelPeer == nil {
		primaryTunnelPeer = tunnelPeerIPs.IPv6
	}
	...
	for peerPodCIDR, peerGatewayIP := range peerConfigs {
		...
		if ipsecTunOFPort != 0 {
			tunnelPeerForFlow = primaryTunnelPeer
		}
