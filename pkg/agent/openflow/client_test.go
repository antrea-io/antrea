...
func prepareSetBasePacketOutBuilder(ctrl *gomock.Controller, success bool) *client {
	ofClient := NewClient(bridgeName, bridgeMgmtAddr, true, true, false, false, false, false, false, false, false, false, nil)
	c := ofClient.(*client)
	m := ovsoftest.NewMockBridge(ctrl)
	c.bridge = m
	bridge := binding.OFBridge{}
...
func Test_client_ReplayFlows(t *testing.T) {
	...
	if fc.ovsMetersAreSupported {
		...
		for _, meterCfg := range []struct {
			...
			bridge.EXPECT().CreateMeter(meterCfg.id, ofctrl.MeterBurst|ofctrl.MeterPktps).Return(meter).Times(1)
			meter.EXPECT().ResetMeterBands().Return(meter).Times(1)
