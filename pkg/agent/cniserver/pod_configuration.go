...
func (pc *podConfigurator) configureInterfacesCommon(
	...
	result *ipam.IPAMResult, containerAccess *containerAccessArbitrator, ifaceMAC net.HardwareAddr) error {
	err := pc.ifConfigurator.configureContainerLink(
		podName, podNamespace, containerID, containerNetNS,
		containerIFDev, mtu, sriovVFDeviceID, "", &result.Result, containerAccess, ifaceMAC)
	if err != nil {
		return err
	}
