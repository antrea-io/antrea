...
func (pc *podConfigurator) recordPodEvent(ifConfig *interfacestore.InterfaceConfig, installed bool) {
	...
	if installed {
		...
		pc.recorder.Eventf(pod, nil, corev1.EventTypeNormal, "NetworkReady", "Installed", "Installed Pod network forwarding rules")
		return
	}

	pc.recorder.Eventf(pod, nil, corev1.EventTypeWarning, "NetworkNotReady", "NotInstalled", "Pod network forwarding rules not installed")
}
