...
func (pc *podConfigurator) initPortStatusMonitor(podInformer cache.SharedIndexInformer) {
	...
	eventBroadcaster := events.NewBroadcaster(&events.EventSinkImpl{Interface: pc.kubeClient.EventsV1()})
	pc.eventBroadcaster = eventBroadcaster
	pc.recorder = eventBroadcaster.NewRecorder(scheme.Scheme, workerName)
