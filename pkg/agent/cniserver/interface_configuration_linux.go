...
func (ic *ifConfigurator) configureContainerLinkVeth(
	...
	if ifaceMAC == nil {
		ifaceMAC = util.GenerateRandomMAC()
	}

	hostIface := &current.Interface{Name: hostIfaceName}
	containerIface := &current.Interface{Name: containerIfaceName, Sandbox: containerNetNS, Mac: ifaceMAC.String()}
	result.Interfaces = []*current.Interface{hostIface, containerIface}

	if err := nsWithNetNSPath(containerNetNS, func(hostNS ns.NetNS) error {
		klog.V(2).Infof("Creating veth devices (%s, %s) for container %s", containerIfaceName, hostIfaceName, containerID)
		hostVeth, containerVeth, err := ipSetupVethWithName(containerIfaceName, hostIfaceName, mtu, ifaceMAC.String(), hostNS)
		if err != nil {
			return fmt.Errorf("failed to create veth devices for container %s: %v", containerID, err)
		}
		containerIface.Mac = ifaceMAC.String()
...
func (ic *ifConfigurator) configureContainerLink(
	...
	if brSriovVFDeviceID != "" {
		...
		return ic.configureContainerLinkVeth(podName, podNamespace, containerID, containerNetNS, containerIfaceName, mtu, result, ifaceMAC)
	}
}
