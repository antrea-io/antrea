...
var mockSecondaryNetworkStateChecker *typestest.MockSecondaryNetworkStateChecker
...
func newMockCNIServer(t *testing.T, controller *gomock.Controller, ipamDriver ipam.IPAMDriver, ipamType string, enableSecondaryNetworkIPAM, isChaining, enableSecondaryNetwork bool) *CNIServer {
	...
	if enableSecondaryNetwork {
		mockSecondaryNetworkStateChecker = typestest.NewMockSecondaryNetworkStateChecker(controller)
		cniServer.secondaryNetworkStateChecker = mockSecondaryNetworkStateChecker
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.enableSecondaryNetwork {
				mockSecondaryNetworkStateChecker.EXPECT().ReadyForCNIDel(gomock.Any(), gomock.Any()).Return(false).Times(1)
			}
