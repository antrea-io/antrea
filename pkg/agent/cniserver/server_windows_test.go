...
func mockCNIServer(t *testing.T, controller *gomock.Controller, podUpdateNotifier *channel.SubscribableChannel) *CNIServer {
	...
	cniServer.podConfigurator, _ = newPodConfigurator(mockOVSBridgeClient, mockOFClient, routeMock, ifaceStore, gwMAC, "system", false, podUpdateNotifier, nil, false)
	return cniServer
}
...
func TestCmdAdd(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			server := mockCNIServer(t, controller, waiter.notifier)
			...
			if tc.connectOVS {
				...
				routeMock.EXPECT().AddLocalAntreaFlexibleIPAMPodRule(gomock.Any()).Return(nil).Times(1)
			}
...
func TestCmdCheck(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			cniserver := mockCNIServer(t, controller, channel.NewSubscribableChannel("podUpdate", 100))
...
func TestReconcile(t *testing.T) {
	...
	cniServer.podConfigurator, _ = newPodConfigurator(mockOVSBridgeClient, mockOFClient, routeMock, ifaceStore, gwMAC, "system", false, waiter.notifier, nil, false)
