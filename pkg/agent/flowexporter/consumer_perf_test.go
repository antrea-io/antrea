...
func NewConsumerForTest(tb testing.TB, config *ConsumerConfig) *Consumer {
	...
	return &Consumer{
		...
		exp:                 exporter.NewIPFIXExporter(string(config.transportProtocol), nodeName, obsDomainID, v4Enabled, v6Enabled),
...
func setupConsumer(tb testing.TB, isConntrackConn bool, stopCh <-chan struct{}) (*Consumer, error) {
	...
	cfg := &ConsumerConfig{
		...
		v4Enabled:         !testWithIPv6,
		v6Enabled:         testWithIPv6,
		transportProtocol: v1alpha1.TransportProtocol(collectorAddr.Network()),
