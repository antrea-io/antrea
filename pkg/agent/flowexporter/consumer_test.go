...
func TestConsumer_Connect(t *testing.T) {
	...
	tests := []struct {
		...
		connected        bool
		commProtocol     v1alpha1.CommunicationProtocol
	...
	}{
		{
			...
			commProtocol: v1alpha1.ProtoGRPC,
			...
			commProtocol: v1alpha1.ProtoIPFix,
			addrOverride: "unknown/invalid:4739",
			wantErr:      true, // Error because the service doesn't exist
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cfg := ConsumerConfig{
				...
				v4Enabled:         true,
				v6Enabled:         true,
				commProtocol:      tt.commProtocol,
				transportProtocol: v1alpha1.ProtoUDP,
			}
			if tt.addrOverride != "" {
...
func TestConsumer_getExporterTLSConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := CreateConsumer(k8sClient, nil, ConsumerConfig{commProtocol: v1alpha1.ProtoGRPC})
