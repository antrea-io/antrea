...
func TestDenyConnectionStore_AddOrUpdateConn(t *testing.T) {
	...
	for _, c := range tc {
		t.Run(c.name, func(t *testing.T) {
			...
			if c.storeLimit == nil || *c.storeLimit > 0 {
			...
			if c.storeLimit != nil {
				options.DenyConnectionBufferLimit = *c.storeLimit
			}
			denyConnStore := NewDenyConnectionStore(mockPodStore, mockProxier, &options)
