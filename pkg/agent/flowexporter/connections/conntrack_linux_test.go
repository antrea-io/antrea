...
func TestConnTrackSystem_DumpFlows(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			"Filter for only SCTP",
			[]string{"sctp"},
			testFlowsMixedProtocols,
			1,
		},
		{
			"Filter for only TCP and SCTP",
			[]string{"tcp", "sctp"},
			testFlowsMixedProtocols,
			4,
		},
		{
			"Filter for UDP and SCTP",
			[]string{"udp", "sctp"},
			testFlowsMixedProtocols,
			3,
		},
...
func TestConnTrackSystem_GetMaxConnections(t *testing.T) {
	connDumperDPSystem := NewConnTrackSystem(&config.NodeConfig{}, netip.Prefix{}, netip.Prefix{}, false, filter.ProtocolFilter{})
