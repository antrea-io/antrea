...
func (ea *egressInfoAugmenter) Augment(conn *connection.Connection, opts ...AugmentOpt) {
	conn.FlowType = ea.findFlowType(conn)
	...
	egress, err := ea.egressQuerier.GetEgress(conn.SourcePodNamespace, conn.SourcePodName)
	if err != nil {
...
func (ea *egressInfoAugmenter) findFlowType(conn *connection.Connection) uint8 {
	// TODO: support Pod-To-External flows in network policy only mode.
	if ea.isNetworkPolicyOnly {
	...
	if ea.nodeRouteController == nil {
		klog.V(5).InfoS("Can't find flow type without nodeRouteController")
		return utils.FlowTypeUnspecified
	}
	srcIsPod, srcIsGw := ea.nodeRouteController.LookupIPInPodSubnets(conn.FlowKey.SourceAddress)
	dstIsPod, dstIsGw := ea.nodeRouteController.LookupIPInPodSubnets(conn.FlowKey.DestinationAddress)
	if srcIsGw || dstIsGw {
