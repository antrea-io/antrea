...
func NewFlowExporter(
	...
	ctConnUpdateChannel := channel.NewSubscribableChannel("Conntrack Connections", 100)
	denyConnUpdateChannel := channel.NewSubscribableChannel("Deny Connections", 100)
	connTrackDumper := connections.InitializeConnTrackDumper(nodeConfig, serviceCIDRNet, serviceCIDRNetv6, ovsDatapathType, proxyEnabled, filter.NewProtocolFilter(nil)) // Use nil filter because the filter will apply per destination
	poller := connections.NewPoller(connTrackDumper, ctConnUpdateChannel, o.PollInterval, v4Enabled, v6Enabled, o.ConnectUplinkToBridge)
	...
	fe := &FlowExporter{
		...
		poller:                poller,
		ctConnUpdateChannel:   ctConnUpdateChannel,
...
func (fe *FlowExporter) deleteDestination(obj any) {
	...
	if !ok {
		...
		if !ok {
			klog.Errorf("DeletedFinalStateUnknown does not contain FlowExporterDestination object: %v", deletedState.Obj)
			return
		}
	}
...
func (exp *FlowExporter) syncFlowExporterDestination(key string) error {
	...
	klog.V(3).InfoS("Adding destination", "name", res.Name)
