...
func TestFlowExporter_networkPolicyWait(t *testing.T) {
	...
	fe := &FlowExporter{
		...
		poller:                connections.NewPoller(mockConnDumper, testSubChannel, nil, nil, 100*time.Millisecond, true, false, false),
		ctConnUpdateChannel:   testSubChannel,
		denyConnUpdateChannel: testSubChannel,
...
func TestFlowExporter_pollerLifecycle(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		fe := &FlowExporter{
			...
			poller:                connections.NewPoller(mockConnDumper, testSubChannel, nil, nil, testPollInterval, true, false, false),
			ctConnUpdateChannel:   testSubChannel,
			denyConnUpdateChannel: testSubChannel,
			destinations:          make(map[string]destinationObj),
