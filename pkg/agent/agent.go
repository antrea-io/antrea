...
func (i *Initializer) FlowRestoreComplete() error {
	...
	err := wait.PollUntilContextTimeout(context.TODO(), 200*time.Millisecond, 2*time.Second, true,
		func(ctx context.Context) (done bool, err error) {
			...
			if !flowRestoreWait {
				...
				klog.Info("flow-restore-wait was not true before the delete call was made, will retry")
				return false, nil
			}
			return true, nil
		})
	...
	for retries := 0; retries < 3; retries++ {
		...
		if flowRestoreWait {
			...
			klog.Info("flow-restore-wait was still true even though the delete call was considered success")
			continue
		}
