...
-A ANTREA-OUTPUT -m comment --comment "Antrea: allow proxy health check output packets" -p tcp --sport 10256 -j ACCEPT
...
`, false, true)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var exists bool
			if tt.isIPv6 {
				gotRules, exists = c.iptablesCache.ipv6[iptablesTagNodeNetworkPolicy].Load(ingressChain)
			} else {
				gotRules, exists = c.iptablesCache.ipv4[iptablesTagNodeNetworkPolicy].Load(ingressChain)
			}
			assert.True(t, exists)
			...
			if tt.isIPv6 {
				gotRules, exists = c.iptablesCache.ipv6[iptablesTagNodeNetworkPolicy].Load(svcChain)
			} else {
				gotRules, exists = c.iptablesCache.ipv4[iptablesTagNodeNetworkPolicy].Load(svcChain)
			}
			assert.True(t, exists)
			...
			if tt.isIPv6 {
				_, exists = c.iptablesCache.ipv6[iptablesTagNodeNetworkPolicy].Load(svcChain)
			} else {
				_, exists = c.iptablesCache.ipv4[iptablesTagNodeNetworkPolicy].Load(svcChain)
			}
			assert.False(t, exists)
