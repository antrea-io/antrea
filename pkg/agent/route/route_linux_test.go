...
-A ANTREA-PREROUTING -m comment --comment "Antrea: do not track external to external IP request packets" -m set --match-set ANTREA-EXTERNAL-IP dst -j NOTRACK
-A ANTREA-PREROUTING -m comment --comment "Antrea: do not track external to external IP reply packets" -m set --match-set ANTREA-EXTERNAL-IP src -j NOTRACK
COMMIT
*mangle
:ANTREA-MANGLE - [0:0]
:ANTREA-OUTPUT - [0:0]
...
*nat
:ANTREA-PREROUTING - [0:0]
-A ANTREA-PREROUTING -m comment --comment "Antrea: bypass external to kube Service traffic when kube Service Endpoint is not overridden" -d 10.96.0.1 -p tcp --dport 443 -j KUBE-SERVICES
-A ANTREA-PREROUTING -m comment --comment "Antrea: accept external to ClusterIP packets" -d 10.96.0.0/12 -j ACCEPT
...
-A ANTREA-PREROUTING -m comment --comment "Antrea: accept external to external IP packets" -m set --match-set ANTREA-EXTERNAL-IP dst -j ACCEPT
:ANTREA-OUTPUT - [0:0]
-A ANTREA-OUTPUT -m comment --comment "Antrea: bypass local to kube Service traffic when kube Service Endpoint is not overridden" -d 10.96.0.1 -p tcp --dport 443 -j KUBE-SERVICES
-A ANTREA-OUTPUT -m comment --comment "Antrea: accept local to ClusterIP packets" -d 10.96.0.0/12 -j ACCEPT
...
-A ANTREA-OUTPUT -m comment --comment "Antrea: accept local to external IP packets" -m set --match-set ANTREA-EXTERNAL-IP dst -j ACCEPT
...
-A ANTREA-PREROUTING -m comment --comment "Antrea: do not track incoming encapsulation packets" -m udp -p udp --dport 6081 -m addrtype --dst-type LOCAL -j NOTRACK
-A ANTREA-OUTPUT -m comment --comment "Antrea: do not track outgoing encapsulation packets" -m udp -p udp --dport 6081 -m addrtype --src-type LOCAL -j NOTRACK
-A ANTREA-PREROUTING -m comment --comment "Antrea: do not track external to external IP request packets" -m set --match-set ANTREA-EXTERNAL-IP6 dst -j NOTRACK
-A ANTREA-PREROUTING -m comment --comment "Antrea: do not track external to external IP reply packets" -m set --match-set ANTREA-EXTERNAL-IP6 src -j NOTRACK
COMMIT
*mangle
:ANTREA-MANGLE - [0:0]
:ANTREA-OUTPUT - [0:0]
...
*nat
:ANTREA-PREROUTING - [0:0]
-A ANTREA-PREROUTING -m comment --comment "Antrea: accept external to ClusterIP packets" -d fec0::/64 -j ACCEPT
...
-A ANTREA-PREROUTING -m comment --comment "Antrea: accept external to external IP packets" -m set --match-set ANTREA-EXTERNAL-IP6 dst -j ACCEPT
:ANTREA-OUTPUT - [0:0]
-A ANTREA-OUTPUT -m comment --comment "Antrea: accept local to ClusterIP packets" -d fec0::/64 -j ACCEPT
...
-A ANTREA-OUTPUT -m comment --comment "Antrea: accept local to external IP packets" -m set --match-set ANTREA-EXTERNAL-IP6 dst -j ACCEPT
:ANTREA-POSTROUTING - [0:0]
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.proxyAll {
				...
				c.serviceCIDRProvider = mockServiceCIDRProvider
				serviceCIDRs, _ := utilnet.ParseCIDRs([]string{"10.96.0.0/12", "fec0::/64"})
				mockServiceCIDRProvider.EXPECT().GetServiceCIDRs().Return(serviceCIDRs, nil).Times(2)
				c.kubeServiceHost = "10.96.0.1"
				c.kubeServicePort = intstr.FromInt32(int32(443))
			}
