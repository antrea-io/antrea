...
func TestIPTables(t *testing.T) {
	client := &Client{}
	ipt, err := iptables.New(true, true)
	require.NoError(t, err)
	client.iptables = ipt

	exitingJumpRuleKeys := client.getJumpRuleKeys(iptables.ProtocolDual)
	expectedJumpRuleKeys := sets.New[jumpRuleKey](jumpRuleKey{iptables.NATTable, iptables.PreRoutingChain, antreaPreRoutingChain})
	for key := range exitingJumpRuleKeys {
		if expectedJumpRuleKeys.Has(key) {
			err := client.iptables.DeleteRule(iptables.ProtocolDual, key.table, key.srcChain, []string{"-j", key.dstChain})
			t.Log(err)
		}
	}
}
...
`, false, true)
				...
				mockIPTables.ListRules(iptables.ProtocolDual, iptables.MangleTable, iptables.PreRoutingChain).Return(map[iptables.Protocol][]string{
					iptables.ProtocolIPv4: {
						"-A " + iptables.MangleTable + " -j ANTREA-MANGLE",
					},
					iptables.ProtocolIPv6: {
						"-A " + iptables.MangleTable + " -j ANTREA-MANGLE",
					},
				...
				mockIPTables.DeleteRule(iptables.ProtocolDual, iptables.MangleTable, iptables.PreRoutingChain, []string{"-j", "ANTREA-MANGLE"})
...
`, false, true)
				...
				mockIPTables.ListRules(iptables.ProtocolDual, iptables.FilterTable, iptables.InputChain).Return(map[iptables.Protocol][]string{
					iptables.ProtocolIPv4: {
						"-A " + iptables.FilterTable + " -j " + antreaInputChain,
					},
					iptables.ProtocolIPv6: {
						"-A " + iptables.FilterTable + " -j " + antreaInputChain,
					},
				...
				mockIPTables.ListRules(iptables.ProtocolDual, iptables.FilterTable, iptables.OutputChain).Return(map[iptables.Protocol][]string{
					iptables.ProtocolIPv4: {
						"-A " + iptables.FilterTable + " -j " + antreaOutputChain,
					},
					iptables.ProtocolIPv6: {
						"-A " + iptables.FilterTable + " -j " + antreaOutputChain,
					},
				}, nil)
				mockIPTables.DeleteRule(iptables.ProtocolDual, iptables.FilterTable, iptables.InputChain, []string{"-j", antreaInputChain})
				mockIPTables.DeleteChain(iptables.ProtocolDual, iptables.FilterTable, antreaInputChain)
				mockIPTables.DeleteRule(iptables.ProtocolDual, iptables.FilterTable, iptables.OutputChain, []string{"-j", antreaOutputChain})
