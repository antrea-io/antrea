...
func TestEnqueue(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// The rules must be programmed, not only cached. Enqueue leaves that to the sync loop, so
			// what it must do is notify it. What the sync then writes is covered by TestSyncIPTables.
			if tt.expectSync {
				assertSyncQueued(t, c.iptablesSyncQueue)
			} else {
				assertSyncNotQueued(t, c.iptablesSyncQueue)
...
func TestEnqueueRemovesRulesWhenEndpointGoesAway(t *testing.T) {
	...
	// The removal below must reach the datapath too, so drain the notification of this first change to
	// tell the two apart.
	assertSyncQueued(t, c.iptablesSyncQueue)
...
// newIPTablesSyncQueue builds the queue a Client uses to notify its sync loop.
...
// assertSyncQueued asserts that a sync notification is waiting in the queue. The notification is delayed,
// so it is not in the queue immediately.
func assertSyncQueued(t *testing.T, queue workqueue.TypedRateLimitingInterface[string]) {
	t.Helper()
	assert.Eventually(t, func() bool { return queue.Len() > 0 }, 2*time.Second, 10*time.Millisecond,
		"Expected a sync to be queued")
}

// assertSyncNotQueued asserts that no sync notification is queued, and that none arrives.
func assertSyncNotQueued(t *testing.T, queue workqueue.TypedRateLimitingInterface[string]) {
	t.Helper()
	assert.Never(t, func() bool { return queue.Len() > 0 }, 2*syncDebounceDuration, 10*time.Millisecond,
		"Expected no sync to be queued")
}

func TestTriggerSync(t *testing.T) {
	c := &Client{iptablesSyncQueue: newIPTablesSyncQueue()}
	// Triggering the sync multiple times must not block, and must queue a single notification: a sync
	// rebuilds the whole state from the caches, so one covers them all.
	for i := 0; i < 3; i++ {
...
// TestRunSyncLoopRetriesFailedSync covers the retry of a failed sync: the notification which caused it was
// consumed, so without the retry nothing would run the sync again until the next update or the next
// periodic notification, up to SyncInterval later.
func TestRunSyncLoopRetriesFailedSync(t *testing.T) {
	queue := workqueue.NewTypedRateLimitingQueue(
		workqueue.NewTypedItemExponentialFailureRateLimiter[string](time.Millisecond, 10*time.Millisecond))
	syncCalls := make(chan struct{}, 10)
	var failures atomic.Int32
	sync := func() error {
		syncCalls <- struct{}{}
		// Fail the first two syncs, so that the retry is exercised more than once.
		if failures.Add(1) <= 2 {
	...
	defer cancel()
	c := &Client{}
	go c.runSyncLoop(ctx, queue, sync)
	...
	for i := 0; i < 3; i++ {
		...
		case <-time.After(5 * time.Second):
			t.Fatalf("The sync was not retried, got %d calls", i)
		}
	}
	// The third sync succeeded, so the notification must not be requeued any more.
	select {
	case <-syncCalls:
		t.Error("The sync was retried after it succeeded")
...
func TestRunSyncLoopIPTables(t *testing.T) {
	...
	go func() {
		defer close(stopped)
		c.runSyncLoop(ctx, c.iptablesSyncQueue, func() error { return c.syncIPTables(false) })
	}()
