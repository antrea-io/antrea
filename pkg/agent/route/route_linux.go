...
type Client struct {
	...
	// used in testing.
	deterministic bool
	// wireguardPort is the port at which to listen the Wireguard traffic. The iptables rules are added to accept the UDP
	// packets destined to the Wireguard port when wireguard is used as the encryption mode.
	wireguardPort int
}
