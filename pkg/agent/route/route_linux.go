...
func (c *Client) Initialize(nodeConfig *config.NodeConfig, done func()) error {
	...
	if err != nil {
		return fmt.Errorf("error creating NFT instance: %v", err)
	}
...
func (c *Client) syncNFTables() error {
	...
	if c.networkConfig.IPv4Enabled {
		// Add the nftset to hold peer Pod IPv4 CIDRs.
		...
		// TODO: add rule to accelerate external-to-Service and Pod-to-external connections.
	}
	if c.networkConfig.IPv6Enabled {
		// Add the nftset to hold peer Pod IPv6 CIDRs.
		tx.Add(&knftables.Set{
