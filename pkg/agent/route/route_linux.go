...
const (
	...
	antreaNFTablesChainForward   = "forward"
...
func (c *Client) Initialize(nodeConfig *config.NodeConfig, done func()) error {
	...
	if c.networkConfig.EnableHostNetworkAcceleration &&
		...
		if err != nil {
			...
			klog.ErrorS(err, "Failed to create nftables instance")
		} else {
			c.nftables = nftables
...
func (c *Client) Run(stopCh <-chan struct{}) {
	...
	wait.Until(c.syncNetworkConfig, SyncInterval, stopCh)
}

// syncNetworkConfig is idempotent and can be safely called on every sync operation.
func (c *Client) syncNetworkConfig() {
...
func (c *Client) syncNFTables() error {
	for ipProtocol, nft := range c.nftables.All() {
		...
		// Add a forward hook chain to hold the rules for matching connections which are eligible for flowtable acceleration.
		tx.Add(&knftables.Chain{
			Name:     antreaNFTablesChainForward,
			Type:     ptr.To(knftables.FilterType),
			Hook:     ptr.To(knftables.ForwardHook),
			Priority: ptr.To(knftables.FilterPriority),
			Comment:  ptr.To("Forward chain for storing rules"),
		})
		...
		if ipProtocol == knftables.IPv4Family {
			// Add the set for storing peer Pod IPv4 CIDRs.
			tx.Add(&knftables.Set{
				Name:    antreaNFTablesSetPeerPodCIDR,
				Type:    "ipv4_addr",
				Flags:   []knftables.SetFlag{knftables.IntervalFlag},
				Comment: ptr.To("Set for storing IPv4 peer Pods CIDRs"),
			})
			...
			tx.Add(&knftables.Rule{
				Chain: antreaNFTablesChainForward,
		...
		if ipProtocol == knftables.IPv6Family {
			// Add the nft set for storing peer Pod IPv6 CIDRs.
			tx.Add(&knftables.Set{
				Name:    antreaNFTablesSetPeerPodCIDR,
				Type:    "ipv6_addr",
				Flags:   []knftables.SetFlag{knftables.IntervalFlag},
				Comment: ptr.To("Set for storing IPv6 peer Pods CIDRs"),
			})
			// Flush the set if it exists.
			tx.Flush(&knftables.Set{
				Name: antreaNFTablesSetPeerPodCIDR,
