...
func (c *Client) Initialize(nodeConfig *config.NodeConfig, done func()) error {
	...
	if c.egressRequiresPolicyBasedRouting() {
...
// egressRequiresPolicyBasedRouting returns true when Egress is enabled and policy based routing
...
// Pod-Pod traffic path (e.g. Pod-Pod may go via routing in hybrid), so symmetric-path handling for
// Egress reply packets is needed.
func (c *Client) egressRequiresPolicyBasedRouting() bool {
