...
func TestSyncRoutes(t *testing.T) {
	...
	mockNetUtil := winnettesting.NewMockInterface(ctrl)
	...
	mockNetUtil.EXPECT().ReplaceNetRoute(nodeRoute1)
	mockNetUtil.EXPECT().ReplaceNetRoute(nodeRoute2)
	mockNetUtil.EXPECT().ReplaceNetRoute(serviceRoute1)
	mockNetUtil.EXPECT().ReplaceNetRoute(serviceRoute2)
	mockNetUtil.EXPECT().ReplaceNetRoute(&winnet.Route{
		LinkIndex:         10,
...
func TestInitServiceIPRoutes(t *testing.T) {
	...
	mockNetUtil.EXPECT().ReplaceNetRoute(generateRoute(virtualServiceIPv4Net, net.IPv4zero, 10, winnet.MetricHigh))
	mockNetUtil.EXPECT().ReplaceNetRoute(generateRoute(virtualNodePortDNATIPv4Net, config.VirtualServiceIPv4, 10, winnet.MetricHigh))
...
func TestReconcile(t *testing.T) {
	...
	mockNetUtil.EXPECT().RouteListFiltered(antreasyscall.AF_INET, &winnet.Route{LinkIndex: 10}, winnet.RT_FILTER_IF).Return([]winnet.Route{
	...
	mockNetUtil.EXPECT().RemoveNetRoute(&winnet.Route{DestinationSubnet: ip.MustParseCIDR("192.168.11.0/24"), LinkIndex: 10})
...
func TestAddRoutes(t *testing.T) {
	...
	tests := []struct {
		...
		nodeIP               net.IP
		nodeGwIP             net.IP
		expectedNetUtilCalls func(mockNetUtil *winnettesting.MockInterfaceMockRecorder)
	}{
		{
			name: "encap IPv4",
			networkConfig: &config.NetworkConfig{
			...
			expectedNetUtilCalls: func(mockNetUtil *winnettesting.MockInterfaceMockRecorder) {
				mockNetUtil.ReplaceNetRoute(&winnet.Route{
...
func TestDeleteRoutes(t *testing.T) {
	...
	mockNetUtil.EXPECT().RemoveNetRoute(&winnet.Route{GatewayAddress: net.ParseIP("172.16.10.3"), DestinationSubnet: ip.MustParseCIDR("192.168.10.0/24")})
...
func TestAddNodePort(t *testing.T) {
	...
	mockNetUtil.EXPECT().ReplaceNetNatStaticMapping(nodePortNetNatStaticMapping)
...
func TestDeleteNodePort(t *testing.T) {
	...
	mockNetUtil.EXPECT().RemoveNetNatStaticMapping(nodePortNetNatStaticMapping)
...
func TestAddServiceCIDRRoute(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedNetUtilCalls: func(mockNetUtil *winnettesting.MockInterfaceMockRecorder) {
				...
				mockNetUtil.RouteListFiltered(antreasyscall.AF_INET, &winnet.Route{LinkIndex: 10}, winnet.RT_FILTER_IF).Return([]winnet.Route{
					{
				...
				mockNetUtil.RemoveNetRoute(&winnet.Route{
					DestinationSubnet: ip.MustParseCIDR("10.96.0.0/24"),
					GatewayAddress:    config.VirtualServiceIPv4,
					RouteMetric:       winnet.MetricHigh,
					LinkIndex:         10,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.expectedNetUtilCalls(mockNetUtil.EXPECT())
...
func TestAddExternalIPRoute(t *testing.T) {
	...
	mockNetUtil.EXPECT().ReplaceNetRoute(ipv4Route1)
	mockNetUtil.EXPECT().ReplaceNetRoute(ipv4Route2)
...
func TestDeleteExternalIPRoute(t *testing.T) {
	...
	mockNetUtil.EXPECT().RemoveNetRoute(ipv4Route1)
	mockNetUtil.EXPECT().RemoveNetRoute(ipv4Route2)
