...
var (
	...
	antreaNatNodePort          = util.AntreaNatNodePortName
...
func (c *Client) Initialize(nodeConfig *config.NodeConfig, done func()) error {
	...
	if err := c.netUtil.EnableIPForwarding(nodeConfig.GatewayConfig.Name); err != nil {
		return err
	}
	if !c.noSNAT {
		err := c.netUtil.NewNetNat(antreaNat, nodeConfig.PodIPv4CIDR)
		if err != nil {
			return err
		}
	}
	...
	if c.proxyAll {
		...
		if err := c.netUtil.NewNetNat(antreaNatNodePort, virtualNodePortDNATIPv4Net); err != nil {
			return err
		}
	}
	done()
...
func (c *Client) listIPRoutesOnGW() ([]winnet.Route, error) {
	...
	return c.netUtil.RouteListFiltered(family, filter, winnet.RT_FILTER_IF)
}
...
func (c *Client) DeleteExternalIPRoute(externalIP net.IP) error {
	...
	if err := c.netUtil.RemoveNetRoute(route.(*winnet.Route)); err != nil {
