...
const (
	...
	AntreaNatNodePortName = "antrea-nat-nodeport"
...
func IsVirtualAdapter(name string) (bool, error) {
	return winnetUtil.IsVirtualNetAdapter(name)
}

func EnableIPForwarding(ifaceName string) error {
	return winnetUtil.EnableIPForwarding(ifaceName)
}

func RenameVMNetworkAdapter(networkName string, macStr, newName string, renameNetAdapter bool) error {
	return winnetUtil.RenameVMNetworkAdapter(networkName, macStr, newName, renameNetAdapter)
}
...
func SetAdapterMACAddress(adapterName string, macConfig *net.HardwareAddr) error {
	return winnetUtil.SetNetAdapterMACAddress(adapterName, macConfig)
}
...
func ConfigureLinkAddresses(idx int, ipNets []*net.IPNet) error {
	...
	for _, addr := range addrsToAdd {
		...
		if err := winnetUtil.ConfigureNetAdapterIPAddress(ifaceName, addr, ""); err != nil {
...
func PrepareHNSNetwork(subnetCIDR *net.IPNet, nodeIPNet *net.IPNet, uplinkAdapter *net.Interface, nodeGateway string, dnsServers string, routes []interface{}, newName string) error {
	...
	if err != nil {
		if wait.Interrupted(err) {
			dhcpStatus, err := winnetUtil.NetAdapterIPv4DHCPEnabled(uplinkAdapter.Name)
			if err != nil {
	...
	if !ipFound {
		...
		if err := winnetUtil.ConfigureNetAdapterIPAddress(vNicName, nodeIPNet, nodeGateway); err != nil {
		...
		for _, route := range routes {
			...
			if err := winnetUtil.NewNetRoute(&newRt); err != nil {
				return err
			}
		}
...
func EnableRSCOnVSwitch(vSwitch string) error {
	return winnetUtil.EnableRSCOnVSwitch(vSwitch)
}
...
func GetDNServersByInterfaceIndex(ifIndex int) (string, error) {
	return winnetUtil.GetDNServersByNetAdapterIndex(ifIndex)
}
...
func RouteListFiltered(family uint16, filter *winnet.Route, filterMask uint64) ([]winnet.Route, error) {
	return winnetUtil.RouteListFiltered(family, filter, filterMask)
}
...
func SetInterfaceMTU(ifaceName string, mtu int) error {
	return winnetUtil.SetNetAdapterMTU(ifaceName, mtu)
}

func VirtualAdapterName(name string) string {
	return winnet.VirtualAdapterName(name)
}
...
func GetVMSwitchInterfaceName() (string, error) {
	return winnetUtil.GetVMSwitchNetAdapterName(LocalVMSwitch)
}

func VMSwitchExists() (bool, error) {
	return winnetUtil.VMSwitchExists(LocalVMSwitch)
}

func CreateVMSwitch(ifName string) error {
	return winnetUtil.CreateVMSwitch(ifName, LocalVMSwitch)
}

func RemoveVMSwitch() error {
	return winnetUtil.RemoveVMSwitch(LocalVMSwitch)
}
