...
func TestSetLinkUp(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedNetUtilCalls: func(mockUtil *winnettesting.MockInterfaceMockRecorder) {
				mockUtil.EnableNetAdapter(testName).Return(nil).Times(1)
			},
		},
		{
			...
			expectedNetUtilCalls: func(mockUtil *winnettesting.MockInterfaceMockRecorder) {
				mockUtil.EnableNetAdapter(testName).Return(fmt.Errorf("failed to enable interface %s: failed reason", testName)).Times(1)
			},
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			mockInterface := winnettesting.NewMockInterface(ctrl)
			winnetUtil = mockInterface

			defer mockNetInterfaceByName(tc.gwInterface, tc.gwInterfaceErr)()
			tc.expectedNetUtilCalls(mockInterface.EXPECT())
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedNetUtilCalls: func(mockUtil *winnettesting.MockInterfaceMockRecorder) {
				...
				mockUtil.ConfigureNetAdapterIPAddress("0", &ipv4ZeroIPNet, "").Return(nil).Times(1)
			},
		},
		{
		...
		},
		{
			...
			expectedNetUtilCalls: func(mockUtil *winnettesting.MockInterfaceMockRecorder) {
				mockUtil.RemoveNetAdapterIPAddress("0", ipv4Public).Return(nil).Times(1)
				mockUtil.ConfigureNetAdapterIPAddress("0", &ipv4ZeroIPNet, "").Return(fmt.Errorf("interface No matching")).Times(1)
			},
		},
		{
...
func TestPrepareHNSNetwork(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			ipFound:    false,
			expectedNetUtilCalls: func(mockUtil *winnettesting.MockInterfaceMockRecorder) {
				mockUtil.NetAdapterIPv4DHCPEnabled(testAdapterName).Times(1)
				mockUtil.ConfigureNetAdapterIPAddress(VirtualAdapterName("0"), &ipv4ZeroIPNet, "testGateway").Times(1)
				mockUtil.SetNetAdapterDNSServers(VirtualAdapterName("0"), testDNSServer).Times(1)
				mockUtil.NewNetRoute(gomock.Any()).Times(1)
				mockUtil.EnableRSCOnVSwitch(LocalHNSNetwork).Times(1)
			},
		},
		{
			...
			expectedNetUtilCalls: func(mockUtil *winnettesting.MockInterfaceMockRecorder) {
				mockUtil.NetAdapterIPv4DHCPEnabled(testAdapterName).Times(1)
				mockUtil.ConfigureNetAdapterIPAddress(VirtualAdapterName("0"), &ipv4ZeroIPNet, "testGateway").Return(alreadyExistsErr).Times(1)
			},
		...
		},
		{
			...
			expectedNetUtilCalls: func(mockUtil *winnettesting.MockInterfaceMockRecorder) {
				mockUtil.NetAdapterIPv4DHCPEnabled(testAdapterName).Times(1)
				mockUtil.ConfigureNetAdapterIPAddress(VirtualAdapterName("0"), &ipv4ZeroIPNet, "testGateway").Times(1)
				mockUtil.NewNetRoute(gomock.Any()).Return(fmt.Errorf("failed to create new IPForward row: ip route not found")).Times(1)
			},
