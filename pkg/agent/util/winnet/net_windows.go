...
func (h *Handle) IsVirtualNetAdapter(name string) (bool, error) {
...
func getNetAdapterStatus(adapterName string) (string, error) {
...
func (h *Handle) EnableNetAdapter(adapterName string) error {
	...
	if err := wait.PollUntilContextTimeout(context.TODO(), commandRetryInterval, commandRetryTimeout, true, func(ctx context.Context) (done bool, err error) {
		...
		status, err := getNetAdapterStatus(adapterName)
		if err != nil {
			klog.Errorf("Failed to run command %s: %v", cmd, err)
			return false, nil
		}
		if !strings.EqualFold(status, "Up") {
			klog.Infof("Waiting for host interface %s to be up", adapterName)
			return false, nil
		}
		return true, nil
	}); err != nil {
		return fmt.Errorf("failed to enable interface %s: %v", adapterName, err)
	}
	return nil
}

// ConfigureNetAdapterIPAddress adds the specified IP address on the specified network adapter.
func (h *Handle) ConfigureNetAdapterIPAddress(adapterName string, ipConfig *net.IPNet, gateway string) error {
...
// SetNetAdapterMACAddress sets the specified MAC address on network adapter.
func (h *Handle) SetNetAdapterMACAddress(adapterName string, macConfig *net.HardwareAddr) error {
...
// NetAdapterIPv4DHCPEnabled returns the IPv4 DHCP status on the specified network adapter.
func (h *Handle) NetAdapterIPv4DHCPEnabled(adapterName string) (bool, error) {
	adapter, err := getAdapterInAllCompartmentsByName(adapterName)
	if err != nil {
		return false, err
	}
...
func (h *Handle) NewNetRoute(route *Route) error {
	if route == nil {
		return nil
	}
...
func (h *Handle) NewNetNat(netNatName string, subnetCIDR *net.IPNet) error {
...
// RemoveNetNatStaticMapping removes a static mapping from a NAT instance.
...
// RemoveNetNatStaticMappings removes all static mappings from a NAT instance.
func (h *Handle) RemoveNetNatStaticMappings(netNatName string) error {
...
func (h *Handle) CreateVMSwitch(adapterName, vmSwitch string) error {
	exists, err := h.VMSwitchExists(vmSwitch)
	if err != nil {
		return err
	}
	if !exists {
		if err = createVMSwitchWithTeaming(vmSwitch, adapterName); err != nil {
			return err
		}
	}

	enabled, err := isOVSExtensionEnabled(vmSwitch)
	if err != nil {
		return err
	}
	if !enabled {
		if err = enableOVSExtension(vmSwitch); err != nil {
			return err
		}
	}
	return nil
...
func createVMSwitchWithTeaming(switchName, adapterName string) error {
	cmd := fmt.Sprintf(`New-VMSwitch -Name "%s" -NetAdapterName "%s" -EnableEmbeddedTeaming $true -AllowManagementOS $true -ComputerName $(hostname)| Enable-VMSwitchExtension "%s"`, switchName, adapterName, ovsExtensionName)
	_, err := runCommand(cmd)
	if err != nil {
		return err
	}
...
func enableOVSExtension(vmSwitch string) error {
...
func isOVSExtensionEnabled(vmSwitch string) (bool, error) {
