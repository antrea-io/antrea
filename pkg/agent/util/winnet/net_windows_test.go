...
func TestSetNetAdapterMACAddress(t *testing.T) {
	tests := []struct {
		name       string
		commandOut string
		commandErr error
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			gotErr := h.SetNetAdapterMACAddress("test-adapter", &testMACAddr)
...
func TestNewNetNat(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			gotErr := h.NewNetNat(testNetNat, testSubnetCIDR)
			assert.Equal(t, tc.wantErr, gotErr)
		})
	}
}
...
func TestRemoveNetNatStaticMapping(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, tc.wantErr, gotErr)
			assert.Equal(t, tc.wantErr, gotErr)
			gotErr = h.RemoveNetNatStaticMappings(testNetNat.Name)
			assert.Equal(t, tc.wantErr, gotErr)
		})
	}
}
...
func TestCreateVMSwitch(t *testing.T) {
	...
	getVMCmd := fmt.Sprintf(`Get-VMSwitch -Name "%s" -ComputerName $(hostname)`, testVMSwitchName)
	getExtensionCmd := fmt.Sprintf(`Get-VMSwitchExtension -VMSwitchName "%s" -ComputerName $(hostname) | ? Id -EQ "%s"`, testVMSwitchName, OVSExtensionID)
	newVMCmd := fmt.Sprintf(`New-VMSwitch -Name "%s" -NetAdapterName "%s" -EnableEmbeddedTeaming $true -AllowManagementOS $true -ComputerName $(hostname)| Enable-VMSwitchExtension "%s"`, testVMSwitchName, testSwitchName, ovsExtensionName)
	enableCmd := fmt.Sprintf(`Get-VMSwitch -Name "%s" -ComputerName $(hostname)| Enable-VMSwitchExtension "%s"`, testVMSwitchName, ovsExtensionName)
	type testFormat struct {
		name       string
		commandOut string
		commandErr error
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			defer mockRunCommand(t, tc.wantCmds, tc.commandOut, tc.commandErr, true)()
			gotErr := h.CreateVMSwitch(testSwitchName, testVMSwitchName)
			assert.Equal(t, tc.wantErr, gotErr)
		})
	}
}
...
func TestEnableNetAdapter(t *testing.T) {
	...
	getCmd := fmt.Sprintf(`Get-NetAdapter -InterfaceAlias "%s" | Select-Object -Property Status | Format-Table -HideTableHeaders`, testAdapterName)
	tests := []struct {
		name           string
		commandOut     string
		commandErr     error
...
func TestConfigureNetAdapterIPAddress(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			defer mockRunCommand(t, tc.wantCmds, tc.commandOut, tc.commandErr, true)()
			gotErr := h.ConfigureNetAdapterIPAddress(testAdapterName, tc.ipNet, tc.gateway)
			assert.Equal(t, tc.wantErr, gotErr)
		})
	}
