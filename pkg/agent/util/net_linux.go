...
func EnsureIPv6AcceptRAOnInterface(ifaceName string, value int) error {
	...
	return sysctl.EnsureSysctlNetValueIfExists(path, value)
}
...
func DisableIPv6RAOnInterface(ifaceName string) error {
	if err := EnsureIPv6AcceptRAOnInterface(ifaceName, 0); err != nil {
		return err
	}
	if err := EnsureIPv6AutoconfOnInterface(ifaceName, 0); err != nil {
		return err
	}
	return nil
}
