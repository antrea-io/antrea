...
func TestBGPRouteQuery(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedCalls: func(mockBGPServer *queriertest.MockAgentBGPPolicyInfoQuerier) {
				mockBGPServer.EXPECT().GetBGPRoutes(context.Background(), true, true).Return(
					sets.New[string]("192.168.1.0/24", "192.168.2.0/24", "fec0::10:96:10:10/128"), nil)
			},
		...
		},
		{
			...
			url:  "?ipv4=true",
			expectedCalls: func(mockBGPServer *queriertest.MockAgentBGPPolicyInfoQuerier) {
				mockBGPServer.EXPECT().GetBGPRoutes(context.Background(), true, false).Return(
					sets.New[string]("192.168.1.0/24", "192.168.2.0/24"), nil)
			},
			expectedStatus: http.StatusOK,
			expectedResponse: []apis.BGPRouteResponse{
				{
		...
		},
		{
			...
			url:  "?ipv6=true",
			expectedCalls: func(mockBGPServer *queriertest.MockAgentBGPPolicyInfoQuerier) {
				mockBGPServer.EXPECT().GetBGPRoutes(context.Background(), false, true).Return(
					sets.New[string]("fec0::192:168:77:150/128", "fec0::10:10:0:10/128"), nil)
			},
			expectedStatus: http.StatusOK,
			expectedResponse: []apis.BGPRouteResponse{
				{
