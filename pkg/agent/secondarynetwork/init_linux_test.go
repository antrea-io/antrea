...
// Used only in tests; production code goes through resolveAndCreateOVSBridge.
...
func makeANCListerFromBridgeCfg(cfg *agenttypes.OVSBridgeConfig) *fakeANCLister {
	phyIfaces := make([]crdv1alpha1.OVSPhysicalInterfaceConfig, 0, len(cfg.PhysicalInterfaces))
	for _, pi := range cfg.PhysicalInterfaces {
		phyIfaces = append(phyIfaces, crdv1alpha1.OVSPhysicalInterfaceConfig{
			Name:         pi.Name,
