...
func (c *Controller) initializeBridgeState(desired *agenttypes.OVSBridgeConfig) error {
	...
	startupBrName, _, err := findStartupSecondaryBridgeFn(c.ovsdbClient, desiredBrName)
	if err != nil {
...
func (c *Controller) reconcileBridge(desired *agenttypes.OVSBridgeConfig) error {
	...
	if desired != nil && currentBrName == desired.BridgeName {
		...
		if err := c.updatePhysicalInterfaces(currentClient, desired); err != nil {
			return err
		}
	...
	if currentClient != nil {
		...
		if err := c.deleteBridge(currentClient); err != nil {
			return err
		}
...
// bridge deletion.
func (c *Controller) deleteBridge(client ovsconfig.OVSBridgeClient) error {
