...
func Initialize(
	...
	if len(phyInterfaces) == 1 {
		...
		bridgedName, _, err := util.PrepareHostInterfaceConnection(ovsBridgeClient, &config.AdapterNetConfig{
			Name:   iface.Name,
			Index:  iface.Index,
...
func RestoreOVSBridge(secNetConfig *agentconfig.SecondaryNetworkConfig) {
	if len(secNetConfig.OVSBridges[0].PhysicalInterfaces) == 1 {
		...
		util.RestoreHostInterfaceConfiguration(secNetConfig.OVSBridges[0].BridgeName, &config.AdapterNetConfig{
			Name:   iface.Name,
			Index:  iface.Index,
			MAC:    iface.HardwareAddr,
			IPs:    ifaceIPs,
