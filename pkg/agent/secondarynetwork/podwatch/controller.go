...
func NewPodController(
	...
	sriovIfaceStore := sriovInterfaceStore
	var err error
	if sriovIfaceStore == nil {
		fs := afero.NewBasePathFs(afero.NewOsFs(), dataPath)
		serializer := &filestore.GobSerializer{}
		sriovIfaceStore, err = filestore.NewFileStore(fs, sriovInterfaceDir, serializer)
		if err != nil {
			return nil, fmt.Errorf("error creating file store for SRIOV Interfaces: %w", err)
		}
	}
	interfaceConfigurator, err := cniserver.NewSecondaryInterfaceConfigurator(ovsBridgeClient, ifaceStore, sriovIfaceStore)
	if err != nil {
