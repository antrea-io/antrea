...
func TestInitializeSecondaryInterfaceStore(t *testing.T) {
	...
	require.NoError(t, err, "No error when OVSBridgeClient is nil")
	...
	err = pc.initializeSecondaryInterfaceStore()
	require.Error(t, err, "Failed to list OVS ports") // require since failure prevents further checks
	...
	require.Equal(t, 3, pc.interfaceStore.Len(), "Only valid interfaces should be stored") // require as test logic depends on it

	_, found1 := pc.interfaceStore.GetContainerInterface(uuids["uuid1"])
	require.True(t, found1, "Interface 1 should be stored")

	_, found2 := pc.interfaceStore.GetContainerInterface(uuids["uuid2"])
	require.True(t, found2, "Interface 2 should be stored")

	_, found3 := pc.interfaceStore.GetContainerInterface(uuids["uuid4"])
	require.False(t, found3, "Unknown interface type should not be stored")
}
...
func TestReconcileSecondaryInterfaces(t *testing.T) {
	...
	require.True(t, foundPod1, "CNI Cache should contain ns1/Pod1")

	_, foundPod2 := pc.cniCache.Load("ns2/Pod2")
	require.True(t, foundPod2, "CNI Cache should contain ns2/Pod2")

	// Ensure stale interfaces are removed
	_, foundPod3 := pc.cniCache.Load("ns3/Pod3")
	require.False(t, foundPod3, "Stale interface should have been removed")
}
