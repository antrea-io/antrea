...
func TestReconcileSecondaryInterfaces(t *testing.T) {
	...
	assert.Equal(t, 2, podCount, "CNI cache should contain three Pods")

	checkPodCNIInfo := func(config *interfacestore.InterfaceConfig) {
	...
	checkPodCNIInfo(containerConfigs[0])
	checkPodCNIInfo(containerConfigs[1])
