...
func (i *Initializer) prepareHNSNetworkAndOVSExtension() error {
	...
	if err := util.PrepareHNSNetwork(subnetCIDR, i.nodeConfig.NodeTransportIPv4Addr, adapter, i.nodeConfig.UplinkNetConfig.Gateway, dnsServers, i.nodeConfig.UplinkNetConfig.Routes, hostIFName); err == nil {
		success = true
		return err
	}
	// Revert the interface rename operation if HNSNetwork creation is failed.
	return err
}
...
func (i *Initializer) prepareOVSBridgeOnHNSNetwork() error {
	...
	if ofport, err := i.ovsBridgeClient.GetOFPort(hostIFName, false); err == nil {
		...
		if uint32(ofport) > config.OfPortRange {
			i.nodeConfig.HostInterfaceOFPort = config.BridgeOFMask | uint32(ofport)
		}
		i.nodeConfig.HostInterfaceOFPort = uint32(ofport)
	} else {
		// Use config.HostOFPort=4 to create OVS host internal port
		externalIDs := map[string]interface{}{
			interfacestore.AntreaInterfaceTypeKey: interfacestore.AntreaHost,
		}
		// Create host port.
		freePort, err := i.ovsBridgeClient.AllocateOFPort(config.HostOFPort)
		if err != nil {
			klog.ErrorS(err, "Failed to find a free port on OVS")
			return err
		}
		if _, err = i.ovsBridgeClient.CreateInternalPort(hostIFName, freePort, "", externalIDs); err != nil {
			return err
		}
