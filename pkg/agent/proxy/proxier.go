...
func (p *proxier) syncProxyRules() {
	...
	// reports whether any Service could not be fully synced. The installed state of a failed Service is
	// not updated, so a later sync (at the latest the runner's maxInterval resync) recomputes the diff
	// and retries it. Both are always run; a failure in one must not skip the other.
	...
	// Service that failed here is retried by a later sync.
	p.syncedOnceMutex.Lock()
	defer p.syncedOnceMutex.Unlock()
	p.syncedOnce = true

	if !syncSuccess {
		// The specific failures were already logged by removeStaleServices/installServices.
		klog.ErrorS(nil, "One or more Services could not be fully synced, they will be retried by a later sync")
	}
}
