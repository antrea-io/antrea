...
func withPreferSameTrafficDistribution(o *proxyOptions) {
	o.preferSameTrafficDistributionEnabled = true
}
...
func newFakeProxier(routeClient route.Interface, ofClient openflow.Client, nodePortAddresses []net.IP, groupIDAllocator openflow.GroupAllocator, isIPv6 bool, options ...proxyOptionsFn) *proxier {
	o := &proxyOptions{
		...
		preferSameTrafficDistributionEnabled: false,
...
func TestMetrics(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			testClusterIPAdd(t, tc.isIPv6, false, []*corev1.Service{}, []*discovery.EndpointSlice{})
			...
			testClusterIPRemove(t, tc.protocol, tc.isIPv6, false)
