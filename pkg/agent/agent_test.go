...
func TestDeleteStaleFlowsWithRetry(t *testing.T) {
	// Subtests adjust staleFlowDeleteMaxAttempts and staleFlowDeleteRetryInterval; do not use t.Parallel().
	origMax, origInterval := staleFlowDeleteMaxAttempts, staleFlowDeleteRetryInterval
	t.Cleanup(func() {
		staleFlowDeleteMaxAttempts = origMax
	...
	tests := []struct {
		...
		},
		{
			...
			interval:    fastRetryInterval,
			setupMock: func(m *openflowtest.MockClient) {
				m.EXPECT().DeleteStaleFlows().Return(ovsErr).Times(3)
			},
			stopCh: openStopCh,
			want:   false,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				staleFlowDeleteMaxAttempts = tt.maxAttempts
