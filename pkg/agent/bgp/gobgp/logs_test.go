...
// gobgpWrapperLog is used to simulate GoBGP's internal wrapper for testing the source location resolution.
...
func TestKlogHandler_Handle(t *testing.T) {
	...
	// Capture original flag values to restore later to avoid mutating global state
	origLogToStderr := klogFlagSet.Lookup("logtostderr").Value.String()
	origV := klogFlagSet.Lookup("v").Value.String()
	defer func() {
		klogFlagSet.Set("logtostderr", origLogToStderr)
		klogFlagSet.Set("v", origV)
	}()
	...
	// Note: Because routerID is injected dynamically in Handle(), it inherits the
	// active group prefix (becoming outer.routerID here). This is a known trade-off
	// of the wrapper approach, but harmless since GoBGP doesn't use WithGroup.
	...
	// Test source location resolution through a wrapper function
	...
	// Test source location resolution through a wrapper function in another file
