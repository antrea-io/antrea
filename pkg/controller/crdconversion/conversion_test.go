...
func TestConvertEgressSingleStack(t *testing.T) {
	...
	requireStringSlice(t, v1beta2Egress, []string{"IPv4"}, "spec", "ipFamilies")
...
func TestConvertEgressAutomaticFamilySelectionRoundTrip(t *testing.T) {
	...
	_, found, _ := unstructured.NestedFieldNoCopy(v1beta1Egress.Object, "spec", "ipFamilies")
	assert.False(t, found)

	roundTripped, status := ConvertEgress(v1beta1Egress, v1beta2APIVersion)
	requireConversionSuccess(t, status)
	requireStringSlice(t, roundTripped, []string{"IPv6"}, "spec", "ipFamilies")
...
func TestConvertEgressDualStackRoundTrip(t *testing.T) {
	v1beta2Egress := &unstructured.Unstructured{Object: map[string]interface{}{
		...
		"spec": map[string]interface{}{
			...
			"ipFamilies":     []interface{}{"IPv4", "IPv6"},
		},
		"status": map[string]interface{}{
			"egressIPs":  []interface{}{"192.0.2.10", "2001:db8::10"},
			"egressNode": "node-a",
	...
	requireValidForCRDVersion(t, "../../../build/charts/antrea/crds/egress.yaml", "v1beta2", v1beta2Egress)
	...
	requireStringSlice(t, roundTripped, []string{"IPv4", "IPv6"}, "spec", "ipFamilies")
...
func TestConvertEgressDualStackOrderRoundTrip(t *testing.T) {
	for _, ips := range [][]string{{"192.0.2.10", "2001:db8::10"}, {"2001:db8::10", "192.0.2.10"}} {
		for _, families := range [][]string{{"IPv4", "IPv6"}, {"IPv6", "IPv4"}} {
			t.Run(ips[0]+"/"+families[0], func(t *testing.T) {
				...
				require.NoError(t, unstructured.SetNestedStringSlice(object.Object, ips, "spec", "egressIPs"))
				require.NoError(t, unstructured.SetNestedStringSlice(object.Object, families, "spec", "ipFamilies"))
				require.NoError(t, unstructured.SetNestedStringSlice(object.Object, ips, "status", "egressIPs"))
				requireValidForCRDVersion(t, "../../../build/charts/antrea/crds/egress.yaml", "v1beta2", object)

				old, status := ConvertEgress(object, v1beta1APIVersion)
				requireConversionSuccess(t, status)
				requireValidForCRDVersion(t, "../../../build/charts/antrea/crds/egress.yaml", "v1beta1", old)
				assert.Equal(t, ips[0], mustNestedString(t, old, "spec", "egressIP"))
				roundTripped, status := ConvertEgress(old, v1beta2APIVersion)
				requireConversionSuccess(t, status)
				assert.Equal(t, object.Object, roundTripped.Object)
			})
		}
	}
}
...
func TestConvertEgressOldClientChangesDualStackSpec(t *testing.T) {
	...
	requireStringSlice(t, roundTripped, []string{"IPv4"}, "spec", "ipFamilies")
	assert.Equal(t, "SingleStack", mustNestedString(t, roundTripped, "spec", "ipFamilyPolicy"))
}
...
func TestConvertExternalIPPoolDualStackRoundTrip(t *testing.T) {
	...
	_, found, _ = unstructured.NestedFieldNoCopy(v1beta1Pool.Object, "spec", "subnets")
	assert.False(t, found)

	roundTripped, status := ConvertExternalIPPool(v1beta1Pool, v1beta2APIVersion)
	requireConversionSuccess(t, status)
	gateways, found, err := unstructured.NestedSlice(roundTripped.Object, "spec", "subnets")
	require.NoError(t, err)
	require.True(t, found)
...
func TestConvertExternalIPPoolV1beta2SubnetDoesNotRestoreStaleSubnets(t *testing.T) {
	v1beta2Pool := &unstructured.Unstructured{Object: map[string]interface{}{
		"apiVersion": v1beta2APIVersion,
		"kind":       "ExternalIPPool",
		"metadata": map[string]interface{}{
		...
		"spec": map[string]interface{}{
			"subnets": []interface{}{map[string]interface{}{"gateway": "192.0.2.254", "prefixLength": int64(24)}},
		},
	}}
	...
	roundTripped, status := ConvertExternalIPPool(v1beta1Pool, v1beta2APIVersion)
	requireConversionSuccess(t, status)
	subnets, found, err := unstructured.NestedSlice(roundTripped.Object, "spec", "subnets")
	require.NoError(t, err)
	require.True(t, found)
	require.Len(t, subnets, 1)
	assert.Equal(t, "192.0.2.254", subnets[0].(map[string]interface{})["gateway"])
	_, found, _ = unstructured.NestedFieldNoCopy(roundTripped.Object, "spec", "subnetInfo")
	assert.False(t, found)
}
...
func TestEgressConversionRejectsInconsistentProjection(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "IP and family mismatch", mutate: func(data *egressConversionData) { data.IPFamilies = []string{"IPv6"} }},
		{name: "IP and family count mismatch", mutate: func(data *egressConversionData) { data.IPFamilies = []string{"IPv6", "IPv4"} }},
	} {
