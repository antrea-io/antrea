...
func validateIPRange(r crdv1beta1.IPRange, subnetInfo *crdv1beta1.SubnetInfo) (bool, string) {
	...
	gatewayIPVersion := utilip.GetIPVersion(gateway)
	if gatewayIPVersion == utilip.IPv4 {
	...
	} else if gatewayIPVersion == utilip.IPv6 {
	...
	if r.CIDR != "" {
		...
		if utilip.GetIPVersion(cidr.IP) != gatewayIPVersion {
		...
		if utilip.GetIPVersion(rStart) != gatewayIPVersion || utilip.GetIPVersion(rEnd) != gatewayIPVersion {
			return false, fmt.Sprintf(
