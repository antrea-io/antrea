...
func TestEgressControllerValidateDualStackEgressIPsCleanupWithDeletedPool(t *testing.T) {
...
func TestEgressControllerValidateDualStackEgressPartialOverlap(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:             "exact same pair with different IPv6 representation is allowed",
			existingEgresses: []runtime.Object{newDualStackEgress("egress-a", "10.10.10.1", "fd00::1")},
			egress:           newDualStackEgress("egress-b", "10.10.10.1", "fd00:0:0:0:0:0:0:1"),
			allowed:          true,
		},
		{
		...
		{
			name:             "overlapping IPv6 with different representation is not allowed",
			existingEgresses: []runtime.Object{newDualStackEgress("egress-a", "10.10.10.1", "fd00::1")},
			egress:           newDualStackEgress("egress-b", "10.10.10.2", "fd00:0:0:0:0:0:0:1"),
			expectedMessage:  "dual-stack EgressIP pair (10.10.10.2, fd00:0:0:0:0:0:0:1) partially overlaps with Egress egress-a pair (10.10.10.1, fd00::1); sharing exactly one IP of a dual-stack pair is not supported",
		},
		{
			name:    "partial overlap with ignored pair in the same Egress is allowed",
			egress:  newDualStackEgress("egress-a", "10.10.10.1", "fd00::1", "10.10.10.1", "fd00::2"),
			allowed: true,
		},
		{
			name:             "overlap from new ignored pair is allowed",
			existingEgresses: []runtime.Object{newDualStackEgress("egress-a", "10.10.10.1", "fd00::1")},
			egress:           newDualStackEgress("egress-b", "10.10.10.2", "fd00::2", "10.10.10.1", "fd00::3"),
			allowed:          true,
		},
		{
			name:             "overlap with existing ignored pair is allowed",
			existingEgresses: []runtime.Object{newDualStackEgress("egress-a", "10.10.10.2", "fd00::2", "10.10.10.1", "fd00::3")},
			egress:           newDualStackEgress("egress-b", "10.10.10.1", "fd00::1"),
			allowed:          true,
		},
		{
		...
		{
			name:             "dual-stack Egress overlapping with differently represented single-stack IPv6 is not allowed",
			existingEgresses: []runtime.Object{newSingleStackEgress("egress-a", "fd00::1")},
			egress:           newDualStackEgress("egress-b", "10.10.10.1", "fd00:0:0:0:0:0:0:1"),
			expectedMessage:  "dual-stack EgressIP pair (10.10.10.1, fd00:0:0:0:0:0:0:1) overlaps with single-stack Egress egress-a IP fd00::1; sharing an IP between single-stack and dual-stack Egresses is not supported",
		},
		{
			name:             "dual-stack Egress with IPv4-mapped address overlapping with single-stack Egress is not allowed",
			existingEgresses: []runtime.Object{newSingleStackEgress("egress-a", "10.10.10.1")},
			egress:           newDualStackEgress("egress-b", "::ffff:10.10.10.1", "fd00::1"),
			expectedMessage:  "dual-stack EgressIP pair (::ffff:10.10.10.1, fd00::1) overlaps with single-stack Egress egress-a IP 10.10.10.1; sharing an IP between single-stack and dual-stack Egresses is not supported",
		},
		{
			name:             "new ignored pair overlapping with single-stack Egress is allowed",
			existingEgresses: []runtime.Object{newSingleStackEgress("egress-a", "10.10.10.1")},
			egress:           newDualStackEgress("egress-b", "10.10.10.2", "fd00::2", "10.10.10.1", "fd00::3"),
			allowed:          true,
		},
		...
		{
			name:             "single-stack Egress overlapping with differently represented dual-stack IPv6 is not allowed",
			existingEgresses: []runtime.Object{newDualStackEgress("egress-a", "10.10.10.1", "fd00:0:0:0:0:0:0:1")},
			egress:           newSingleStackEgress("egress-b", "fd00::1"),
			expectedMessage:  "single-stack EgressIP fd00::1 overlaps with Egress egress-a dual-stack pair (10.10.10.1, fd00:0:0:0:0:0:0:1); sharing an IP between single-stack and dual-stack Egresses is not supported",
		},
		{
			name:             "single-stack Egress overlapping with ignored pair is allowed",
			existingEgresses: []runtime.Object{newDualStackEgress("egress-a", "10.10.10.2", "fd00::2", "10.10.10.1", "fd00::3")},
			egress:           newSingleStackEgress("egress-b", "10.10.10.1"),
			allowed:          true,
		},
	}
	for _, tt := range tests {
