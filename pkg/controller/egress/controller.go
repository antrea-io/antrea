...
func getRequestedIPAllocations(egress *egressv1beta1.Egress) []externalippool.IPAllocation {
	pools := egress.Spec.ExternalIPPools
	ips := egress.Spec.EgressIPs
	if egress.Spec.ExternalIPPool != "" {
		pools = []string{egress.Spec.ExternalIPPool}
		ips = []string{egress.Spec.EgressIP}
	}

	var allocations []externalippool.IPAllocation
	for i, pool := range pools {
		if i >= len(ips) {
			break
		}
		ipStr := ips[i]
		// Ignore entries that don't have an EgressIP assigned.
		if ipStr == "" {
			continue
		}
		...
		if ip == nil {
			klog.ErrorS(
				fmt.Errorf("invalid IP address %q", ipStr),
				"Skipping EgressIP while restoring allocations",
				"egress", klog.KObj(egress),
				"pool", pool,
			)
			continue
		}
		allocations = append(allocations, externalippool.IPAllocation{
...
func (c *EgressController) restoreIPAllocations(egresses []*egressv1beta1.Egress) {
	...
	for _, egress := range egresses {
		...
		previousIPAllocations = append(previousIPAllocations, getRequestedIPAllocations(egress)...)
	}
	...
	for _, alloc := range succeededAllocations {
		...
		existingAllocation, _ := c.getIPAllocation(egressName)
		if existingAllocation == nil {
			existingAllocation = &multipleIPAllocation{
		...
		for idx, pool := range pools {
			if pool == alloc.IPPoolName {
				existingAllocation.allocs[idx] = a
				break
			}
		}
		c.setIPAllocation(egressName, existingAllocation)
...
func (c *EgressController) ipAllocationsValid(pools, specIPs []string, alloc *multipleIPAllocation) bool {
	...
	for i, poolName := range pools {
		...
		if i < len(specIPs) && specIPs[i] != "" {
			if !prevAlloc.ip.Equal(net.ParseIP(specIPs[i])) {
...
func sameSpecAllocation(pools, specIPs []string, alloc *multipleIPAllocation) bool {
	...
	for i, poolName := range pools {
		...
		if i < len(specIPs) && specIPs[i] != "" && !a.ip.Equal(net.ParseIP(specIPs[i])) {
			return false
		}
	}
	return true
...
func (c *EgressController) rollbackIPAllocations(egressName string, allocs []*ipAllocation) {
	for _, alloc := range allocs {
		c.releaseEgressIP(egressName, alloc.ip, alloc.ipPool)
	}
}
	...
	if len(spec.specIPs) > 0 {
		for i, ipStr := range spec.specIPs {
			...
			if err := c.externalIPAllocator.UpdateIPAllocation(spec.pools[i], ip); err != nil {
				c.rollbackIPAllocations(egress.Name, allocs)
...
// releaseEgressIP releases the IP to the pool.
