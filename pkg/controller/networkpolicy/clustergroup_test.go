...
func TestGetGroupsForNode(t *testing.T) {
	...
	t.Run("groups returned", func(t *testing.T) {
		tests := []struct {
			name           string
			queryName      string
		...
		_, npc := newController([]runtime.Object{nodeA, nodeB}, nil)
		stopCh := make(chan struct{})
		defer close(stopCh)
		npc.informerFactory.Start(stopCh)
		npc.informerFactory.WaitForCacheSync(stopCh)
	...
	t.Run("no groups returned", func(t *testing.T) {
		t.Run("node cannot be found", func(t *testing.T) {
			_, npc := newController(nil, nil)
			_, exists := npc.getGroupsForNode("any")
			assert.False(t, exists, "Expected no groups when the node cannot be found")
		})
		t.Run("no groups exist", func(t *testing.T) {
			_, npc := newController([]runtime.Object{nodeA, nodeB, nodeC}, nil)
			stopCh := make(chan struct{})
			defer close(stopCh)
			npc.informerFactory.Start(stopCh)
			npc.informerFactory.WaitForCacheSync(stopCh)
			_, exists := npc.getGroupsForNode("nodeB")
			assert.False(t, exists, "Expected no groups when the node cannot be found")
		})
		t.Run("no applicable groups exist", func(t *testing.T) {
			_, npc := newController([]runtime.Object{nodeA, nodeB, nodeC}, nil)
			stopCh := make(chan struct{})
			defer close(stopCh)
			npc.informerFactory.Start(stopCh)
			npc.informerFactory.WaitForCacheSync(stopCh)
			createGroups(npc, groups)
			_, exists := npc.getGroupsForNode("nodeC")
			assert.False(t, exists, "Expected no groups when the node cannot be found")
		})
	})
}
