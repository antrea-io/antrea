...
// addACNP receives AntreaClusterNetworkPolicy ADD events and enqueues a reference of
// the AntreaClusterNetworkPolicy to trigger its process.
func (n *NetworkPolicyController) addACNP(obj interface{}) {
	defer n.heartbeat("addACNP")
	cnp := obj.(*crdv1beta1.ClusterNetworkPolicy)
	klog.Infof("Processing AntreaClusterNetworkPolicy %s ADD event", cnp.Name)
	n.enqueueInternalNetworkPolicy(getACNPReference(cnp))
}

// updateACNP receives AntreaClusterNetworkPolicy UPDATE events and enqueues a
// reference of the AntreaClusterNetworkPolicy to trigger its process.
func (n *NetworkPolicyController) updateACNP(_, cur interface{}) {
	defer n.heartbeat("updateACNP")
	curCNP := cur.(*crdv1beta1.ClusterNetworkPolicy)
	klog.Infof("Processing AntreaClusterNetworkPolicy %s UPDATE event", curCNP.Name)
	n.enqueueInternalNetworkPolicy(getACNPReference(curCNP))
}

// deleteACNP receives AntreaClusterNetworkPolicy DELETE events and enqueues a
// reference of the AntreaClusterNetworkPolicy to trigger its process.
func (n *NetworkPolicyController) deleteACNP(old interface{}) {
	cnp, ok := old.(*crdv1beta1.ClusterNetworkPolicy)
	if !ok {
		tombstone, ok := old.(cache.DeletedFinalStateUnknown)
		if !ok {
			klog.Errorf("Error decoding object when deleting AntreaClusterNetworkPolicy, invalid type: %v", old)
			return
		}
		cnp, ok = tombstone.Obj.(*crdv1beta1.ClusterNetworkPolicy)
		if !ok {
			klog.Errorf("Error decoding object tombstone when deleting AntreaClusterNetworkPolicy, invalid type: %v", tombstone.Obj)
			return
		}
	}
	defer n.heartbeat("deleteACNP")
	klog.Infof("Processing AntreaClusterNetworkPolicy %s DELETE event", cnp.Name)
	n.enqueueInternalNetworkPolicy(getACNPReference(cnp))
}
...
// processAntreaClusterNetworkPolicy creates an internal NetworkPolicy instance
...
// of an UPDATE event.
func (n *NetworkPolicyController) processAntreaClusterNetworkPolicy(cnp *crdv1beta1.ClusterNetworkPolicy) (*antreatypes.NetworkPolicy, map[string]*antreatypes.AppliedToGroup, map[string]*antreatypes.AddressGroup) {
