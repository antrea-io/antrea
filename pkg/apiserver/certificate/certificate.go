...
func generateSelfSignedCertificate(secureServing *options.SecureServingOptionsWithLoopback, caConfig CAConfig) (dynamiccertificates.CAContentProvider, error) {
	...
	secureServing.ServerCert.PairName = caConfig.AntreaPairName

	if err := secureServing.MaybeDefaultWithSelfSignedCerts(caConfig.AntreaServiceName, GetAntreaServerNames(caConfig.AntreaServiceName), []net.IP{net.ParseIP("127.0.0.1"), net.IPv6loopback}); err != nil {
...
func generateNewServingCertificate(secureServing *options.SecureServingOptionsWithLoopback, caConfig CAConfig) error {
	cert, key, err := certutil.GenerateSelfSignedCertKeyWithFixtures(caConfig.AntreaServiceName, []net.IP{net.ParseIP("127.0.0.1"), net.IPv6loopback}, GetAntreaServerNames(caConfig.AntreaServiceName), secureServing.ServerCert.FixtureDirectory)
	if err != nil {
