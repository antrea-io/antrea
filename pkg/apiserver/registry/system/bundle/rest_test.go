...
import (
	...
	mockofctl "github.com/vmware-tanzu/antrea/pkg/ovs/ovsctl/testing"
)
...
func TestClean(t *testing.T) {
	...
	originClientFactory := ofctlClientFactory
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	ofctlClientFactory = func(bridge string) ovsctl.OVSCtlClient {
		c := mockofctl.NewMockOVSCtlClient(ctrl)
		c.EXPECT().DumpFlows().Return([]string{"flow1"}, nil).Times(1)
		return c
	}
	defer func() {
		ofctlClientFactory = originClientFactory
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			go storage.Bundle.clean(ctx, f.Name(), tc.duration)
			time.Sleep(200 * time.Millisecond)
			exist, err := afero.Exists(defaultFS, f.Name())
			require.NoError(t, err)
			require.False(t, exist)
			require.Equal(t, system.BundleStatusNone, storage.Bundle.cache.Status)
		})
	}
}
...
func TestCollect(t *testing.T) {
	...
	for name, function := range map[string]func(context.Context) (*bundle, error){
		"Agent":      storage.Bundle.collectAgent,
		"Controller": storage.Bundle.collectController,
	} {
		t.Run(name, func(t *testing.T) {
			defaultFS = afero.NewMemMapFs()
			defaultExecutor = new(testExec)
			...
			require.NoError(t, err)
			require.Equal(t, bundle.Status, system.BundleStatusCollected)
