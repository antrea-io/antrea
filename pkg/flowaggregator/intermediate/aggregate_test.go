...
func TestFromExternalCorrelationRequired(t *testing.T) {
	...
	t.Run("IPv6", func(t *testing.T) {
		t.Run("correlation is not required", func(t *testing.T) {
			record := &flowpb.Flow{
				Ip: &flowpb.IP{
					// 2001:0db8:85a3:0000:0000:8a2e:0370:7334 would look like this:
					Source: []byte{0x20, 0x01, 0x0d, 0xb8, 0x85, 0xa3, 0x00, 0x00, 0x00, 0x00, 0x8a, 0x2e, 0x03, 0x70, 0x73, 0x34},
				},
				K8S: &flowpb.Kubernetes{
					DestinationPodName:         destinationPodName,
					DestinationPodNamespace:    "something",
		...
		t.Run("the flow is from the destinationNode", func(t *testing.T) {
			record := &flowpb.Flow{
				Ip: &flowpb.IP{
					// IPv6 address beginning with fd (internal address)
					Source: []byte{0xfd, 0x01, 0x0d, 0xb8, 0x85, 0xa3, 0x00, 0x00, 0x00, 0x00, 0x8a, 0x2e, 0x03, 0x70, 0x73, 0x34},
				},
				K8S: &flowpb.Kubernetes{
					DestinationPodName:         "not empty",
					DestinationPodNamespace:    "not empty",
