...
const (
	...
	// Incrementing next_cfg explicitly asks ovs-vswitchd to reload the configuration.
	...
	// ovsConfigWaitInterval is the interval at which cur_cfg is checked while waiting
	// for ovs-vswitchd to acknowledge the configuration.
	ovsConfigWaitInterval = 10 * time.Millisecond
...
// SetMcastSnooping enables or disables multicast snooping on the bridge.
func (br *OVSBridge) SetMcastSnooping(enable bool) error {
	// TODO: use ctx from parent context
	ctx := context.TODO()

	updateBridge := &Bridge{McastSnoopingEnable: enable}
	ops, err := br.ovsdb.Where(bridgeWithUUID(br.uuid)).Update(updateBridge, &updateBridge.McastSnoopingEnable)
	if err != nil {
		klog.ErrorS(err, "Failed to construct update operations for multicast snooping", "bridge", br.name)
		return err
	}
...
// DeletePorts deletes the ports with the provided port UUIDs in a single transaction.
// If a port does not exist no change will be done.
func (br *OVSBridge) DeletePorts(portUUIDList []string) error {
	portUUIDs := make([]string, 0, len(portUUIDList))
	seen := sets.New[string]()
	for _, portUUID := range portUUIDList {
		if portUUID == "" || seen.Has(portUUID) {
			continue
		}
		seen.Insert(portUUID)
	...
	_, err = br.transact(ctx, ops, "delete ports")
	return err
}
...
func (br *OVSBridge) SetPortTrunks(portName string, vlanSpecs []string) error {
	...
	return br.transactAndWaitForConfig(ctx, ops, "set port trunks")
}
...
// transactAndWaitForConfig executes the supplied operations, requests an ovs-vswitchd
...
func (br *OVSBridge) transactAndWaitForConfig(ctx context.Context, ops []ovsdb.Operation, action string) error {
	ovs, err := br.getOpenvSwitch(ctx)
	if err != nil {
		return err
	}
...
func (br *OVSBridge) waitForConfig(ctx context.Context, nextCfg int) error {
	ticker := time.NewTicker(ovsConfigWaitInterval)
	defer ticker.Stop()
