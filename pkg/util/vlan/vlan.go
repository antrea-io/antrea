...
func ParseSpec(spec string) (IDRange, error) {
	...
	if idx := strings.IndexByte(spec, '-'); idx >= 0 {
		...
		if err != nil {
			return IDRange{}, fmt.Errorf("invalid VLAN range start %q: %v", startStr, err)
		}
		end, err := strconv.ParseUint(endStr, 10, 16)
		if err != nil {
			return IDRange{}, fmt.Errorf("invalid VLAN range end %q: %v", endStr, err)
		}
		if start > end {
			return IDRange{}, fmt.Errorf("VLAN range start %d is greater than end %d", start, end)
		}
		if end > uint64(MaxID) {
			return IDRange{}, fmt.Errorf("VLAN range end %d is greater than maximum VLAN ID %d", end, MaxID)
		}
	...
	if err != nil {
		return IDRange{}, fmt.Errorf("invalid VLAN ID %q: %v", spec, err)
	}
	if id > uint64(MaxID) {
		return IDRange{}, fmt.Errorf("VLAN ID %d is greater than maximum VLAN ID %d", id, MaxID)
	}
	return IDRange{Start: uint16(id), End: uint16(id)}, nil
}

// ExpandSpecs converts VLAN specifications into a flat slice of VLAN IDs.
