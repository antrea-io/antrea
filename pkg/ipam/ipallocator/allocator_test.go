...
func newCIDRAllocator(cidr string, reservedIPs []string) *SingleIPAllocator {
	return newCIDRAllocatorWithOptions(cidr, reservedIPs, false)
}

func newFullCIDRAllocator(cidr string, reservedIPs []string) *SingleIPAllocator {
	return newCIDRAllocatorWithOptions(cidr, reservedIPs, true)
}

func newCIDRAllocatorWithOptions(cidr string, reservedIPs []string, allowNetworkAndBroadcast bool) *SingleIPAllocator {
	_, ipNet, _ := net.ParseCIDR(cidr)
	var parsedIPs []net.IP
	for _, ip := range reservedIPs {
		parsedIPs = append(parsedIPs, net.ParseIP(ip))
	}
	allocator, _ := NewCIDRAllocator(ipNet, parsedIPs, allowNetworkAndBroadcast)
	return allocator
}
...
func TestAllocateNext(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "IPv4-CIDR-prefix-24-allowNetworkAndBroadcast",
		...
		{
			name:        "IPv4-CIDR-prefix-30-allowNetworkAndBroadcast",
		...
		{
			name:        "IPv4-CIDR-prefix-31-allowNetworkAndBroadcast",
...
func TestAllowNetworkAndBroadcastTotal(t *testing.T) {
