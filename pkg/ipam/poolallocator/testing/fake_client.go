...
// InitPool registers a pool using Create, which triggers the create reactor
// to assign ResourceVersion and fire watch events. Kept for compatibility
// with existing test code that calls InitPool.
func (c *IPPoolClientset) InitPool(pool *crdv1b1.IPPool) {
	c.CrdV1beta1().IPPools().Create(context.TODO(), pool, metav1.CreateOptions{})
}
