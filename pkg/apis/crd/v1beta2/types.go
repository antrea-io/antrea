...
type EgressSpec struct {
	...
	// Egress. Two addresses, one per IP family in any order, represent a dual-stack Egress. When EgressIPs is not set,
	// ExternalIPPool must be provided and the addresses will be allocated from the pool automatically.
	...
	// specified and the addresses need to be manually assigned to a Node.
	...
	// IPFamilies specifies the IP families when allocating EgressIPs, in any order.
	// It can be omitted for a dual-stack Egress or when ExternalIPPool contains a single IP family.
	// A single-stack Egress which requests automatic allocation from a dual-stack ExternalIPPool must specify one IP
	// family.
	// +listType=atomic
...
type EgressStatus struct {
	...
	// EgressIPs are the effective Egress IPs for the selected workloads, in any order.
	// It may be empty when no IP has been allocated or assigned yet.
	EgressIPs  []string          `json:"egressIPs,omitempty"`
...
type ExternalIPPoolSpec struct {
	...
	// Subnets specifies the subnet attributes for each IP family. At most one subnet per family is allowed.
	// When omitted, allocated IPs are assumed to belong to the Node subnet.
	// +listType=atomic
