...
type AntreaNodeConfigSpec struct {
	...
	// (the earliest-created one — oldest creationTimestamp wins) will be applied.
	NodeSelector metav1.LabelSelector `json:"nodeSelector"`
...
type OVSBridgeConfig struct {
	// BridgeName is the name of the OVS bridge. Once set, it is immutable.
	// +required
...
// OVSPhysicalInterfaceConfig defines a physical interface configuration including VLAN filters on OVS bridge.
type OVSPhysicalInterfaceConfig struct {
