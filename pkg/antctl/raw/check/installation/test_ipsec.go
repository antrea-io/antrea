...
type IPsecConnectivityTest struct{}
...
func init() {
	RegisterTest("ipsec-connectivity", &IPsecConnectivityTest{})
}

func (t *IPsecConnectivityTest) Run(ctx context.Context, testContext *testContext) error {
	...
	defer func() {
		...
		if err := testContext.client.AppsV1().Deployments(testContext.namespace).Delete(ctx, ipsecToolboxDeploymentName, metav1.DeleteOptions{}); err != nil {
			testContext.Warning("Failed to delete tcpdump Deployment: %w", err)
		}
	}()
