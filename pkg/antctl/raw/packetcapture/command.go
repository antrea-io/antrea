...
func parseFlow(options *packetCaptureOptions) (*v1alpha1.Packet, error) {
	...
	if typeVal, typeOK := fields["icmp_type"]; typeOK {
		codeVal, codeOK := fields["icmp_code"]
		var icmpType intstr.IntOrString
		if val, err := strconv.ParseUint(typeVal, 0, 8); err == nil {
			icmpType = intstr.FromInt32(int32(val))
		} else {
			_, found := capture.ICMPMsgTypeMap[v1alpha1.ICMPMsgType(typeVal)]
			if !found {
				return nil, fmt.Errorf("unknown icmp_type: %s", typeVal)
			}
			icmpType = intstr.FromString(typeVal)
		}

		pkt.TransportHeader.ICMP = new(v1alpha1.ICMPHeader)

		if codeOK {
			icmpCode, err := strconv.ParseUint(codeVal, 0, 8)
			if err != nil {
				return nil, err
			}
