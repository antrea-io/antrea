...
func isCaptureConditionFailed(reason string) bool {
	return reason == "Failed" || reason == "Timeout"
}

func checkConditionAndAppendError(
	conditions map[v1alpha1.PacketCaptureConditionType]v1alpha1.PacketCaptureCondition,
	conditionType v1alpha1.PacketCaptureConditionType,
	errorMessages *[]string,
	prefix string,
) {
	if c, ok := conditions[conditionType]; ok && isCaptureConditionFailed(c.Reason) && c.Message != "" {
		*errorMessages = append(*errorMessages, fmt.Sprintf("%s: %s", prefix, c.Message))
	}
}

// getCaptureFailureError processes the status of a PacketCapture and returns a
// consolidated error if any of the capture stages failed.
func getCaptureFailureError(status *v1alpha1.PacketCaptureStatus) error {
	...
	checkConditionAndAppendError(conditions, v1alpha1.PacketCaptureAtSrcComplete, &errorMessages, "Source")
	checkConditionAndAppendError(conditions, v1alpha1.PacketCaptureAtDstComplete, &errorMessages, "Destination")
	checkConditionAndAppendError(conditions, v1alpha1.PacketCaptureComplete, &errorMessages, "Overall")
...
func packetCaptureRun(ctx context.Context, out io.Writer, restConfig *rest.Config, k8sClient kubernetes.Interface, antreaClient antrea.Interface, options *packetCaptureOptions) error {
	...
	if err := getCaptureFailureError(&latestPC.Status); err != nil {
		return err
	}
