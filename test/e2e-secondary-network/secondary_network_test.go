...
func (data *testData) getIPPoolNames(networkName, namespace string) ([]string, error) {
	client, err := netattdef.NewForConfig(e2eTestData.kubeConfig)
	if err != nil {
		return nil, err
	}
	...
	return netAttachDefConfig.IPAM.IPPools, nil
}

func (data *testData) checkIPReleased(ipPoolName, podIPString string) error {
	crdClient, err := e2eTestData.crdClient
	if err != nil {
		return fmt.Errorf("failed to get CRD client: %w", err)
	}

	return wait.PollUntilContextTimeout(context.Background(), 5*time.Second, 40*time.Second, true, func(ctx context.Context) (bool, error) {
		ipPool, err := crdClient.CrdV1beta1().IPPools().Get(ctx, ipPoolName, metav1.GetOptions{})
		if err != nil {
