...
var expectedSecondaryBridgeName = flag.String("expected-secondary-bridge-name", "", "Expected secondary OVS bridge name")
...
func (data *testData) verifySecondaryOVSBridgeName(t *testing.T, expectedBridgeName string) {
	t.Helper()
	for i := 0; i < antreae2e.NodeCount(); i++ {
		...
		assert.Equal(t, expectedBridgeName, bridgeName,
			"Unexpected secondary OVS bridge on node %s", nodeName)
	}
}
...
// reconciliationAfterAgentRestart verifies OVS Ports cleanup and IP release.
func (data *testData) reconciliationAfterAgentRestart(t *testing.T) error {
...
func runSecondaryNetworkTestSteps(t *testing.T, e2eTestData *antreae2e.TestData, networkType string, pods []*testPodInfo) {
	...
	t.Run("ReconciliationAfterAgentRestart", func(t *testing.T) {
		err := testData.reconciliationAfterAgentRestart(t)
...
// TestSecondaryOVSBridgeName verifies the secondary OVS bridge name supplied by the test runner.
func TestSecondaryOVSBridgeName(t *testing.T) {
	if *expectedSecondaryBridgeName == "" {
		t.Fatal("-expected-secondary-bridge-name must be provided")
	}
	e2eTestData, err := antreae2e.SetupTest(t)
	require.NoError(t, err)
	defer antreae2e.TeardownTest(t, e2eTestData)
