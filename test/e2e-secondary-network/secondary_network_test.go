...
func (data *testData) reconcilationAfterAgentRestart(t *testing.T) error {
	nodes := []string{data.pods[0].nodeName, data.pods[2].nodeName}
	for _, node := range nodes {
		ovsPortsBeforeAgentRestart, err := data.getOVSPortsOnSecondaryBridge(t, node)
		if err != nil {
		...
		ovsPortsAfterAgentRestart, err := data.getOVSPortsOnSecondaryBridge(t, node)
		if err != nil {
...
func (data *testData) deleteVLANPod(t *testing.T) error {
	...
	if checkOVSPorts(ovsPortsBeforeDeletion, ovsPortsAfterDeletion) {
		return fmt.Errorf("OVS Ports for Vlan Pod still exists: %w", err)
	}
