...
// generateTrafficUntilPacketCaptureComplete generates traffic until the PacketCapture is complete.
// The PacketCaptureStarted condition is reported by the Antrea Agent before the capture is actually
// able to capture packets: the Agent opens the capture socket, then drains it and installs the BPF
// filter from a different goroutine, which takes at least 50ms. Traffic generated as soon as the
// condition is observed can therefore be missed, and a test case which generates just enough
// packets to reach FirstN would then never see the capture complete. We keep generating traffic
// until the capture is reported as complete: for these test cases the capture stops at FirstN,
// hence the extra traffic cannot increase the number of captured packets beyond the expected one.
// We cannot stop as soon as FirstN packets have been captured: the Agent does not publish the
// status when the FirstN-th packet is captured, but only once it is done with the capture, which
// includes uploading the packets to the file server if needed. The amount of extra traffic is
// bounded by maxTrafficRounds instead. Note that a test case which expects the capture to end
// because of a timeout cannot use this at all, as it asserts an exact number of captured packets:
// see expectsCaptureTimeout.
func (data *TestData) generateTrafficUntilPacketCaptureComplete(
	...
	for {
		if rounds < maxTrafficRounds && (lastRound.IsZero() || time.Since(lastRound) >= trafficInterval) {
			start := time.Now()
			generateTraffic(t, data, tc, srcPod, dstPodIPs)
			lastRound = time.Now()
			rounds++
		...
		if time.Now().After(deadline) {
			...
			return nil, wait.ErrorInterrupted(fmt.Errorf("timed out waiting for PacketCapture %s to complete", tc.pc.Name))
		}
