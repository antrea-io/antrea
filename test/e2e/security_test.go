...
func testCert(t *testing.T, data *TestData, expectedCABundle string, restartPod bool) {
	...
	defer func() {
		if err := data.clientset.CoreV1().ConfigMaps(data.testNamespace).Delete(context.TODO(), cm.Name, metav1.DeleteOptions{}); err != nil {
			t.Errorf("Error when deleting ConfigMap %s: %v", cm.Name, err)
		}
	}()
	if _, err := data.clientset.CoreV1().ConfigMaps(data.testNamespace).Create(context.TODO(), &cm, metav1.CreateOptions{}); err != nil {
		t.Errorf("Error when creating ConfigMap %s: %v", cm.Name, err)
	}
	...
	require.NoError(t, NewPodBuilder(clientName, data.testNamespace, agnhostImage).WithContainerName(getImageName(agnhostImage)).MountConfigMap(cm.Name, "/etc/config/", "config-volume").WithHostNetwork(false).Create(data))
