...
func createPerftestServices(data *TestData, isIPv6 bool) (svcB *corev1.Service, svcC *corev1.Service, err error) {
	...
	svcB, err = data.createService("perftest-b", testNamespace, []servicePorts{{port: iperfPort, targetPort: iperfPort}}, map[string]string{"antrea-e2e": "perftest-b"}, false, false, corev1.ServiceTypeClusterIP, &svcIPFamily)
	if err != nil {
		return nil, nil, fmt.Errorf("Error when creating perftest-b Service: %v", err)
	}

	svcC, err = data.createService("perftest-c", testNamespace, []servicePorts{{port: iperfPort, targetPort: iperfPort}}, map[string]string{"antrea-e2e": "perftest-c"}, false, false, corev1.ServiceTypeClusterIP, &svcIPFamily)
	if err != nil {
