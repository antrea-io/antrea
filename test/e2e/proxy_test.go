...
func proxyEnabled(data *TestData, t *testing.T) (bool, error) {
...
func TestProxyServiceSessionAffinity(t *testing.T) {
	skipIfProviderIs(t, "kind", "Not work in kind test, need to be investigated.")
	...
	if enabled, err := proxyEnabled(data, t); err != nil {
	...
	stdout, stder, err := data.runCommandFromPod(testNamespace, "busybox", busyboxContainerName, []string{"wget", "-O", "-", svc.Spec.ClusterIP, "-T", "1"})
	require.NoError(t, err, fmt.Sprintf("stdout: %s\n, stderr: %s", stdout, stder))
	...
	require.Contains(t, table40Output, fmt.Sprintf("load:0x%s->NXM_NX_REG3[]", hexString(net.ParseIP(nginxIP).To4())))
}
...
func TestProxyServiceLifeCycle(t *testing.T) {
	...
	groupKeyword := fmt.Sprintf("load:0x%s->NXM_NX_REG3[],load:0x%x->NXM_NX_REG4[0..15],load:0xfff1->NXM_NX_REG4[16..31]", hexString(net.ParseIP(nginxIP).To4()), 80)
