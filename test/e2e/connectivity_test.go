...
func TestConnectivityAcceleration(t *testing.T) {
	...
	if err := NewPodBuilder("perftest-a", data.testNamespace, ToolboxImage).
		OnNode(nodeName(1)).
		Create(data); err != nil {
	...
	if err := NewPodBuilder("perftest-b", data.testNamespace, ToolboxImage).
		WithCommand([]string{"iperf3", "-s"}).
		OnNode(nodeName(2)).
		Create(data); err != nil {
		t.Fatalf("Error when creating the perftest server Pod: %v", err)
	}
	podIPs, err := data.podWaitForIPs(defaultTimeout, "perftest-b", data.testNamespace)
	if err != nil {
	...
	if ipv4 := podIPs.IPv4.String(); ipv4 != "" {
		verifyNFTFlowtable(ipv4)
	}
	if ipv6 := podIPs.IPv6.String(); ipv6 != "" {
		verifyNFTFlowtable(ipv6)
	}
}
