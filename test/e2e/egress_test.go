		...
		t.Run(tt.name, func(t *testing.T) {
			...
			err := wait.PollImmediate(500*time.Millisecond, 3*time.Second, func() (done bool, err error) {
				...
				if tt.expectedNodes.Len() == 0 {
					if egress.Status.NodeName != "" {
						return false, fmt.Errorf("this Egress shouldn't be assigned to any Node")
					}
				} else {
					if !tt.expectedNodes.Has(egress.Status.NodeName) {
						return false, nil
					}
				}
				return true, nil
			})
			require.NoError(t, err, "Expected egressIP=%s nodeName in %s, got egressIP=%s nodeName=%s", tt.expectedEgressIP, tt.expectedNodes.List(), egress.Spec.EgressIP, egress.Status.NodeName)
			if egress.Status.NodeName != "" {
				exists, err := hasIP(data, egress.Status.NodeName, egress.Spec.EgressIP)
...
func testEgressUpdateEgressIP(t *testing.T, data *TestData) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			egress, err = data.checkEgressState(egress.Name, newIP, tt.newNode, "", time.Second)
			require.NoError(t, err)
...
func testEgressMigration(t *testing.T, data *TestData, triggerFunc, revertFunc func(poolName, evictNode string), checkEvictNode bool, timeout time.Duration) {
	...
	require.NoError(t, err)
	assert.True(t, nodeCandidates.Has(egress.Status.NodeName))

	fromNode, toNode := nodeName(0), nodeName(1)
	if egress.Status.NodeName != fromNode {
...
func (data *TestData) checkEgressState(egressName, expectedIP, expectedNode, otherNode string, timeout time.Duration) (*v1alpha2.Egress, error) {
	...
	pollErr := wait.PollImmediate(200*time.Millisecond, timeout, func() (done bool, err error) {
		...
		if egress.Status.NodeName == expectedNode {
			return false, fmt.Errorf("expected Egress Node %s, got %s", expectedNode, egress.Status.NodeName)
		}
