...
func applyDefaultDenyToAllNamespaces(k8s *KubernetesUtils, namespaces map[string]string) error {
	...
	k8s.Validate(allPods, r, []int32{p80}, ProtocolTCP, false)
	_, wrong, _ := r.Summary()
	if wrong != 0 {
...
func testMutateANNPNoTier(t *testing.T) {
	...
	builder = builder.SetName(namespaces["x"], "annp-no-tier").
...
func testCreateValidationInvalidANNP(t *testing.T) {
	...
	builder := &AntreaNetworkPolicySpecBuilder{}
	builder = builder.SetName(namespaces["x"], "annp-no-priority").
		SetAppliedToGroup([]ANNPAppliedToSpec{{PodSelector: map[string]string{"pod": "a"}}}).
		SetPriority(1.0).
		SetTier("non-exist")
	annp := builder.Get()
...
func testUpdateValidationInvalidANNP(t *testing.T) {
	...
	builder := &AntreaNetworkPolicySpecBuilder{}
	builder = builder.SetName(namespaces["x"], "annp-applied-to-update").
		SetAppliedToGroup([]ANNPAppliedToSpec{{PodSelector: map[string]string{"pod": "a"}}}).
		SetPriority(1.0)
...
func testACNPClusterGroupAppliedToPodAdd(t *testing.T, data *TestData) {
	...
	executeTestsWithData(t, testCase, data, false)
}
...
func testANNPEgressRulePodsAToGrpWithPodsC(t *testing.T) {
	...
	builder := &AntreaNetworkPolicySpecBuilder{}
	builder = builder.SetName(namespaces["x"], "annp-deny-xa-to-grp-xc-egress").
		SetPriority(1.0).
		SetAppliedToGroup([]ANNPAppliedToSpec{{PodSelector: map[string]string{"pod": "a"}}})
...
func testANNPIngressRuleDenyGrpWithXCtoXA(t *testing.T) {
	...
	port81Name := "serve-81"
	builder := &AntreaNetworkPolicySpecBuilder{}
	builder = builder.SetName(namespaces["x"], "annp-deny-grp-with-xb-to-xa").
		SetPriority(2.0).
		SetAppliedToGroup([]ANNPAppliedToSpec{{PodSelector: map[string]string{"pod": "a"}}})
...
func testANNPGroupUpdate(t *testing.T) {
	...
	builder := &AntreaNetworkPolicySpecBuilder{}
	builder = builder.SetName(namespaces["x"], "annp-deny-xa-to-grp-with-xc-egress").
		SetPriority(1.0).
		SetAppliedToGroup([]ANNPAppliedToSpec{{PodSelector: map[string]string{"pod": "a"}}})
	builder.AddEgress(ProtocolTCP, &p80, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil,
		nil, nil, nil, nil, crdv1beta1.RuleActionDrop, grpName, "")
...
func testANNPAppliedToDenyXBtoGrpWithXA(t *testing.T) {
	...
	port81Name := "serve-81"
	builder := &AntreaNetworkPolicySpecBuilder{}
	builder = builder.SetName(namespaces["x"], "annp-deny-grp-with-xa-from-xb").
		SetPriority(2.0).
...
func testANNPGroupUpdateAppliedTo(t *testing.T) {
	...
	updatedGrpBuilder = updatedGrpBuilder.SetName(grpName).SetNamespace(namespaces["x"]).SetPodSelector(map[string]string{"pod": "b"}, nil)
	builder := &AntreaNetworkPolicySpecBuilder{}
	builder = builder.SetName(namespaces["x"], "annp-deny-grp-xc-to-xa-egress").
		SetPriority(1.0).
		SetAppliedToGroup([]ANNPAppliedToSpec{{Group: grpName}})
	builder.AddEgress(ProtocolTCP, &p80, nil, nil, nil, nil, nil, nil, nil, nil, map[string]string{"pod": "c"}, nil, nil,
		nil, nil, nil, nil, crdv1beta1.RuleActionDrop, "", "")
...
func testANNPGroupAppliedToPodAdd(t *testing.T, data *TestData) {
	...
	builder := &AntreaNetworkPolicySpecBuilder{}
	builder = builder.SetName(namespaces["x"], "annp-deny-grp-with-xj-to-xd-egress").
		SetPriority(1.0).
		SetAppliedToGroup([]ANNPAppliedToSpec{{Group: grpName}})
...
func testANNPGroupServiceRefDelete(t *testing.T) {
	...
	reachability := NewReachability(allPods, Connected)
	reachability.Expect(Pod(namespaces["x"]+"/b"), Pod(namespaces["x"]+"/a"), Dropped)
	k8sUtils.Validate(allPods, reachability, []int32{80}, ProtocolTCP, false)
	...
	reachability2 := NewReachability(allPods, Connected)
	k8sUtils.Validate(allPods, reachability2, []int32{80}, ProtocolTCP, false)
...
func testANNPGroupRefRuleIPBlocks(t *testing.T) {
	...
	builder := &AntreaNetworkPolicySpecBuilder{}
	builder = builder.SetName(namespaces["x"], "annp-deny-xb-xc-ips-ingress-for-xa").
		SetPriority(1.0).
		SetAppliedToGroup([]ANNPAppliedToSpec{{PodSelector: map[string]string{"pod": "a"}}})
	builder.AddIngress(ProtocolTCP, &p80, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil,
		nil, nil, nil, nil, crdv1beta1.RuleActionDrop, grpName, "")
...
func testRejectServiceTraffic(t *testing.T, data *TestData, clientNamespace, serverNamespace string) {
	...
	for _, tc := range testcases {
		...
		connectivity, err := k8sUtils.ProbeAddr(tc.clientPod.Namespace(), "antrea-e2e", tc.clientPod.PodName(), tc.destAddr, tc.destPort, ProtocolTCP, &tc.expectedConnectivity, false)
		if err != nil {
...
func testANNPPortRange(t *testing.T) {
	builder := &AntreaNetworkPolicySpecBuilder{}
	builder = builder.SetName(namespaces["y"], "annp-deny-yb-to-xc-egress-port-range").
		SetPriority(1.0).
...
func testAuditLoggingBasic(t *testing.T, data *TestData) {
	...
	oneProbe := func(ns1, pod1, ns2, pod2 string) {
		...
		go func() {
			defer wg.Done()
			k8sUtils.Probe(ns1, pod1, ns2, pod2, p80, ProtocolTCP, nil, nil, false)
		}()
	}
...
func testAuditLoggingK8sService(t *testing.T, data *TestData) {
	...
	oneProbe := func(ns, pod string, matcher *auditLogMatcher) {
		for _, ip := range serverIP.ipStrings {
			...
			go func() {
				defer wg.Done()
				k8sUtils.ProbeAddr(ns, "pod", pod, ip, serverPort, ProtocolTCP, nil,false)
			}()
		}
	}
...
func testFQDNPolicy(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		connectivity, err := k8sUtils.ProbeAddr(tc.clientPod.Namespace(), "pod", tc.clientPod.PodName(), tc.destAddr, tc.destPort, ProtocolTCP, &tc.expectedConnectivity, false)
		if err != nil {
			t.Errorf("Failure -- could not complete probe: %v", err)
		}
		if connectivity != tc.expectedConnectivity {
...
func testFQDNPolicyTCP(t *testing.T) {
	...
	for _, tc := range testcases {
		...
		connectivity, err := k8sUtils.ProbeAddr(tc.clientPod.Namespace(), "pod", tc.clientPod.PodName(), destIP, tc.destPort, ProtocolTCP, &tc.expectedConnectivity, false)
		if err != nil {
			t.Errorf("Failure -- could not complete probe: %v", err)
		}
		if connectivity != tc.expectedConnectivity {
...
func testACNPNodeSelectorIngress(t *testing.T, data *TestData) {
	...
	for _, tc := range testcases {
		log.Tracef("Probing: %s -> %s", tc.clientPod.PodName(), tc.destAddr)
		connectivity, err := k8sUtils.ProbeAddr(tc.clientPod.Namespace(), "antrea-e2e", tc.clientPod.PodName(), tc.destAddr, tc.destPort, ProtocolTCP, &tc.expectedConnectivity, true)
		if err != nil {
			t.Errorf("Failure -- could not complete probe: %v", err)
		}
		if connectivity != tc.expectedConnectivity {
...
func testACNPICMPSupport(t *testing.T, data *TestData) {
	...
	for _, tc := range testcases {
		log.Tracef("Probing: %s -> %s", tc.clientPod.PodName(), tc.destAddr)
		connectivity, err := k8sUtils.ProbeAddr(tc.clientPod.Namespace(), "antrea-e2e", tc.clientPod.PodName(), tc.destAddr, tc.destPort, ProtocolICMP, &tc.expectedConnectivity, false)
		if err != nil {
			t.Errorf("Failure -- could not complete probe: %v", err)
		}
		if connectivity != tc.expectedConnectivity {
...
func testACNPIGMPQuery(t *testing.T, data *TestData, acnpName, caseName, groupAddress string, action crdv1beta1.RuleAction, testNamespace string) {
	...
	tcpdumpName, _, cleanupFunc := createAndWaitForPod(t, data, data.createNetshootPodOnNode, "test-tcpdump-", nodeName(mc.receiverConfigs[0].nodeIdx), data.testNamespace, true)
	defer cleanupFunc()
...
func executeTests(t *testing.T, testList []*TestCase, flag bool) {
	executeTestsWithData(t, testList, nil, flag)
}

func executeTestsWithData(t *testing.T, testList []*TestCase, data *TestData, flag bool) {
	for _, testCase := range testList {
		...
		for _, step := range testCase.Steps {
			...
			if reachability != nil {
				start := time.Now()
				k8sUtils.Validate(allPods, reachability, step.Ports, step.Protocol, flag)
...
func doProbe(t *testing.T, data *TestData, p *CustomProbe, protocol AntreaPolicyProtocol) {
	...
	connectivity, err := k8sUtils.Probe(p.SourcePod.Pod.Namespace(), p.SourcePod.Pod.PodName(), p.DestPod.Pod.Namespace(), p.DestPod.Pod.PodName(), p.Port, protocol, nil, &p.ExpectConnectivity, false)
	if err != nil {
		t.Errorf("Failure -- could not complete probe: %v", err)
	}
...
// testANNPNetworkPolicyStatsWithDropAction tests antreanetworkpolicystats can correctly collect dropped packets stats from ANNP if
