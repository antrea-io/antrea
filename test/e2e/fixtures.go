...
func setupFlowAggregator(tb testing.TB, testData *TestData, o flowVisibilityTestOptions) error {
	...
	if o.ipfixCollector.tls {
		var err error
		if ipfixServerCert, ipfixServerKey, err = GenerateCert([]string{"ipfix-collector"}, time.Unix(0, 0), 100*365*24*time.Hour, true, false, 0, "P256", false); err != nil {
			return fmt.Errorf("failed to generate server certificate for ipfix-collector: %w", err)
		}
		if o.ipfixCollector.clientAuth {
			var err error
			if ipfixClientCert, ipfixClientKey, err = GenerateCert([]string{"ipfix-collector"}, time.Unix(0, 0), 100*365*24*time.Hour, true, true, 0, "P256", false); err != nil {
