...
func newTestRouteClient(networkConfig *config.NetworkConfig, options routeClientOptions) (*route.Client, error) {
	return route.NewClient(networkConfig,
		...
		route.NewHostNetworkPortRules().
			Allow(apis.AntreaAgentAPIPort, route.FeatureAgentAPIServer).
			Allow(apis.AntreaAgentClusterMembershipPort, route.FeatureAgentClusterMembership).
			Allow(apis.WireGuardListenPort, route.FeatureWireguard).
			Allow(apis.AntreaProxyHealthServerPort, route.FeatureProxyHealthCheck),
	)
}
...
-A ANTREA-OUTPUT -s 127.0.0.0/8 -d 127.0.0.0/8 -o lo -p tcp -m comment --comment "Antrea: do not track localhost API health check request packets" -m multiport --dports 10349,10350 -j NOTRACK
...
`, tc.expectUDPPortInRules)
			...
			if tc.networkConfig.TrafficEncryptionMode == config.TrafficEncryptionModeIPSec {
				expectedIPTables["filter"] += `-A ANTREA-INPUT -p esp -m comment --comment "Antrea: allow IPsec ESP input packets" -j ACCEPT
-A ANTREA-INPUT -p ah -m comment --comment "Antrea: allow IPsec AH input packets" -j ACCEPT
-A ANTREA-INPUT -p udp -m comment --comment "Antrea: allow IPsec IKE input packets" -m udp --dport 500 -j ACCEPT
-A ANTREA-INPUT -p udp -m comment --comment "Antrea: allow IPsec NAT traversal input packets" -m udp --dport 4500 -j ACCEPT
`
			}
			if tc.proxyAll {
...
-A ANTREA-INPUT -s 127.0.0.0/8 -d 127.0.0.0/8 -i lo -p tcp -m comment --comment "Antrea: allow localhost API health check request input packets" -m multiport --dports 10349,10350 -j ACCEPT
-A ANTREA-INPUT -s 127.0.0.0/8 -d 127.0.0.0/8 -i lo -p tcp -m comment --comment "Antrea: allow localhost API health check reply input packets" -m multiport --sports 10349,10350 -j ACCEPT
`
			if tc.networkConfig.TrafficEncapMode.SupportsEncap() {
...
`, tc.expectUDPPortInRules)
			...
			if tc.networkConfig.TrafficEncryptionMode == config.TrafficEncryptionModeIPSec {
				expectedIPTables["filter"] += `-A ANTREA-OUTPUT -p esp -m comment --comment "Antrea: allow IPsec ESP output packets" -j ACCEPT
-A ANTREA-OUTPUT -p ah -m comment --comment "Antrea: allow IPsec AH output packets" -j ACCEPT
-A ANTREA-OUTPUT -p udp -m comment --comment "Antrea: allow IPsec IKE output packets" -m udp --dport 500 -j ACCEPT
-A ANTREA-OUTPUT -p udp -m comment --comment "Antrea: allow IPsec NAT traversal output packets" -m udp --dport 4500 -j ACCEPT
`
			}
			if tc.proxyAll {
				expectedIPTables["filter"] += `-A ANTREA-OUTPUT -p tcp -m comment --comment "Antrea: allow proxy health check reply packets" -m tcp --sport 10256 -m conntrack --ctstate ESTABLISHED -j ACCEPT
`
			}
...
-A ANTREA-OUTPUT -s 127.0.0.0/8 -d 127.0.0.0/8 -o lo -p tcp -m comment --comment "Antrea: allow localhost API health check request output packets" -m multiport --dports 10349,10350 -j ACCEPT
-A ANTREA-OUTPUT -s 127.0.0.0/8 -d 127.0.0.0/8 -o lo -p tcp -m comment --comment "Antrea: allow localhost API health check reply output packets" -m multiport --sports 10349,10350 -j ACCEPT
`
...
func TestIpTablesSync(t *testing.T) {
	...
	tcs := []struct {
		...
		{Table: "filter", Cmd: "-A", Chain: "ANTREA-INPUT", RuleSpec: "-p tcp -m comment --comment \"Antrea: allow Agent cluster memberships TCP input packets\" -m tcp --dport 10351 -j ACCEPT"},
		{Table: "filter", Cmd: "-A", Chain: "ANTREA-INPUT", RuleSpec: "-p tcp -m comment --comment \"Antrea: allow Agent cluster memberships TCP reply input packets\" -m tcp --sport 10351 -m conntrack --ctstate ESTABLISHED -j ACCEPT"},
		...
		{Table: "filter", Cmd: "-A", Chain: "ANTREA-OUTPUT", RuleSpec: "-p tcp -m comment --comment \"Antrea: allow Agent cluster memberships TCP output packets\" -m tcp --dport 10351 -j ACCEPT"},
		{Table: "filter", Cmd: "-A", Chain: "ANTREA-OUTPUT", RuleSpec: "-p tcp -m comment --comment \"Antrea: allow Agent cluster memberships TCP reply packets\" -m tcp --sport 10351 -m conntrack --ctstate ESTABLISHED -j ACCEPT"},
