...
func Update(ctx context.Context, kClient kubernetes.Interface, ns, clientDaemonSetName string, probes []string, containerName string, desiredNum int) (clientPods []corev1.Pod, err error) {
	err = retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		if _, err = kClient.AppsV1().DaemonSets(ns).Update(context.TODO(), daemonSet, metav1.UpdateOptions{}); err != nil {
			return err
		}
		if err = kClient.CoreV1().Pods(ns).DeleteCollection(ctx, metav1.DeleteOptions{}, metav1.ListOptions{LabelSelector: ScaleClientPodTemplateName}); err != nil {
			return err
		}
		return nil
	})
	...
	if err := wait.PollUntilContextTimeout(ctx, config.WaitInterval, 10*time.Minute, true, func(ctx context.Context) (bool, error) {
		...
		for i := range podList.Items {
			pod := podList.Items[i]
			if pod.DeletionTimestamp != nil || !podutils.IsPodReady(&pod) || pod.Status.Phase != corev1.PodRunning {
				return false, nil
			}
		}
