		...
		for i := range services {
			...
			gErr.Go(func() error {
				...
				if err := utils.DefaultRetry(func() error {
					...
					clientPod, err = clientpod.CreatePod(ctx, cs, []string{fmt.Sprintf("%s:%d", clusterIP, 80)}, clientpod.ScaleTestPodProbeContainerName, ns)
					...
					go func() {
						...
						key := utils.DownToUp
						if err := utils.FetchTimestampFromLog(ctx, cs, clientPod.Namespace, clientPod.Name, clientpod.ScaleTestPodProbeContainerName, ch, startTimeStamp, key); err != nil {
