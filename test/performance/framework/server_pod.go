...
func renderServerPods(templatePath string, ns string, num, serviceNum int) (serverPods []*corev1.Pod, err error) {
	...
	for i := 0; i < num; i++ {
		...
		serverPod.Labels = map[string]string{
			...
			clientpod.AppLabelKey:       clientpod.AppLabelValue,
		...
		serverPod.Spec.Affinity = &clientpod.RealNodeAffinity
	...
	for _, ns := range data.namespaces {
		...
		err = wait.PollUntilContextCancel(ctx, config.WaitInterval, true, func(ctx context.Context) (bool, error) {
			podsResult, err := data.kubernetesClientSet.
				CoreV1().Pods(ns).
				List(ctx, metav1.ListOptions{LabelSelector: fmt.Sprintf("%s=%s", clientpod.AppLabelKey, clientpod.AppLabelValue)})
			if err != nil {
