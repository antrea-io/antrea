...
func ScaleNetworkPolicy(ctx context.Context, data *ScaleData) error {
	...
	nps, err := networkpolicy.ScaleUp(ctx, npNum, data.kubernetesClientSet, ScaleTestNamespacePrefix, data.Specification.IPv6)
	if err != nil {
	...
	for _, np := range nps {
		fromPod, ip, err := networkpolicy.SelectConnectPod(ctx, data.kubernetesClientSet, ScaleTestNamespacePrefix, np)
	...
	for _, np := range nps {
		fromPod, ip, err := networkpolicy.SelectIsoPod(ctx, data.kubernetesClientSet, ScaleTestNamespacePrefix, np, data.clientPods)
		if err != nil || fromPod == nil || ip == "" {
			continue
		}
		if err := retryWithRateLimiter(ctx, rateLimiter, func() error {
	...
	if err := networkpolicy.ScaleDown(ctx, []string{ScaleTestNamespacePrefix}, data.kubernetesClientSet); err != nil {
		return err
	}
	return nil
}
