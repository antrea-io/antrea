...
function run_e2e {
    ...
    if [[ $FLEXIBLE_IPAM == true ]]; then
        go test -v antrea.io/antrea/test/e2e --logs-export-dir `pwd`/antrea-test-logs --provider remote -timeout=100m --prometheus --antrea-ipam -run=TestIPSec
    else
        go test -v antrea.io/antrea/test/e2e --logs-export-dir `pwd`/antrea-test-logs --provider remote -timeout=100m --prometheus -run=TestIPSec
    fi
    if [[ "$?" != "0" ]]; then
        TEST_FAILURE=true
    fi
