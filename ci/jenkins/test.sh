...
function run_e2e {
    ...
    if [[ $TESTBED_TYPE == "flexible-ipam" ]]; then
        go test -v antrea.io/antrea/test/e2e -run=TestMulticast --logs-export-dir `pwd`/antrea-test-logs --provider remote -timeout=100m --prometheus --antrea-ipam
    else
        go test -v antrea.io/antrea/test/e2e -run=TestMulticast --logs-export-dir `pwd`/antrea-test-logs --provider remote -timeout=100m --prometheus
    fi
    if [[ "$?" != "0" ]]; then
        TEST_FAILURE=true
    fi
