    ...
    kubectl get nodes -o wide --no-headers=true | awk -v role="$CONTROL_PLANE_NODE_ROLE" '$3 !~ role && $1 ~ /win/ {print $1}' | while read WORKER_NAME; do
        ...
        if [ "$TESTCASE" == "windows-networkpolicy-process" ]; then
            ...
            if ! (test -f antrea-windows.tar.gz); then
                ...
                ssh -o StrictHostKeyChecking=no -n Administrator@${IP} "rm -rf antrea && mkdir antrea && cd antrea && tar -xzf ../antrea_repo.tar.gz > /dev/null && NO_PULL=${NO_PULL}; DOCKER_NETWORK=host make build-windows && docker save -o antrea-windows.tar ${DOCKER_REGISTRY}/antrea/antrea-windows:latest && gzip -f antrea-windows.tar" || true
                for i in `seq 2`; do
...
function run_e2e {
    ...
    if [[ $TESTBED_TYPE == "flexible-ipam" ]]; then
        go test -v -run TestAntreaPolicy/TestMulticastN antrea.io/antrea/test/e2e --logs-export-dir `pwd`/antrea-test-logs --provider remote -timeout=100m --prometheus --antrea-ipam
    else
        go test -v -run TestAntreaPolicy/TestMulticastN antrea.io/antrea/test/e2e --logs-export-dir `pwd`/antrea-test-logs --provider remote -timeout=100m --prometheus
    fi
    if [[ "$?" != "0" ]]; then
        TEST_FAILURE=true
    fi
