...
function  build_and_deliver_antrea_windows_and_linux_images {
    ...
    curl -OL http://build-squid.vcfd.broadcom.net/build/mts/release/sb-81769839/publish/windows-advanced/manifests/${WINDOWS_YAML_NAME}.yml
...
function deliver_antrea_windows {
    ...
    curl -OL http://build-squid.vcfd.broadcom.net/build/mts/release/sb-81769839/publish/windows-advanced/images/antrea-advanced-windows-v2.2.0_vmware.1.tar.gz
    mv antrea-advanced-windows-v2.2.0_vmware.1.tar.gz antrea-windows.tar.gz
...
if [[ ${TESTCASE} =~ "windows" ]]; then
    if [[ ${TESTCASE} == "windows-e2e-ovs-as-service" ]]; then
        WINDOWS_YAML_NAME="antrea-windows-v2.2.0+vmware.1"
    else
        WINDOWS_YAML_NAME="antrea-windows-v2.2.0+vmware.1"
    fi
