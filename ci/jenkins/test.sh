...
function run_conformance_windows {
    ...
    if [ "$TESTCASE" == "windows-networkpolicy" ]; then
        ginkgo --seed=1592804472 --noColor $E2ETEST_PATH -- --provider=skeleton --ginkgo.focus="$WINDOWS_NETWORKPOLICY_FOCUS" --ginkgo.skip="$WINDOWS_NETWORKPOLICY_SKIP" > windows_conformance_result_no_color.txt || true
    else
