...
EOF
    ...
    AGENT_IMG_NAME="$ANTREA_AGENT_CNI_HARBOR_IMAGE" CONTROLLER_IMG_NAME="$ANTREA_CONTROLLER_CNI_HARBOR_IMAGE" bash $WORKSPACE/antrea-cni/hack/generate-manifest.sh --hw-offload > $ARTIFACTS/antrea.yml
    let status=status+$?
    if [ "$status" != 0 ]; then
