...
Scale a jenkins agent to run CI tests.
        ...
        --gke-subnet             The GKE subnetwork to be used.
...
function check_jobs_in_queue {
    ...
    for i in {1..2}; do
        ...
        echo $JENKINS_URL
        echo $QUEUE_JOBS
        JOB_FOUND=$(echo "$QUEUE_JOBS" | jq -r '.items[] | select(.task.labels != null) | .task.labels[]' | grep -w "$LABEL" || true)
...
function add_agent {
    ...
    JOIN_COMMAND="java -jar $SWARM_CLIENT_JAR -master $JENKINS_URL -username $JENKINS_USER -password $JENKINS_API_TOKEN \
        -name $NEW_AGENT_NAME -labels $LABEL -executors 1 -retry 3 -mode exclusive -disableSslVerification -deleteExistingClients -workDir $WORKDIR -webSocket &"

    # Run from agent command line
    set +e
    gcloud compute ssh ubuntu@"$NEW_AGENT_NAME" --internal-ip --zone="$ZONE" --command \
        "wget https://repo.jenkins-ci.org/releases/org/jenkins-ci/plugins/swarm-client/3.43/swarm-client-3.43.jar -O swarm-client.jar && sudo apt update && sudo apt-get install openjdk-17-jdk -y && sudo mkdir -p $WORKDIR && sudo chown ubuntu:ubuntu $WORKDIR && $JOIN_COMMAND"
    rc=$?
    set -e
    if [ "$rc" != 0 ]; then
      sudo gcloud compute instances delete "$NEW_AGENT_NAME" --zone="$ZONE" --quiet
