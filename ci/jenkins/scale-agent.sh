...
function add_agent {
    ...
    gcloud compute ssh ubuntu@"$NEW_AGENT_NAME" --internal-ip --zone="$ZONE" --command \
        "wget https://repo.jenkins-ci.org/releases/org/jenkins-ci/plugins/swarm-client/3.43/swarm-client-3.43.jar -O swarm-client.jar && sudo apt-get install openjdk-11-jdk -y && sudo mkdir -p $WORKDIR && sudo chown ubuntu:ubuntu $WORKDIR && $JOIN_COMMAND"
    rc=$?
    set -e
    if [ "$rc" != 0 ]; then
      sudo gcloud compute instances delete "$NEW_AGENT_NAME" --zone="$ZONE" --quiet
