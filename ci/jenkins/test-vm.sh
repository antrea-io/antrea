...
echo "Debug: current workdir $(pwd)"
...
function apply_antrea {
    ...
    ./hack/generate-manifest.sh --enable-nodeport > ${WORKDIR}/antrea.yml
    ...
    kubectl get nodes -o wide --no-headers=true | awk -v role="$CONTROL_PLANE_NODE_ROLE" '$3 != role {print $6}' | while read IP; do
        echo "Skip copying new antrea.yml to ${CONTROL_PLANE_NODE_ROLE} node ${IP} home directory"
        #scp -q -o StrictHostKeyChecking=no -i "${WORKDIR}/jenkins_id_rsa" ${WORKDIR}/antrea.yml ubuntu@${IP}:~
    done
}
...
function clean_vm_agent {
    ...
    kubectl get nodes -o wide --no-headers=true | awk -v role="$CONTROL_PLANE_NODE_ROLE" '$3 != role {print $6}' | while read IP; do
        echo "Do cleanup on the control plane ${IP}"
    done
...
function configure_vm_agent {
    ...
    cp ./build/yamls/externalnode/vm-agent-rbac.yaml ${WORKDIR}/vm-agent-rbac.yaml
    echo "Applying vm-agent rbac yaml"
    kubectl apply -f ${WORKDIR}/vm-agent-rbac.yaml
...
function run_e2e_vms {
    ...
    go test -v "./test/e2e" -run=TestVMAgent --logs-export-dir `pwd`/antrea-test-logs -provider=remote -winVMs=${WIN_HOSTNAME} -linVMs=${LIN_HOSTNAME}
    if [[ "$?" != "0" ]]; then
        TEST_FAILURE=true
    fi
    set -e
...
function deliver_antrea_vm {
    ...
    sed -i 's|#namespace: default|namespace: test-ns|g' ${WORKDIR}/antrea-agent.conf
    sed -i 's|kubeconfig: |kubeconfig: /tmp/|g' ${WORKDIR}/antrea-agent.conf
    echo "Copying binaries and conf to linux VM: $UBUNTU_VM_IP"
    scp -q -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i "${WORKDIR}/jenkins_id_rsa" ./bin/antrea-agent ubuntu@${UBUNTU_VM_IP}:/tmp/antrea-agent
    #scp -q -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i "${WORKDIR}/jenkins_id_rsa" ./bin/antctl-linux ubuntu@${UBUNTU_VM_IP}:/tmp/antctl
    scp -q -o StrictHostKeyChecking=no -i "${WORKDIR}/jenkins_id_rsa" ${WORKDIR}/antrea-agent.conf  ubuntu@${UBUNTU_VM_IP}:/tmp/antrea-agent.conf
    echo "Copying binaries and conf to windows VM: $WINDOWS_VM_IP"
    scp -q -o StrictHostKeyChecking=no -i "${WORKDIR}/jenkins_id_rsa" ./bin/antrea-agent.exe Administrator@${WINDOWS_VM_IP}:/tmp/antrea-agent.exe
    #scp -q -o StrictHostKeyChecking=no -i "${WORKDIR}/jenkins_id_rsa" ./bin/antctl-windows.exe Administrator@${WINDOWS_VM_IP}:/tmp/antctl.exe
    scp -q -o StrictHostKeyChecking=no -i "${WORKDIR}/jenkins_id_rsa" ${WORKDIR}/antrea-agent.conf  Administrator@${WINDOWS_VM_IP}:/tmp/antrea-agent.conf
}
