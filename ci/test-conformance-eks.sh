...
if ! [[ "${GC_CLUSTER_AGE_HOURS}" =~ ^[0-9]+$ ]] || [[ "${GC_CLUSTER_AGE_HOURS}" -lt 1 ]]; then
    echoerr "--gc-cluster-age-hours must be a positive integer (got: '${GC_CLUSTER_AGE_HOURS}')"
    exit 1
fi
...
function clusters_gc() {
    ...
    for cluster in $clusters; do
        ...
        creation_time_raw=$(aws eks describe-cluster --region "${REGION}" --name "${cluster}" --query 'cluster.createdAt' --output text)
        ...
        if ! creation_epoch=$(date -d "$creation_time_raw" +%s 2>/dev/null); then
            echo "WARNING: Could not parse creation time for cluster $cluster (raw value: $creation_time_raw)"
            failed_count=$((failed_count + 1))
            continue
        fi

        cluster_age_seconds=$((CURRENT_TIME - creation_epoch))
        cluster_age_hours=$((cluster_age_seconds / 3600))

        if [ $cluster_age_seconds -gt $GC_CLUSTER_AGE_SECONDS ]; then
            echo "Found old cluster: $cluster (age: ${cluster_age_hours}h, created: $creation_time_raw)"
    ...
    echo "$stacks" | jq -c '.[]' | while read -r stack_info; do
        ...
        if ! stack_creation_epoch=$(date -d "$stack_creation_time" +%s 2>/dev/null); then
            echo "WARNING: Could not parse creation time for stack $stack_name (raw value: $stack_creation_time)"
            continue
        fi
        ...
        if [ $stack_age_seconds -gt $GC_CLUSTER_AGE_SECONDS ]; then
            ...
            while [[ "${retry}" -gt 0 ]]; do
                ...
                echo "Failed to delete stack $stack_name, retrying... (${retry} attempts left)"
                sleep 10
                retry=$((retry - 1))
            done
            if [[ "${retry}" -eq 0 ]]; then
                echo "ERROR: Failed to delete CloudFormation stack ${stack_name} after multiple attempts"
            fi
        fi
    done
