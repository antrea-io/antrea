...
EOF
  ...
  if ! kind create cluster --name $cluster_name --config $config_file $IMAGE_OPT; then
    echoerr -e "attmped to create kind cluster with name $cluster_name, image options $IMAGE_OPT, config file:"
    cat $config_file
    exit 1
  fi
