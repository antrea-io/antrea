...
ANTREA_NODE_CONFIG_NO_UPLINK_YAML=$THIS_DIR"/../../test/e2e-secondary-network/infra/antrea-node-config-no-uplink.yml"
...
function configure_extra_network_bridges {
  local cluster_name="${KIND_CLUSTER_NAME:-kind}"
  local i=0
  local network br

  for _ in $EXTRA_NETWORKS; do
    network="antrea-${cluster_name}-${i}"
    i=$((i+1))
    if ! docker network inspect "$network" >/dev/null 2>&1; then
      echoerr "Skipping Linux bridge configuration for missing Docker network $network"
      continue
    fi

    br="$(docker_bridge_for_network "$network")"
    # Extra Docker networks are created for secondary-network data traffic with IPv4
    # subnets only, so Docker does not install ip6tables FORWARD rules for their
    # Linux bridges. When bridge-nf-call-ip6tables is enabled on the host, IPv6
    # packets bridged between Kind Nodes, including ICMPv6 Neighbor Discovery, hit
    # the host ip6tables FORWARD chain and can be dropped by the default policy.
    # Add the missing rule explicitly and keep it idempotent for --test-only reruns.
    echo "Allowing IPv6 forwarding from Linux bridge $br for Docker network $network"
    sudo ip6tables -C FORWARD -i "$br" -j ACCEPT 2>/dev/null || \
...
function set_secondary_network_static_bridge_config {
  local ovs_bridges_json="$1"

  # AntreaNodeConfig is intentionally used only when the static bridge
  # configuration is empty: static agent configuration has higher precedence.
  # Updating the Helm value rolls antrea-agent Pods, so wait for the new config
  # to be observed before applying or removing an AntreaNodeConfig.
  helm upgrade antrea "$ANTREA_CHART" --namespace kube-system --reuse-values \
    --set-json secondaryNetwork.ovsBridges="$ovs_bridges_json"
  kubectl rollout status --timeout=5m daemonset.apps/antrea-agent -n kube-system
}

function set_antrea_node_config_feature_gate {
  local enabled="$1"

  helm upgrade antrea "$ANTREA_CHART" --namespace kube-system --reuse-values \
    --set featureGates.AntreaNodeConfig="$enabled"
  kubectl rollout status --timeout=5m daemonset.apps/antrea-agent -n kube-system
}
...
function run_test_without_antrea_node_config {
  echo "Disabling the AntreaNodeConfig feature gate and verifying the static secondary bridge"
  set_antrea_node_config_feature_gate false
  go test -v -timeout="$TIMEOUT" antrea.io/antrea/v2/test/e2e-secondary-network \
    -run='^(TestSecondaryOVSBridgeName|TestVLANNetwork)$' -expected-secondary-bridge-name=br-secondary -provider=kind "${TEST_OPTIONS[@]}"

  echo "Re-enabling the AntreaNodeConfig feature gate for AntreaNodeConfig tests"
  set_antrea_node_config_feature_gate true
}

function run_test_with_antreanodeconfigs {
  ...
  echo "Verifying static bridge configuration takes precedence over AntreaNodeConfig and traffic still works"
  go test -v -timeout="$TIMEOUT" antrea.io/antrea/v2/test/e2e-secondary-network \
    -run='^(TestSecondaryOVSBridgeName|TestVLANNetwork)$' -expected-secondary-bridge-name=br-secondary -provider=kind "${TEST_OPTIONS[@]}"

  echo "Removing the static secondary bridge configuration so AntreaNodeConfig takes effect"
  set_secondary_network_static_bridge_config '[]'
  echo "Verifying AntreaNodeConfig changes the secondary bridge to br1 and VLAN traffic with the AntreaNodeConfig bridge"
  go test -v -timeout="$TIMEOUT" antrea.io/antrea/v2/test/e2e-secondary-network \
    -run='^(TestSecondaryOVSBridgeName|TestVLANNetwork)$' -expected-secondary-bridge-name=br1 -provider=kind "${TEST_OPTIONS[@]}"
  ...
  sleep 10
  go test -v -timeout="$TIMEOUT" antrea.io/antrea/v2/test/e2e-secondary-network -run='^TestSecondaryOVSBridgeMulticastSnooping$' -provider=kind "${TEST_OPTIONS[@]}"

  echo "Clean up the previous AntreaNodeConfig"
  kubectl delete -f "$ANTREA_NODE_CONFIGS_LINUX_YAML"
  sleep 10

  echo "Restoring and verifying the static bridge configuration after AntreaNodeConfig deletion"
  set_secondary_network_static_bridge_config "$SECONDARY_OVS_BRIDGES_JSON"
  go test -v -timeout="$TIMEOUT" antrea.io/antrea/v2/test/e2e-secondary-network -run='^TestVLANNetwork$' -provider=kind "${TEST_OPTIONS[@]}"

  echo "Removing the static secondary bridge configuration for further AntreaNodeConfig tests"
  set_secondary_network_static_bridge_config '[]'
  echo "Applying AntreaNodeConfig for a secondary OVS bridge without an uplink"
  kubectl apply -f "$ANTREA_NODE_CONFIG_NO_UPLINK_YAML"
  sleep 10
  go test -v -timeout="$TIMEOUT" antrea.io/antrea/v2/test/e2e-secondary-network \
    -run='^TestVLANNetworkNoUplink$' -provider=kind "${TEST_OPTIONS[@]}"

  echo "Cleaning up the AntreaNodeConfig without an uplink"
  kubectl delete -f "$ANTREA_NODE_CONFIG_NO_UPLINK_YAML"
  sleep 10
...
configure_extra_network_bridges
