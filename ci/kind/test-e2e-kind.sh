...
function run_test {
  ...
  go test -v -timeout=$timeout $RUN_OPT antrea.io/antrea/test/e2e $flow_visibility_args -provider=kind --logs-export-dir=$ANTREA_LOG_DIR --skip-cases=$skiplist --networkpolicy-evaluation=$np_evaluation $coverage_args $EXTRA_ARGS
}
