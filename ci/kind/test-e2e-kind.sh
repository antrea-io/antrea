...
function run_test {
  ...
  if $flow_visibility; then
      ...
      # clusterScopedNameSuffix must be unique for each one: without it, all these manifests
      ...
      $FLOWAGGREGATOR_YML_CMD "${flow_visibility_manifest_default_args[@]}" --extra-helm-values 'clusterScopedNameSuffix=-1' -n 'flow-aggregator-1' ${coverage_flag} | docker exec -i kind-control-plane dd of=/root/flow-aggregator-1.yml
      $FLOWAGGREGATOR_YML_CMD "${flow_visibility_manifest_default_args[@]}" --extra-helm-values 'clusterScopedNameSuffix=-2' -n 'flow-aggregator-2' ${coverage_flag} | docker exec -i kind-control-plane dd of=/root/flow-aggregator-2.yml
