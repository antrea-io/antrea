...
function run_test {
  ...
  sleep 1
  if $coverage; then
      go test -v -run=TestFlowAggregator -count=1 antrea.io/antrea/test/e2e -provider=kind --logs-export-dir=$ANTREA_LOG_DIR --coverage --coverage-dir $ANTREA_COV_DIR
    ...
    #   go test -v -timeout=65m antrea.io/antrea/test/e2e -provider=kind --logs-export-dir=$ANTREA_LOG_DIR
      go test -v -run=TestFlowAggregator -count=1 antrea.io/antrea/test/e2e -provider=kind --logs-export-dir=$ANTREA_LOG_DIR
  fi
  $TESTBED_CMD destroy kind
}
