...
function run_test {
  ...
  sleep 1
  if $coverage; then
      go test -v -timeout=60m github.com/vmware-tanzu/antrea/test/e2e -provider=kind --logs-export-dir=$ANTREA_LOG_DIR --coverage --coverage-dir $ANTREA_COV_DIR
  else
      go test -v -timeout=55m github.com/vmware-tanzu/antrea/test/e2e -provider=kind --logs-export-dir=$ANTREA_LOG_DIR
  fi
  $TESTBED_CMD destroy kind
}
