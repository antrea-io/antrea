...
function run_test {
  ...
  sleep 1
  if $coverage; then
      go test -v -timeout=70m github.com/vmware-tanzu/antrea/test/e2e -provider=kind --short --logs-export-dir=$ANTREA_LOG_DIR --coverage --coverage-dir $ANTREA_COV_DIR
  else
      go test -v -timeout=65m github.com/vmware-tanzu/antrea/test/e2e -provider=kind --short --logs-export-dir=$ANTREA_LOG_DIR
  fi
  $TESTBED_CMD destroy kind
}
