...
Clients must present a Kubernetes credential in gRPC metadata, as one of:

- a bearer token, as `authorization: Bearer <token>`, which the Flow Aggregator
  validates with a `TokenReview`. `kubectl create token <serviceaccount>` and, in
  a cluster whose kube-apiserver trusts an OIDC issuer, an OIDC ID token both
  work.
- a PEM client certificate and key, in the `client-cert-bin` and `client-key-bin`
  headers, which the Flow Aggregator validates with a `SelfSubjectReview` — the
  real API server does the certificate verification. This covers the clients
  whose only credential is a short-lived client certificate, such as one issued
  by a Pinniped Concierge.
