...
- Two Alpha feature gates `TrafficControl` and `L7NetworkPolicy` are enabled.
- Default settings are maintained for other feature gates and options.
...
- *Node Route Controller*: the [Kubernetes controller](https://kubernetes.io/docs/concepts/architecture/controller/)
  which is part of the Antrea Agent and watches for updates to Nodes. When a Node is added, it updates the local networking
  configuration (e.g. configure the tunnel to the new Node). When a Node is deleted, it performs the necessary clean-ups.
...
- *Service session affinity*: a Service feature that selects the same backend Pod for connections from a
  particular client. For a Kubernetes Service, session affinity can be enabled by setting `service.spec.sessionAffinity`
  to `ClientIP` (default is `None`). See [Kubernetes Service](https://kubernetes.io/docs/concepts/services-networking/service/)
...
- *action `IN_PORT`*: an action to output packets to the port on which they were received. This is the only standard way
  to output the packet to the input port.
...
- *ct zone*: a zone is to isolate connection tracking rules stored in the field `ct_zone` of OVS conntrack. It is
  conceptually similar to the more generic Linux network namespace, but ct zone is specific to conntrack and has less
  overhead. Explore all the ct zones used in the pipeline in the [Ct Zones] section.
...
- *Global Virtual MAC*: a virtual MAC address that is used as the destination MAC for all tunneled traffic across all
  ...
  the appropriate remote gateway. This enables each OVS to act as a "proxy" for the local gateway when receiving
...
# Dump all groups.
kubectl exec -n kube-system <ANTREA_AGENT_POD_NAME> -c antrea-ovs -- ovs-ofctl dump-groups <BRIDGE_NAME> -O Openflow15 [--no-stats] [--names]
```
...
You can also dump the flows of a table or a group on a given Node as follows:
...
# Dump a group.
kubectl exec -n kube-system <ANTREA_AGENT_POD_NAME> -c antrea-ovs -- ovs-ofctl dump-groups <BRIDGE_NAME> <GROUP_ID> -O Openflow15 [--no-stats] [--names]
```
...
|               |             |                                 | 0x2            | FromGatewayRegMark              | Packet source is Antrea gateway port.                                                                |
|               |             |                                 | 0x3            | FromLocalRegMark                | Packet source is local Pod port.                                                                     |
...
NetworkPolicy](https://kubernetes.io/docs/concepts/services-networking/network-policies/) implementation (table
[EgressRule], [EgressDefault], [IngressRule], and [IngressDefault]).

In this document, the Kubernetes NetworkPolicy example below is used to explain how these simple ingress and egress
rules are mapped to flows in the pipeline.

This Kubernetes NetworkPolicy is applied to Pods with the label `app: web` in the `default` Namespace. For these Pods,
only TCP traffic on port 80 from Pods with the label `app: client` and to Pods with the label `app: db` is allowed.
Because Antrea will only install OVS flows for this Kubernetes NetworkPolicy on Nodes that have Pods selected by the
policy, we have scheduled an `app: web` Pod on the current Node. It received an IP address `10.10.0.19` from the Antrea
CNI, so you will see the IP address shown in the associated OVS flows.
...
ClusterIP is supported. In this document, we use the sample Kubernetes Services below. These Services select Pods with
the label `app: web` as Endpoints.
...
A sample ClusterIP Service with `clusterIP` set to `10.96.76.15` configured with session affinity.

```yaml
apiVersion: v1
kind: Service
metadata:
  name: sample-clusterip-session-affinity
spec:
  selector:
    app: web
  ports:
...
A sample NodePort Service with a `nodePort` set to `30004`.
...
A sample ClusterIP Service with a `nodePort` set to `30005` configured with session affinity.

```yaml
apiVersion: v1
kind: Service
metadata:
  name: sample-nodeport-session-affinity
spec:
  selector:
    app: web
  ports:
...
A sample LoadBalancer Service with the ingress IP `192.168.77.150` assigned by an ingress controller.
...
A sample LoadBalancer Service with the ingress IP `192.168.77.151` assigned by an ingress controller,
configured with session affinity.

```yaml
apiVersion: v1
kind: Service
metadata:
  name: sample-loadbalancer-session-affinity
spec:
  selector:
    app: web
  ports:
...
A sample Service with the external IP `192.168.77.200`.
...
### Service with ExternalIP and Session Affinity

A sample Service with the external IP `192.168.77.201` configured with session affinity.

```yaml
apiVersion: v1
kind: Service
metadata:
  name: sample-service-externalip-session-affinity
spec:
  selector:
    app: web
  ports:
...
In addition to the tables created for Kubernetes NetworkPolicy, Antrea creates additional dedicated tables to support
the [Antrea-native NetworkPolicy](../antrea-network-policy.md) (table [AntreaPolicyEgressRule] and [AntreaPolicyIngressRule]).
...
80 from the Pods with the label `app: client` and to the Pods with the label `app: db` is allowed. Similar to Kubernetes
NetworkPolicy, Antrea will only install OVS flows for this policy on Nodes that have Pods selected by the policy. We
still use the Pods with the label `app: web` mentioned above as the target of this policy.
...
## Antrea L7 NetworkPolicy Implementation
...
traffic, not layer 3 or layer 4 protocol.

Consider the following Antrea layer 7 NetworkPolicy in the Application tier as an example for the remainder of this
document.

This ACNP is applied to all Pods with the label `app: client` in all Namespaces. For these Pods, egress DNS traffic on
port 53 and egress HTTP traffic with host `*.bar.com` is allowed.
...
This ACNP is applied to all Pods with the label `app: web` in all Namespaces. For these Pods, only ingress HTTP
traffic with method `GET` path `/api/v2/*` from the Pods with the label `app: client` is allowed, and others will
be dropped.
...
Antrea creates a dedicated table [TrafficControl] to implement `TrafficControl`. Use the following TrafficControls
as examples throughout the remainder of this document.

This is a TrafficControl applied to Pods with the label `app: web`. For these Pods, both ingress and egress traffic of
them will be redirected to the port `antrea-tc-tap0`, and returned back via the port `antrea-tc-tap1`.
...
This is a TrafficControl applied to Pods with the label `app: db`. For these Pods, all traffic, both ingress and egress,
will be mirrored to the port `antrea-tc-tap2`.
...
Table [EgressMark] is dedicated to the implementation of `Egress` feature.

Consider the following Egresses as examples for the remainder of this document.

This is a Egress applied to Pods with the label `app: web`. For these Pods, all egress traffic will be SNAT'd on Node
`k8s-node-control-plane` from which we dumped flows in the document with the IP `192.168.77.102`, which is the transport
IP of the Node.
...
spec:
  appliedTo:
    podSelector:
      matchLabels:
        app: web
  egressIP: 192.168.77.102
status:
  egressNode: k8s-node-control-plane
```

This is a Egress applied to Pods with the label `app: client`. For these Pods, all egress traffic will be SNAT'd on Node
`k8s-node-worker-1` with the IP `192.168.77.103`, which is the transport IP of a remote Node.
...
1. Packets originating from the local Antrea gateway, requiring IP spoof legitimacy verification.
2. Packets originating from the external network through the Antrea gateway port.
3. Packets through an overlay tunnel.
4. Packets through a TrafficControl return port defined in a user-provided TrafficControl CR (for feature `TrafficControl`).
5. Packets through Antrea layer 7 tap port (for feature `L7NetworkPolicy`).
6. Packets originating from local Pods, requiring IP spoof legitimacy verification.

If you dump the flows of this table, you may see the following:

```text
1. table=Classifier, priority=210,ip,in_port="antrea-gw0",nw_src=10.10.0.1 actions=set_field:0x2/0xf->reg0,goto_table:SpoofGuard
...
4. table=Classifier, priority=200,in_port="antrea-l7-tap1",vlan_tci=0x1000/0x1000 actions=pop_vlan,set_field:0x6/0xf->reg0,goto_table:L3Forwarding
5. table=Classifier, priority=190,in_port="client-6-3353ef" actions=set_field:0x3/0xf->reg0,goto_table:SpoofGuard
6. table=Classifier, priority=190,in_port="web-7975-274540" actions=set_field:0x3/0xf->reg0,goto_table:SpoofGuard
7. table=Classifier, priority=190,in_port="db-755c6-5080e3" actions=set_field:0x3/0xf->reg0,goto_table:SpoofGuard
...
the local Antrea gateway. `FromGatewayRegMark` is loaded to mark the packet source, which is consumed in tables
[L3Forwarding], [L3DecTTL], [SNATMark] and [SNAT].

Flow 2 is designed for case 2, matching packets originating from the external network through the Antrea gateway port and
forwarding them to table [SpoofGuard]. Since packets originating from the local Antrea gateway are matched by flow 1,
flow 2 can only match packets originating from the external network. The following reg marks are loaded:
...
Flow 3 is case 3, matching packets through an overlay tunnel (i.e., from another Node) and forwarding them to table
[UnSNAT]. This approach is based on the understanding that tunnel-borne packets originate from remote Nodes, potentially
...
- `FromTunnelRegMark`, to mark packet source, consumed in table [L3Forwarding].
...
to decide the egress port. It's important to note that both the source and destination MAC addresses of the packets have been
set to the expected state before redirecting the packets to the TrafficControl target port in table [Output]. The only
purpose of forwarding the packets to table [L3Forwarding] is to load tunnel destination IP for packets destined for remote
Nodes. This ensures that the returned packets destined for remote Nodes are forwarded through the tunnel.
`FromTCReturnRegMark`, which is used in table [TrafficControl], is loaded to mark the packet source.

Flow 5 is for case 5. It matches packets from the Antrea layer 7 tap port and forwards them to table [L3Forwarding] to
decide the egress port. Like flow 4, the purpose of forwarding the packets to table [L3Forwarding] is to load tunnel
destination IP for packets destined for remote Nodes. `FromTCReturnRegMark`, which is used in table [TrafficControl],
is also loaded to mark the packet source.

Flows 6-7 are for case 6, matching packets from local Pods and forwarding them to table [SpoofGuard] to do legitimacy
verification. `FromLocalRegMark`, which is used in tables [L3Forwarding] and [SNATMark], is loaded to mark the packet source.

Flow 8 is the table-miss flow to drop packets that are not matched by flows 1-7.
...
Flow 1 matches reply packets from Service connections SNAT'd with the *Virtual Service IP* `169.254.0.253` and invokes
action `ct` on them. For the packets, the destination IP of them is the *Virtual Service IP*.

Flow 2 matches packets from Service connections SNAT'd with the local Antrea gateway IP `10.10.0.1` and invokes
action `ct` on them. For the packets, the destination IP of them is the local Antrea gateway IP. This flow also matches
request packets destined for the local Antrea gateway IP from local Pods by accident. However, this is harmless since
such connections will never be committed with `SNATCtZone`, and therefore, connection tracking fields for the packets
are unset.
...
the destination IP of the packets will be restored to the original IP before SNAT stored in the connection tracking field
`ct_nw_dst`.

### ConntrackZone

The main purpose of this table is to invoke action `ct` on packets from all connections. After invoking `ct` action,
packets will be in a "tracked" state, restoring all connection tracking fields to their appropriate values. It's worth
noting that when invoking action `ct` with `CtZone` to the packets that have a "tracked" state associated with
`SNATCtZone`, then the "tracked" state associated with `SNATCtZone` will be inaccessible. This transition occurs because
the "tracked" state shifts to another state associated with `CtZone`. A ct zone is similar in spirit to the more generic
Linux network namespaces, uniquely containing a "tracked" state within each ct zone.
...
Flow 1 invokes `ct` action on packets from all connections, and the packets are then forwarded to table
[ConntrackStateTable] with the "tracked" state associated with `CtZone`. It's worth noting that:

- For request packets but not the first one from Service connections DNAT'd with an Endpoint IP, whose destination IP is
  a ServiceIP, the destination IP of the packets will be restored to the translated IP, which is just the Endpoint IP,
  stored in the connection tracking field `ct_nw_dst`.
- For reply packets from Service connections DNAT'd with an Endpoint IP, whose source IP is the EndpointIP, the
  source IP of the packets will be restored to the original IP, which is just the Service IP, stored in the connection
  tracking field `ct_nw_src`.
...
Flow 1 is a learned flow generated by flow 3 in table [ServiceLB], designed for sample Service [ClusterIP with Session
Affinity], to implement Service session affinity. Here are some details about the flow:

- The hard timeout of the learned flow should be equal to the value of `service.spec.sessionAffinityConfig.clientIP.timeoutSeconds`
  defined in the Service. This means that during the hard timeout, this flow is present in the pipeline, and the session
  affinity of the Service takes effect during the timeout.
...
2. ClusterIP configured with session affinity, as demonstrated in the example [ClusterIP with Session Affinity].
3. NodePort, as demonstrated in the example [NodePort].
4. NodePort configured with session affinity, as demonstrated in the example [NodePort with Session Affinity].
5. LoadBalancer, as demonstrated in the example [LoadBalancer with ExternalIP].
6. LoadBalancer configured with session affinity, as demonstrated in the example [LoadBalancer with Session Affinity].
7. Service with ExternalIP, as demonstrated in the example [Service with ExternalIP].
8. Service with ExternalIP configured with session affinity, as demonstrated in the example [Service with ExternalIP
...
3. table=ServiceLB, priority=200,tcp,reg4=0x10000/0x70000,nw_dst=10.96.76.15,tp_dst=80 actions=set_field:0x200/0x200->reg0,set_field:0x30000/0x70000->reg4,set_field:0xa->reg7,group:11
4. table=ServiceLB, priority=190,tcp,reg4=0x30000/0x70000,nw_dst=10.96.76.15,tp_dst=80 actions=learn(table=SessionAffinity,hard_timeout=300,priority=200,delete_learned,cookie=0x203000000000a,\
...
5. table=ServiceLB, priority=200,tcp,reg4=0x90000/0xf0000,tp_dst=30004 actions=set_field:0x200/0x200->reg0,set_field:0x20000/0x70000->reg4,set_field:0x200000/0x200000->reg4,set_field:0xc->reg7,group:12
6. table=ServiceLB, priority=200,tcp,reg4=0x90000/0xf0000,tp_dst=30005 actions=set_field:0x200/0x200->reg0,set_field:0x30000/0x70000->reg4,set_field:0x200000/0x200000->reg4,set_field:0xd->reg7,group:13
7. table=ServiceLB, priority=190,tcp,reg4=0xb0000/0xf0000,tp_dst=30005 actions=learn(table=SessionAffinity,hard_timeout=300,priority=200,delete_learned,cookie=0x303000000000e,\
   eth_type=0x800,nw_proto=6,NXM_OF_TCP_DST[],NXM_OF_IP_DST[],NXM_OF_IP_SRC[],load:NXM_NX_REG4[0..15]->NXM_NX_REG4[0..15],load:NXM_NX_REG4[26]->NXM_NX_REG4[26],load:NXM_NX_REG3[]->NXM_NX_REG3[],load:0x2->NXM_NX_REG4[16..18],load:0x1->NXM_NX_REG0[9],load:0x1->NXM_NX_REG4[21]),\
   set_field:0x20000/0x70000->reg4,goto_table:EndpointDNAT
8. table=ServiceLB, priority=200,tcp,reg4=0x10000/0x70000,nw_dst=192.168.77.150,tp_dst=80 actions=set_field:0x200/0x200->reg0,set_field:0x20000/0x70000->reg4,set_field:0xe->reg7,group:14
9. table=ServiceLB, priority=200,tcp,reg4=0x10000/0x70000,nw_dst=192.168.77.151,tp_dst=80 actions=set_field:0x200/0x200->reg0,set_field:0x30000/0x70000->reg4,set_field:0xf->reg7,group:15
10. table=ServiceLB, priority=190,tcp,reg4=0x30000/0x70000,nw_dst=192.168.77.151,tp_dst=80 actions=learn(table=SessionAffinity,hard_timeout=300,priority=200,delete_learned,cookie=0x203000000000a,\
    eth_type=0x800,nw_proto=6,NXM_OF_TCP_DST[],NXM_OF_IP_DST[],NXM_OF_IP_SRC[],load:NXM_NX_REG4[0..15]->NXM_NX_REG4[0..15],load:NXM_NX_REG4[26]->NXM_NX_REG4[26],load:NXM_NX_REG3[]->NXM_NX_REG3[],load:0x2->NXM_NX_REG4[16..18],load:0x1->NXM_NX_REG0[9]),\
    set_field:0x20000/0x70000->reg4,goto_table:EndpointDNAT
11. table=ServiceLB, priority=200,tcp,reg4=0x10000/0x70000,nw_dst=192.168.77.200,tp_dst=80 actions=set_field:0x200/0x200->reg0,set_field:0x20000/0x70000->reg4,set_field:0x10->reg7,group:16
12. table=ServiceLB, priority=200,tcp,reg4=0x10000/0x70000,nw_dst=192.168.77.201,tp_dst=80 actions=set_field:0x200/0x200->reg0,set_field:0x30000/0x70000->reg4,set_field:0x11->reg7,group:17
13. table=ServiceLB, priority=190,tcp,reg4=0x30000/0x70000,nw_dst=192.168.77.201,tp_dst=80 actions=learn(table=SessionAffinity,hard_timeout=300,priority=200,delete_learned,cookie=0x203000000000a,\
    eth_type=0x800,nw_proto=6,NXM_OF_TCP_DST[],NXM_OF_IP_DST[],NXM_OF_IP_SRC[],load:NXM_NX_REG4[0..15]->NXM_NX_REG4[0..15],load:NXM_NX_REG4[26]->NXM_NX_REG4[26],load:NXM_NX_REG3[]->NXM_NX_REG3[],load:0x2->NXM_NX_REG4[16..18],load:0x1->NXM_NX_REG0[9]),\
    set_field:0x20000/0x70000->reg4,goto_table:EndpointDNAT
14. table=ServiceLB, priority=0 actions=goto_table:EndpointDNAT
```

Flow 1 or flow 2 is designed for case 1, matching the first packet of connections destined for a ClusterIP Service. This
is achieved by matching `EpToSelectRegMark` loaded in table [SessionAffinity], clusterIP, and port. The target
of the packet is an OVS group where the Endpoint will be selected. Before forwarding the packet to the OVS group,
`RewriteMACRegMark`, which will be consumed in table [L3Forwarding], is loaded, indicating that the source and destination
MAC addresses of the packets should be rewritten. `EpSelectedRegMark`, which will be consumed in table [EndpointDNAT],
is also loaded, indicating that the Endpoint is selected. Note that the Service Endpoint selection is not completed yet,
as it will be done in the target OVS group. The action is set here to support more Endpoints in an
OVS group. Refer to PR [#2101](https://github.com/antrea-io/antrea/pull/2101) for more information.

Flow 3 is the initial process for case 2, matching the first packet of connections destined for a ClusterIP Service configured
with session affinity. This is achieved by matching the conditions similar to flow 1. Like flow 1, the target of the
flow is also an OVS group, and `RewriteMACRegMark` is loaded. The difference is that `EpToLearnRegMark` is loaded, rather
than `EpSelectedRegMark`, indicating that the selected Endpoint needs to be cached.

Flow 4 is the final process for case 2, matching the packet previously matched by flow 2, sent back from the target OVS
...
Flow 5 is for case 3, matching the first packet of connections destined for a NodePort Service. This is achieved by matching
`EpToSelectRegMark` loaded in table [SessionAffinity], `NodePortRegMark` loaded in table [NodePortMark], and NodePort port.
Similar to flows 1-2, `RewriteMACRegMark` and `EpSelectedRegMark` are also loaded.

Flows 6-7 are for case 4, processing the first packet of connections destined for a NodePort Service configured with
session affinity, similar to flows 3-4.

Flow 8 is for case 5, processing the first packet of connections destined for a LoadBalancer Service, similar to flow 1.

Flows 9-10 are for case 6, processing the first packet of connections destined for a LoadBalancer Service configured with
session affinity, similar to flows 3-4.

Flow 11 is for case 7, processing the first packet of connections destined for a Service with an external IP, similar to
flow 1.

Flows 12-13 are for case 8, processing the first packet of connections destined for a Service configured with session
affinity with an external IP, similar to flows 3-4.

Flow 14 is the table-miss flow.

As mentioned above, the Service Endpoint selection is performed within OVS groups. The following lists three typical
OVS groups:
...
Group 1 is the destination of packets matched by flow 1, designed for a Service without Endpoints. The group only has a
single bucket where `SvcNoEpRegMark`, to be used in table [EndpointDNAT], is loaded, indicating that the Service has no
Endpoint, and then packets are forwarded to table [EndpointDNAT]. It's important to note that any Services lacking
Endpoints share the same target group as group 9.

Group 2 is the destination of packets matched by flow 2, designed for a Service with Endpoints. The group has 2 buckets,
indicating the availability of 2 selectable Endpoints. Each bucket has an equal chance of being chosen since they have
the same weights. For every bucket, the Endpoint IP and Endpoint port are loaded into `EndpointIPField` and
`EndpointPortField`, respectively. These loaded values will be consumed in table [EndpointDNAT] to which the packets are
forwarded and in which DNAT will be performed. It's worth noting that `RemoteEndpointRegMark` is loaded for remote
Endpoints, such as in bucket 1.

Group 3 is the destination of packets matched by flow 3, designed for a Service that has Endpoints and is configured
with session affinity. The group closely resembles group 2, except that the destination of the packets is table
[ServiceLB], rather than table [EndpointDNAT]. After being sent back to table [ServiceLB], they will be matched by flow 4.
...
by matching `SvcNoEpRegMark`. Subsequently, the packet is forwarded to the OpenFlow controller (Antrea Agent) for
further processing.
...
performing DNAT'd and forwarding it to table [ConntrackStateTable] with the "tracked" state associated with `CtZone`.
It is worth noting that some bits of ct mark are persisted:
...
Kubernetes NetworkPolicies and their egress rules are installed in tables [EgressDefault] and [EgressRule] respectively,
i.e.

```text
Antrea-native NetworkPolicy Baseline Tier  ->  EgressDefault
Kubernetes NetworkPolicy                   ->  EgressRule
...
"exploding" the number of flows. For our use case, we have at most 3 dimensions.
...
([AntreaPolicyEgressRule], [EgressRule], [EgressDefault], [AntreaPolicyIngressRule], [IngressRule], and [EgressDefault]).
...
[specification](#antrea-l7-networkpolicy-implementation) that we are using. If you dump the flows of this table, you may see the
following:
...
3. table=AntreaPolicyEgressRule, priority=44900,ip,nw_src=10.10.0.24 actions=conjunction(7,1/3)
4. table=AntreaPolicyEgressRule, priority=44900,ip,nw_dst=10.10.0.25 actions=conjunction(7,2/3)
5. table=AntreaPolicyEgressRule, priority=44900,tcp,tp_dst=3306 actions=conjunction(7,3/3)
6. table=AntreaPolicyEgressRule, priority=44900,conj_id=7,ip actions=set_field:0x7->reg5,ct(commit,table=EgressMetric,zone=65520,exec(set_field:0x700000000/0xffffffff00000000->ct_label))
7. table=AntreaPolicyEgressRule, priority=44899,ip,nw_src=10.10.0.24 actions=conjunction(5,1/2)
8. table=AntreaPolicyEgressRule, priority=44899,ip actions=conjunction(5,2/2)
9. table=AntreaPolicyEgressRule, priority=44899,conj_id=5 actions=set_field:0x5->reg3,set_field:0x400/0x400->reg0,goto_table:EgressMetric
10. table=AntreaPolicyEgressRule, priority=44880,ip,nw_src=10.10.0.26 actions=conjunction(11,1/3)
11. table=AntreaPolicyEgressRule, priority=44880,ip actions=conjunction(11,2/3)
12. table=AntreaPolicyEgressRule, priority=44880,tcp,tp_dst=53 actions=conjunction(11,3/3)
13. table=AntreaPolicyEgressRule, priority=44880,udp,tp_dst=53 actions=conjunction(11,3/3)
14. table=AntreaPolicyEgressRule, priority=44880,conj_id=11,ip actions=set_field:0xb->reg5,ct(commit,table=EgressMetric,zone=65520,exec(set_field:0xb00000000/0xffffffff00000000->ct_label))
15. table=AntreaPolicyEgressRule, priority=44879,ip actions=conjunction(13,2/2)
16. table=AntreaPolicyEgressRule, priority=44879,ip,nw_src=10.10.0.26 actions=conjunction(13,1/2)
17. table=AntreaPolicyEgressRule, priority=44879,conj_id=13,ip actions=set_field:0xd->reg5,ct(commit,table=EgressMetric,zone=65520,exec(set_field:0xd00000000/0xffffffff00000000->ct_label,set_field:0x80/0x80->ct_mark,set_field:0x40000000000000000/0xfff0000000000000000->ct_label))
18. table=AntreaPolicyEgressRule, priority=0 actions=goto_table:EgressRule
```
...
It's worth noting that the priorities of flows 3-17 installed for the egress rules are decided by the following:

- The value of field `spec.priority` in the Antrea-native NetworkPolicies. A lower value in this field corresponds to
  a higher priority for the flow.
- The position of a rule. A rule positioned closer to the beginning has a higher priority for the flow.
...
- Flow 3 is utilized to match packets with the source IP address in set {10.10.0.24}, which has all IP addresses of
  the Pods selected by label `app: web`, constituting the first dimension for `conjunction` with `conj_id` 7.
- Flow 4 is utilized to match packets with the destination IP address in set {10.10.0.25}, which has all IP addresses of
  the Pods selected by label `app: db`, constituting the second dimension for `conjunction` with `conj_id` 7.
- Flow 5 is utilized to match packets with the destination TCP port in set {3306} specified in the rule, constituting
  the third dimension for `conjunction` with `conj_id` 7.
- Flow 6 is utilized to match packets meeting all the three dimensions of `conjunction` with `conj_id` 7 and forward
  them to table [EgressMetric], persisting `conj_id` to `EgressRuleCTLabel` that is consumed in table [EgressMetric].

Flows 7-9, whose priorities are all 44899, are installed for the egress rule with a `Drop` action defined after the rule
`AllowToDb` in the sample policy, serves as a default rule. Unlike the default of Kubernetes NetworkPolicy,
Antrea-native NetworkPolicy has no default rule, and all rules should be explicitly defined. Hence, they are evaluated
as-is, and there is no need for a table [AntreaPolicyEgressDefault]. These flows are described as follows:
...
- Flow 9 is utilized to match packets meeting both dimensions of `conjunction` with `conj_id` 5. `APDenyRegMark` is
  loaded and will be consumed in table [EgressMetric] to which the packets are forwarded.

Flows 10-14, whose priorities are all 44880, are installed for the egress rule `AllowDNS` in the sample policy,
applying to Pods with the label `app: client`. The pattern of these flows is similar to that of flows 3-6 or flows 7-9.

Flows 14-17, whose priories are all 44879, are installed for the egress rule `AllowHTTP` in the sample policy.
Unlike other policies, this sample policy is for L7 protocol, allowing HTTP with host `*.bar.com` ingress traffic.
Flows 15-16 are nothing different. In flow 17, there is something specific for L7 protocol:

- `L7NPRedirectCTMark` is persisted, indicating the packets should be redirected to an application-aware engine.
...
Flow 18 is the table-miss flow to forward packets not matched by other flows to table [EgressMetric].
...
Flows 1-4 are installed for the egress rule in the sample Kubernetes NetworkPolicy. These flows are described as follows:

- Flow 1 is to match packets with the source IP address in set {10.10.0.24}, which has all IP addresses of the Pods
  selected by label `app: web`, constituting the first dimension for `conjunction` with `conj_id` 2.
...
This table complements table [EgressRule] for Kubernetes NetworkPolicy egress rule implementation. When a NetworkPolicy
is applied to a set of Pods, and the default behavior for these Pods becomes "deny" (it becomes an [isolated Pod](
...
at a lower priority than Kubernetes NetworkPolicy default drop flows. These flows are similar to flows 3-9 in table
[AntreaPolicyEgressRule].

### EgressMetric

This table is used to collect egress metrics for Antrea-native NetworkPolicies and Kubernetes NetworkPolicies.
...
the value persisted in the ct label can be read throughout the entire lifecycle of a connection, but the reg mark is only valid for
...
local Antrea gateway, loads `ToGatewayRegMark`, and forwards them to table [L3DecTTL] to decrease TTL value. It's worth
noting that the action of rewriting the destination MAC address is not necessary but not harmful for Pod-to-gateway request
packets because the destination MAC address is already the local gateway MAC address. However, it is utilized by the
feature AntreaIPAM, which is not enabled by default.
...
of the connection. This flow is required to handle the following cases when AntreaProxy is not disabled:
...
- Reply traffic for connections from a local Pod to a ClusterIP Service, which are handled by kube-proxy and go through
  ...
  Service (no SNAT by kube-proxy). We need to make sure that these packets are sent back through the gateway so that
  the source IP can be rewritten to the ClusterIP ("undo" DNAT). If we do not use connection tracking and do not rewrite
...
to table [EgressMark] dedicated to feature Egress. To match the expected packets, `FromTunnelRegMark` is utilized to
exclude packets that are not from remote Pods through a tunnel. Considering that the packets from remote Pods traverse a
...
This table is dedicated to `Egress`. It includes flows to select the right SNAT IPs for egress traffic originating from
Pods and destined for the external network.
...
4. table=EgressMark, priority=200,ip,in_port="client-6-3353ef" actions=set_field:ba:5e:d1:55:aa:c0->eth_src,set_field:aa:bb:cc:dd:ee:ff->eth_dst,set_field:192.168.77.103->tun_dst,set_field:0x10/0xf0->reg0,set_field:0x80000/0x80000->reg0,goto_table:L2ForwardingCalc
5. table=EgressMark, priority=200,ct_state=+new+trk,ip,tun_dst=192.168.77.102 actions=set_field:0x1/0xff->pkt_mark,set_field:0x20/0xf0->reg0,goto_table:L2ForwardingCalc
...
sending the packets to the tunnel, the source and destination MAC addresses are set to those of the local Antrea
gateway and the *Global Virtual MAC*, respectively. Additionally, `ToTunnelRegMark`, indicating that the output port is
a tunnel, and `EgressSNATRegMark`, indicating that packets should undergo SNAT on a remote Node, are loaded. Finally,
the packets are forwarded to table [L2ForwardingCalc].
...
whose SNAT IP is configured on the local Node. Similar to flow 4, the 8-bit ID is loaded to `pkt_mark`, `ToGatewayRegMark`
...
unexpectedly and should be dropped.
...
associated with `CtZone`. After SNAT, their "track" state is associated with `SNATCtZone`, and then `ServiceCTMark` and
...
and the port number of the local Antrea gateway to `TargetOFPortField`. Both of these two values will be consumed in
table [Output].
...
Pods. Similar to flow 1, `OutputToOFPortRegMark` is loaded, and the port number of the tunnel is loaded to
`TargetOFPortField`.
...
Flow 2 is also installed for the sample TrafficControl `redirect-web-to-local`. Similar to flow 2,
...
NetworkPolicies. Depending on the tier to which the policy belongs, the rules will be installed in a table
corresponding to that tier. The ingress table to tier mappings is as follows:

```text
Antrea-native NetworkPolicy Baseline Tier  ->  IngressDefault
Kubernetes NetworkPolicy                   ->  IngressRule
...
10. table=AntreaPolicyIngressRule, priority=44880,reg1=0x7 actions=conjunction(14,2/2)
11. table=AntreaPolicyIngressRule, priority=44880,ip,nw_src=10.10.0.26 actions=conjunction(14,1/2)
12. table=AntreaPolicyIngressRule, priority=44880,conj_id=0x25,ip actions=set_field:0xd->reg6,ct(commit,table=IngressMetric,zone=65520,exec(set_field:0xd/0xffffffff->ct_label,set_field:0x80/0x80->ct_mark,set_field:0x20000000000000000/0xfff0000000000000000->ct_label))
13. table=AntreaPolicyIngressRule, priority=44879,reg1=0x7 actions=conjunction(15,2/2)
14. table=AntreaPolicyIngressRule, priority=44879,ip actions=conjunction(15,1/2)
15. table=AntreaPolicyIngressRule, priority=44879,conj_id=15 actions=set_field:0xf->reg3,set_field:0x400/0x400->reg0,goto_table:IngressMetric
16. table=AntreaPolicyIngressRule, priority=0 actions=goto_table:IngressRule
```
...
Similar to table [AntreaPolicyEgressRule], the priorities of flows 3-15 installed for the ingress rules are decided by
the following:
...
Flows 3-6, whose priorities are 44900, are installed for the egress rule `AllowFromClient` in the sample policy.
...
Flows 7-9, whose priorities are all 44899, are installed for the egress rule with a `Drop` action defined after the rule
`AllowFromClient` in the sample policy, serves as a default rule. Unlike the default of Kubernetes NetworkPolicy,
Antrea-native NetworkPolicy has no default rule, and all rules should be explicitly defined. Hence, they are evaluated
...
- Flow 8 is utilized to match packets with the output port in set {0x25},  which has all the ports of the Pods selected
  by the label `app: web`, constituting the first dimension for `conjunction` with `conj_id` 4.
- Flow 9 is utilized to match packets meeting both dimensions of `conjunction` with `conj_id` 4. `APDenyRegMark` that
  will be consumed in table [IngressMetric] to which the packets are forwarded is loaded.

Flows 10-12, whose priories are all 44880, are installed for the egress rule `AllowFromClientL7` in the sample policy,
applying to Pods with the label `app: web`. Unlike other policies, this sample policy is for L7 protocol, allowing HTTP
with GET method and path `/api/v2/*` ingress traffic. Flows 10-11 are nothing different. In flow 12, there is something
specific for L7 protocol:

- `L7NPRedirectCTMark` is persisted, indicating the packets should be redirected to an application-aware engine.
- A VLAN ID allocated for the Antrea-native L7 NetworkPolicy is persisted in `L7NPRuleVlanIDCTLabel`, which will be
  consumed in table [Output].

Flows 13-15 are similar to flows 3-6 or flows 7-9.

Flow 16 is the table-miss flow to forward packets not matched by other flows to table [IngressMetric].

### IngressRule

This table is very similar to table [EgressRule] but implements ingress rules for Kubernetes NetworkPolicies. Once again,
you will need to keep in mind the Kubernetes NetworkPolicy [specification](#kubernetes-networkpolicy-implementation) that
we are using.
...
Flows 1-4 are installed for the ingress rule in the sample Kubernetes NetworkPolicy. These flows are described as follows:
...
This table is similar in its purpose to table [IngressDefault], and it complements table [IngressRule] for Kubernetes
...
Flow 1, based on our sample Kubernetes NetworkPolicy, is to drop traffic destined for port 0x25, the port number
associated with a Pod selected by the label `app: web`.
...
1. Outputting packets to an application-aware engine for further L7 protocol processing.
2. Outputting packets to an OVS port and a port defined in a TrafficControl CR with `Mirror` action.
3. Outputting packets to a port defined in a TrafficControl CR with `Redirect` action.
4. Outputting packets from hairpin connections to the ingress port where they were received.
5. Outputting packets to an OVS port.
6. Outputting packets to the OpenFlow controller (Antrea Agent).
7. Dropping packets.
...
Flow 1 is for case 1. It matches packets with `L7NPRedirectCTMark` and `OutputToOFPortRegMark`, and then forwards them to
the port `antrea-l7-tap0` specifically created for connecting to an application-aware. Notably, these packets are pushed
with an 802.1Q header and loaded with the VLAN ID value persisted in `L7NPRuleVlanIDCTLabel` before being forwarded, due to
the `L7NetworkPolicy` implementation.

Flow 2 is for case 2. It matches packets with `TrafficControlMirrorRegMark` and `OutputToOFPortRegMark`, and then
forwards them to the port specified in `TargetOFPortField` and the port specified in `TrafficControlTargetOFPortField`.

Flow 3 is for case 3. It matches packets with `TrafficControlRedirectRegMark` and `OutputToOFPortRegMark`, and then
forwards them to the port specified in `TrafficControlTargetOFPortField`.

Flow 4 is for case 4. It matches packets from hairpin connections by matching `HairpinCTMark` and forwards it back to the
