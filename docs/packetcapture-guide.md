  ...
  packet:
    ...
    transportHeader:
      icmp:
        ...
        # type value can be provided in either numeric or string type. Available options: 'icmp-echo', 'icmp-echoreply', 'icmp-unreach' or 'icmp-timxceed' 
        # and code value can be provided in only numeric type.
        messages:
...
to a Pod named `backend` using ICMP protocol and are either echo reply or destination (host) unreachable packets.
