...
`egressIP` and `externalIPPool` fields. These field sets are mutually exclusive:
an Egress must use either `egressIPs` / `externalIPPools` or `egressIP` /
`externalIPPool`, but not both.
