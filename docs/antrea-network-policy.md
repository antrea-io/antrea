...
The Service referred in `toServices` represents all Endpoints IPs+Ports of this Service.
`toServices` can't be used with `to` or `ports` and only ClusterIP type Service is support in `toServices` for now.
...
The [fourth example](#acnp-for-toservice-rule) policy contains a single rule,
which drops matched traffic to all Endpoints IPs+Ports of this Service.
