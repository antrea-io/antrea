...
Under `ports`, `ports.protocal` and `ports.port` are the same as K8s network policy. 
And `ports.portRange` can either match a port range inclusive with optional exceptions 
using `portRange.from`, `portRange.to` and `portRange.except` or match a single port 
using `portRange.port`. As a consequence, `portRange.from` and `portRange.to` must be 
set at the same time and can't set with `portRange.port`.
...
Under `ports`, `ports.protocal` and `ports.port` are the same as K8s network policy. 
And `ports.portRange` is the same as ingress.
Also, each rule has an optional `name` field, which should be unique within 
the policy describing the intention of this rule. If `name` is not provided for 
