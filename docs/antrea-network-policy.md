...
allows or drops traffic which matches all `from`, `ports` and `portRanges` sections.
Under `portRanges`, `range` matches a port range inclusive with optional exceptions 
using `range.from`, `range.to` and `range.except` 
...
or drops traffic which matches all `from`, `ports` and `portRanges` sections.
Under `portRanges`, `range` matches a port range inclusive with optional exceptions 
using `range.from`, `range.to` and `range.except` 
Also, each rule has an optional `name` field, which should be unique within 
the policy describing the intention of this rule. If `name` is not provided for 
