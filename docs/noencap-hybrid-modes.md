...
Some features still send part of their traffic through a tunnel in `NoEncap`
mode, while Pod-to-Pod traffic remains routed. When one of them is enabled,
Antrea creates the tunnel interface (`antrea-tun0`) and reduces the Pod MTU by
the encapsulation overhead, as it does in `Encap` mode, and the Node network
must allow the tunnel traffic between Nodes:

* [Egress](egress.md) sends the traffic of a Pod through the tunnel when its
