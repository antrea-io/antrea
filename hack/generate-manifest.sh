...
Generate a YAML manifest for Antrea using Kustomize and print it to stdout.
        ...
        --encap-mode          Inter-node pod traffic encapitulation mode.
...
if [[ $ENCAP_MODE != "" ]]; then
    sed -i.bak -E "s/^[[:space:]]*#[[:space:]]*podTrafficEncapMode[[:space:]]*:[[:space:]]*[a-z]+[[:space:]]*$/podTrafficEncapMode: $ENCAP_MODE/" antrea-agent.conf
fi
