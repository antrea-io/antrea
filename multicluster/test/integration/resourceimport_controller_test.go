...
var _ = Describe("ResourceImport controller", func() {
	...
	It("Should create EndpointSlice when ResourceImport only contains Subsets from legacy leader", func() {
		subsetsSvcResImport := &mcsv1alpha1.ResourceImport{
			ObjectMeta: metav1.ObjectMeta{
				Namespace: "leader-ns",
				Name:      "cluster-a-subsets-nginx-service",
			},
			Spec: mcsv1alpha1.ResourceImportSpec{
				Name:      "subsets-nginx",
				Namespace: "default",
		...
		subsetsEPResImport := &mcsv1alpha1.ResourceImport{
			ObjectMeta: metav1.ObjectMeta{
				Namespace: "leader-ns",
				Name:      "cluster-a-subsets-nginx-endpoints",
			},
			Spec: mcsv1alpha1.ResourceImportSpec{
				Name:      "subsets-nginx",
				Namespace: "default",
				...
				Endpoints: &mcsv1alpha1.EndpointsImport{
					Subsets: []corev1.EndpointSubset{
						{
							Addresses: []corev1.EndpointAddress{{IP: "10.244.2.20"}},
		...
		Expect(k8sClient.Create(ctx, subsetsSvcResImport)).Should(Succeed())
		Expect(k8sClient.Create(ctx, subsetsEPResImport)).Should(Succeed())

		Eventually(func() bool {
			eps := &discovery.EndpointSlice{}
			err := k8sClient.Get(ctx, types.NamespacedName{Namespace: "default", Name: "antrea-mc-subsets-nginx"}, eps)
			if err != nil || len(eps.Endpoints) == 0 {
				return false
			}
			return eps.Endpoints[0].Addresses[0] == "10.244.2.20"
		}, timeout, interval).Should(BeTrue())

		// Cleanup
		Expect(k8sClient.Delete(ctx, subsetsSvcResImport)).Should(Succeed())
		Expect(k8sClient.Delete(ctx, subsetsEPResImport)).Should(Succeed())
	})
})
