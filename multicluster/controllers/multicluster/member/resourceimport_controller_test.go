...
func TestResourceImportReconciler_handleUpdateEvent(t *testing.T) {
	...
	// epResImportSubsets simulates an older leader that only populates Subsets
	existMCSvcSubsets := existMCSvc.DeepCopy()
	existMCSvcSubsets.Name = "antrea-mc-nginx-subsets"
	existMCSvcSubsets.UID = "uid-subsets"

	epResImportSubsets := epResImport.DeepCopy()
	epResImportSubsets.Name = leaderNamespace + "-nginx-subsets-endpoints"
	epResImportSubsets.Spec.Name = "nginx-subsets"
	epResImportSubsets.Spec.Endpoints = &mcv1alpha1.EndpointsImport{
		Subsets: []corev1.EndpointSubset{
			{
				Addresses: []corev1.EndpointAddress{{IP: "192.168.17.11"}},
	...
	fakeClient := fake.NewClientBuilder().WithScheme(common.TestScheme).WithObjects(
		...
		legacyEP, existMCSvcLegacy, existMCSvcSubsets).Build()
	fakeRemoteClient := fake.NewClientBuilder().WithScheme(common.TestScheme).WithObjects(
		...
		epResImportLegacy, epResImportSubsets).Build()
	remoteCluster := commonarea.NewFakeRemoteCommonArea(fakeRemoteClient, "leader-cluster", localClusterID, "default", nil)
	...
	tests := []struct {
		...
		{
			name:    "fallback to Subsets when Endpoints is empty from legacy leader",
			objType: "Endpoints",
			req: ctrl.Request{NamespacedName: types.NamespacedName{
				Namespace: leaderNamespace,
				Name:      epResImportSubsets.Name,
			}},
			resNamespaceName: types.NamespacedName{Namespace: "default", Name: "antrea-mc-nginx-subsets"},
			expectedEPEndpoints: []discovery.Endpoint{
				{
					Addresses:  []string{"192.168.17.11"},
				...
				case "Endpoints":
					...
					if err := fakeClient.Get(ctx, tt.resNamespaceName, eps); err != nil {
						...
						if len(eps.OwnerReferences) > 0 {
							assert.Nil(t, eps.OwnerReferences[0].BlockOwnerDeletion, "BlockOwnerDeletion must not be set on EndpointSlice OwnerReference")
						}
					}
