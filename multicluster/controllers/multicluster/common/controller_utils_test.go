...
func TestIsK8sVersionGreaterThanOrEqualTo(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := IsK8sVersionGreaterThanOrEqualTo(fakeDiscovery, "v1.31.0")
			if tt.expectedErr != "" {
...
func TestGetServiceCIDR(t *testing.T) {
	...
	tests := []struct {
		name         string
		cidrList     *networkingv1beta1.ServiceCIDRList
		expectedErr  string
		expectedCIDR string
	}{
		{
			name: "success",
			cidrList: &networkingv1beta1.ServiceCIDRList{
				Items: []networkingv1beta1.ServiceCIDR{
					{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fakeClient.Fake.PrependReactor("list", "servicecidrs", func(action k8stesting.Action) (handled bool, ret runtime.Object, err error) {
				return true, tt.cidrList, nil
			})
			result, err := GetServiceCIDR(ctx, fakeClient)
			if tt.expectedErr != "" {
				assert.EqualError(t, err, tt.expectedErr)
			} else {
				require.NoError(t, err)
